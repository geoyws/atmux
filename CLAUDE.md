...
Default `spawn()` timeout is 30s (`src/abstractions/spawn.ts::DEFAULT_TIMEOUT_MS`). Submodule-heavy projects (e.g. sopx — nested aix-root / std-root chains) regularly need >30s for cold submodule init, which surfaces as `SpawnTimeoutError` mid-`spawn-epic` worktree provisioning. Operators export `ATMUX_SPAWN_TIMEOUT_MS=<ms>` (e.g. `120000`) at team-start time to lift the default. Parsing fails closed: non-numeric / non-positive / non-finite values fall back to 30_000ms silently. Per-call `opts.timeoutMs` continues to take precedence over the resolved default. Source: t-681e5b91 (sopx cross-repo dispatch, FIX-A3 Gap #1).
