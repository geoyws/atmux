...
`ATMUX_BOOT_VERIFY_TIMEOUT_MS` is the **third distinct** seam, governing only how long ONE `C-m` submit waits for claude's composer to clear during member bootstrap. Default is `src/core/boot-claude.ts::DEFAULT_SUBMIT_VERIFY_TIMEOUT_MS`, raised 3_000 → **8_000** per t-f72e96ab; resolution via `resolveSubmitVerifyTimeoutMs()` with precedence per-call `opts.submitVerifyTimeoutMs` > env `ATMUX_BOOT_VERIFY_TIMEOUT_MS` > default, both override layers failing closed to the default on non-numeric / non-positive / non-finite values (a `0` never disables the timeout — an un-timed verify on a dead pane hangs the whole team start instead of failing one member). Unlike its two siblings this one is read at CALL time, not module load, so an operator or test may set it after import. Why it moved: 3_000ms × the one retry gave a 6s budget, and t-f72e96ab measured 21/21 members failing `submit-not-verified` across three unum epic-teams while a manual replay showed the composer DID clear just past that window — the verifier was manufacturing wedged panes and triggering the very re-send it exists to prevent. Source: t-f72e96ab (moved from the unum board 2026-08-24).

**This did NOT change [ADR-138](docs/adr/138-verified-send-keys.md).** That ADR's documented `timeoutMs // default 3000` is `src/core/safe-send.ts::DEFAULT_VERIFY_TIMEOUT_MS`, the generic verified-send-keys default, and it remains 3_000. Only boot-claude's own per-submit constant moved; it now passes 8_000 explicitly into the unchanged safe-send machinery.
