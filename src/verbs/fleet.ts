...
/** Epic-team entries carry the PARENT team's root (ADR-089 shared
 *  worktree), so neither their roster nor their session anchor can be
 *  resolved from the cockpit entry alone. Probing one would read the
 *  parent's cage and report the wrong answer, so they are reported as
 *  unreadable — visible and honest — rather than skipped. */
export const EPIC_TEAM_UNREADABLE_REASON =
  "epic-team shares the parent's root (ADR-089) — its own cage is not resolvable from the cockpit entry";
...
export async function sweepFleet(
  ...
  const worker = async (): Promise<void> => {
    for (;;) {
      ...
      if (team.type === "epic-team") {
        unreadable.push({ team: team.name, reason: EPIC_TEAM_UNREADABLE_REASON });
        continue;
      }
