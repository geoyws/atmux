...
function assertDeliverableLen(v: string, hint: string): string {
  if (v.length > DELIVERABLE_MAX) {
    throw new UsageError({
      what: `--deliverable exceeds ${DELIVERABLE_MAX} chars (got: ${v.length})`,
      hint,
    });
  }
  return v;
...
async function taskUpdate(argv: ReadonlyArray<string>): Promise<number> {
  ...
  while (i < rest.length) {
    ...
    if (a === "--deliverable") {
      ...
      if (v === undefined) {
        throw new UsageError({
          what: "task update: --deliverable requires a value",
          hint: USAGE_UPDATE,
        });
      }
      deliverable = v.length === 0 ? null : assertDeliverableLen(v, USAGE_UPDATE);
      i += 2;
      continue;
    }
    if (a === "--team-dir") {
...
export function parseAddArgs(argv: ReadonlyArray<string>): ParsedAddArgs {
  ...
  while (i < argv.length) {
    ...
    if (a === "--deliverable") {
      ...
      if (v === undefined) {
        throw new UsageError({
          what: "task add: --deliverable requires a value",
          hint: USAGE_ADD,
        });
      }
      if (v.length > 0) deliverable = assertDeliverableLen(v, USAGE_ADD);
      i += 2;
      continue;
    }
    if (a === "--") {
