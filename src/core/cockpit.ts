...
/** ADR-290 §D5 lane placement: an operator window named `_sdN` (N ≥ 2 as
 ...
 *  `_sd0`, `_sd1` (ADR-290 §D2: there is no `_sd1`) or zero-padded forms
 ...
 * `_sd` is the ADR-290 §D1 window-1 literal; `_superdriver` (ADR-135
  ...
  for (const window of cockpit.windows) {
    if (MALFORMED_LANE_RE.test(window.name) && !isSuperdriverLaneName(window.name)) {
      throw new ConfigError({
        ...
        hint: "ADR-290 §D2: there is no `_sd1` (window 1 is `_sd`); lanes are `_sd2`, `_sd3`, … with no zero-padding",
      });
    }
