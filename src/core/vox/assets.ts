...
//
// Pure map + resolver: the asset FILES land in `templates/vox/` in a
...
// path-traversal surface: anything not literally a key in VOX_ROUTES
...
export interface VoxAssetEntry {
...
function asset(file: string, mime: string, cacheControl: string): Readonly<VoxAssetEntry> {
  return Object.freeze({ file, mime, cacheControl });
}

/** The complete, closed route map. `/` aliases `index.html`. */
export const VOX_ROUTES: Readonly<Record<string, Readonly<VoxAssetEntry>>> = Object.freeze({
...
export interface ResolvedVoxAsset {
  filePath: string;
  mime: string;
  cacheControl: string;
}
 ...
 * pathname is not an exact VOX_ROUTES key. `Object.hasOwn` guards the
 * lookup so inherited prototype properties can never satisfy a route.
 * Default assets dir: `<templates>/vox` via `resolveTemplatesDir()`.
 */
export function resolveVoxAsset(
  pathname: string,
  opts?: { assetsDir?: string },
): ResolvedVoxAsset | null {
  const entry = Object.hasOwn(VOX_ROUTES, pathname) ? VOX_ROUTES[pathname] : undefined;
  if (entry === undefined) return null;
  const dir = opts?.assetsDir ?? resolve(resolveTemplatesDir(), "vox");
  return {
