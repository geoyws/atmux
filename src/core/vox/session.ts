...
// timers (VoxTimers seam), no wall clock (clock seam), no uuid source
...
//
// Ownership indirection: `VoxSharedState.owners` maps sessionId → the
...
//     (`src/core/vox/log.ts`). Default is a no-op, so a session built
...
//     `openTranscript` sink (`src/core/vox/transcript.ts`), which is
//     absent unless the operator set `ATMUX_VOX_TRANSCRIPTS=1`. The
...
import type { VoxConfig } from "./config.ts";
import {
  decodeFrame,
  encodeFrame,
  nextSeq,
...
import { toolJsonSchema, type VoxToolEntry } from "./tool-catalog.ts";
import type { VoxTranscriptSink } from "./transcript.ts";
 ...
 * Applied by {@link VoxServerSessionImpl.toolLabel} at every callsite
...
export interface ParkedLeg {
  ...
  seq: number;
  /** Grace-expiry timer handle (via the VoxTimers seam). */
  timer: unknown;
}

/** Server-wide singleton state shared by every connection's session. */
export interface VoxSharedState {
...
export function createVoxSharedState(opts: {
  clock: () => number;
  graceMs: number;
}): VoxSharedState {
  return {
...
export interface VoxSessionDeps {
  phone: PhoneLeg;
  ...
  catalog: ReadonlyArray<VoxToolEntry>;
   ...
   * Production points this at stderr through `createVoxLogger`, which
   ...
   * operator opted in with `ATMUX_VOX_TRANSCRIPTS=1` — a session built
...
class VoxServerSessionImpl implements VoxServerSession {
  private readonly deps: VoxSessionDeps;
  ...
  private transcript: VoxTranscriptSink | null = null;
  ...
  constructor(deps: VoxSessionDeps) {
    this.deps = deps;
    ...
    try {
      for (let attempt = 0; attempt < REDIAL_MAX_ATTEMPTS; attempt += 1) {
        ...
        try {
          ...
          if (outcome === "ready") {
            this.log(
              `vox: provider ready — ${this.providerTag()} attempt ${attempt + 1}/${REDIAL_MAX_ATTEMPTS} in ${this.deps.clock() - dialStart}ms`,
            );
            return true;
          }
        ...
        if (!this.isEnded()) {
          const next =
            attempt + 1 < REDIAL_MAX_ATTEMPTS
              ...
              : "no attempts left";
          this.log(
            `vox: dial attempt ${attempt + 1}/${REDIAL_MAX_ATTEMPTS} failed (${this.providerTag()}) — ${failure}${this.providerErrorSuffix()}; ${next}`,
          );
        }
      }
      this.log(
        `vox: dial exhausted — ${REDIAL_MAX_ATTEMPTS} attempts in ${this.deps.clock() - dialStart}ms (${this.providerTag()}); closing phone ${VOICE_CLOSE.PROVIDER} provider-unrecoverable; last failure: ${lastFailure}${this.providerErrorSuffix()}`,
      );
  ...
  private onAudioOut(pcm: Uint8Array): void {
    ...
    const frame = encodeFrame({ flags: VOX_FLAG_SYNTHETIC, seq: this.seq, payload: pcm });
   ...
   * under `ATMUX_VOX_READONLY=1` the messaging tools are genuinely
   ...
   */
  private logToolInvoked(entry: VoxToolEntry | undefined, name: string): void {
    const gate =
      entry === undefined
        ? "not in catalog"
        : entry.confirm
    ...
    this.log(`vox: tool ${this.toolLabel(name)} invoked (${gate})`);
  }
   ...
   * `ATMUX_VOX_READONLY` is cleared that difference is how a refusal is
  ...
  private logToolDone(
    ...
    if (stage === "previewed") {
      this.log(
        `vox: tool ${label} previewed in ${ms}ms — NOT executed, awaiting the operator's confirmation`,
      );
      return;
    }
    const notes: string[] = [];
    ...
    this.log(`vox: tool ${label} ${ok ? "ok" : "failed"} in ${ms}ms${suffix}`);
  }
  ...
  private async onToolCall(ev: { id: string; name: string; argsJson: string }): Promise<void> {
    ...
    if (this.conn === null || this.conn !== issuedBy) {
      ...
      this.log(
        `vox: dropping tool result for ${this.toolLabel(ev.name)} — the provider leg that issued it is gone (redial during execution)`,
      );
      return;
    }
    try {
  ...
  private noteProviderError(ev: { message: string; fatal: boolean; code?: string }): void {
    ...
    if (this.providerErrorsSeen === PROVIDER_ERROR_LOG_CAP + 1) {
      this.log(
        `vox: further provider errors on this leg suppressed (logged ${PROVIDER_ERROR_LOG_CAP})`,
      );
      return;
    }
    if (this.providerErrorsSeen > PROVIDER_ERROR_LOG_CAP) return;
    this.log(
      `vox: provider error${ev.fatal ? " (fatal)" : ""} (${this.providerTag()}) ${formatProviderError(note)}`,
    );
  }
  ...
  private async onProviderClosed(code: number | null, reason: string): Promise<void> {
    ...
    this.log(
      `vox: provider closed mid-session (${this.providerTag()}) ${formatCloseInfo(code, reason)}${this.providerErrorSuffix()} — redialing`,
    );
    this.sendStatus("thinking");
...
export function createVoxSession(deps: VoxSessionDeps): VoxServerSession {
  return new VoxServerSessionImpl(deps);
}
