...
// (RUNBOOK-vox §6). `scripts/vox-probe.ts` is a thin shim over this
...
// `VOX_SAMPLE_RATE`), so the probe has no binary dependency and runs
...
const USAGE =
  "bun scripts/vox-probe.ts --url <ws-url> --token <t> [--seconds N] [--text <s>] [--tone HZ]";
...
function requireValue(argv: ReadonlyArray<string>, i: number, flag: string): string {
  const v = argv[i + 1];
  if (v === undefined) {
    throw new UsageError({ what: `vox-probe: ${flag} requires a value`, hint: USAGE });
  }
  return v;
}
...
function positiveNumber(raw: string, flag: string): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    throw new UsageError({
      what: `vox-probe: ${flag} must be a positive number, got '${raw}'`,
      hint: USAGE,
    });
  }
  return n;
...
export function parseProbeArgs(argv: ReadonlyArray<string>): ProbeArgs {
  ...
  while (i < argv.length) {
    ...
    throw new UsageError({ what: `vox-probe: unknown arg: ${a ?? ""}`, hint: USAGE });
  }
  if (url === undefined) {
    throw new UsageError({ what: "vox-probe: --url is required", hint: USAGE });
  }
  if (token === undefined) {
    throw new UsageError({ what: "vox-probe: --token is required", hint: USAGE });
  }
...
/**
 * Synthesize a mono PCM16LE sine at `VOX_SAMPLE_RATE`. Sample n is
...
export function synthesizeTone(opts: {
  ...
  const rate = opts.sampleRate ?? VOX_SAMPLE_RATE;
...
/**
 * Slice PCM into wire frames: `VOX_BYTES_PER_FRAME` each, sequence
...
export function toneFrames(pcm: Uint8Array, startSeq = 0): Uint8Array[] {
  ...
  for (let off = 0; off < pcm.length; off += VOX_BYTES_PER_FRAME) {
    chunks.push(pcm.subarray(off, Math.min(off + VOX_BYTES_PER_FRAME, pcm.length)));
  }
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    ...
    frames.push(
      encodeFrame({
        flags: last ? VOX_FLAG_TURN_END : 0,
        seq,
...
export async function runProbe(deps: ProbeDeps): Promise<ProbeResult> {
  ...
  ws.send(JSON.stringify({ type: "hello", v: 1, token: args.token, mode: "ptt", ua: "vox-probe" }));
  ...
  if (args.text !== undefined) {
    ...
    log(`streaming ${frames.length} frames (${pcm.length} bytes PCM16 @ ${VOX_SAMPLE_RATE} Hz)`);
    for (const f of frames) {
      if (result.closeCode !== null) break;
      ws.send(f);
      result.uplinkFrames += 1;
      // Real-time pace: one nominal frame per VOX_FRAME_MS. A server
