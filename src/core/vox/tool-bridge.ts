...
//
// Vox runs unattended in a detached tmux session, so the service being
...
// wrapper cannot run two verbs concurrently — so `atmux vox --stop`
...
export interface ToolBridgeDeps {
  ...
  runners: Partial<Record<VoxRunnerKey, VerbRunner>>;
