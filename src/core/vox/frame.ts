...
//   byte 0    magic    0xa1 (VOX_MAGIC_PCM16_V1 — PCM16 wire v1)
...
export const VOX_MAGIC_PCM16_V1 = 0xa1;
/** Flags bit0 — this frame ends the speaker's turn. */
export const VOX_FLAG_TURN_END = 0x01;
/** Flags bit1 — server-generated (synthetic) audio, not mic capture. */
export const VOX_FLAG_SYNTHETIC = 0x02;
/** Header size in bytes (magic + flags + uint16 seq). */
export const VOX_HEADER_BYTES = 4;
/** PCM sample rate on the wire (Hz). */
export const VOX_SAMPLE_RATE = 24000;
/** Nominal frame duration (ms). */
export const VOX_FRAME_MS = 40;
/** Samples per nominal frame: 24000 Hz × 40 ms. */
export const VOX_SAMPLES_PER_FRAME = 960;
/** Payload bytes per nominal frame: 960 samples × 2 bytes. */
export const VOX_BYTES_PER_FRAME = 1920;

export interface VoxFrameInput {
  flags: number;
  seq: number;
  payload: Uint8Array;
}

export type DecodedVoxFrame =
  | {
      ok: true;
      flags: number;
      seq: number;
...
export function encodeFrame({ flags, seq, payload }: VoxFrameInput): Uint8Array {
  const out = new Uint8Array(VOX_HEADER_BYTES + payload.length);
  const view = new DataView(out.buffer);
  view.setUint8(0, VOX_MAGIC_PCM16_V1);
  view.setUint8(1, flags & 0xff);
  view.setUint16(2, seq & 0xffff, true);
  out.set(payload, VOX_HEADER_BYTES);
  return out;
}
 ...
 * buffer offset).
 */
export function decodeFrame(buf: Uint8Array): DecodedVoxFrame {
  if (buf.length < VOX_HEADER_BYTES) return { ok: false, reason: "short" };
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  if (view.getUint8(0) !== VOX_MAGIC_PCM16_V1) return { ok: false, reason: "bad-magic" };
  if ((buf.length - VOX_HEADER_BYTES) % 2 !== 0) return { ok: false, reason: "odd-payload" };
  const flags = view.getUint8(1);
  return {
    ok: true,
    flags,
    seq: view.getUint16(2, true),
    turnEnd: (flags & VOX_FLAG_TURN_END) !== 0,
    synthetic: (flags & VOX_FLAG_SYNTHETIC) !== 0,
    payload: buf.subarray(VOX_HEADER_BYTES),
  };
}
