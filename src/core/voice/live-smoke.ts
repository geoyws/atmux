...
//
// `scripts/voice-live-smoke.ts` is a thin shim over this module; the logic
// lives here so it sits inside the `src/**` coverage universe (the same
// split `scripts/voice-probe.ts` → `src/core/voice/probe.ts` uses).
//
// ---------------------------------------------------------------------
...
export function formatLiveSmoke(r: LiveSmokeResult): string {
  ...
  return `voice-live-smoke: ${parts.join(" ")}`;
}
