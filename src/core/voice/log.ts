...
//      {@link redactVoiceSecrets} on the way out — a caller cannot forget
...
//      `ATMUX_VOICE_TOKEN`), plus shape-based patterns for a credential
...
//      TRANSCRIPTS to `~/.atmux/voice-logs/`, local-only, 7-day
...
export const VOICE_REDACTED = "<redacted>";
 ...
 * chars (`ATMUX_VOICE_TOKEN` is refused below 32; provider API keys are
...
export type VoiceLog = (line: string) => void;
...
const SECRET_PATTERNS: ReadonlyArray<{ re: RegExp; replace: string }> = Object.freeze([
  ...
  // uses. Stops at the next `&`, whitespace, or quote.
  {
    ...
    replace: `$1${VOICE_REDACTED}`,
  },
  // `Authorization: Bearer <key>` in any error text.
  { re: /\b(Bearer\s+)\S+/gi, replace: `$1${VOICE_REDACTED}` },
  // The OpenAI browser-style subprotocol element carrying the raw key.
  { re: /(openai-insecure-api-key\.)[^\s,"'`\]]+/gi, replace: `$1${VOICE_REDACTED}` },
  // Bare OpenAI-shaped keys (`sk-…`), wherever they appear.
  { re: /\bsk-[A-Za-z0-9_-]{8,}/g, replace: VOICE_REDACTED },
]);
 ...
 */
export function redactVoiceSecrets(line: string, secrets: ReadonlyArray<string>): string {
  let out = line;
  for (const secret of secrets) {
    if (secret.length < MIN_REDACTABLE_SECRET_CHARS) continue;
    const encoded = encodeURIComponent(secret);
    if (encoded !== secret) out = out.split(encoded).join(VOICE_REDACTED);
    out = out.split(secret).join(VOICE_REDACTED);
  }
...
export interface CreateVoiceLoggerOpts {
 ...
 */
export function createVoiceLogger(opts: CreateVoiceLoggerOpts): VoiceLog {
  ...
  return (line: string): void => {
    write(`${redactVoiceSecrets(line, secrets)}\n`);
  };
}
