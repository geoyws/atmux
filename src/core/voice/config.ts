...
export interface VoiceConfig {
  /** ATMUX_VOICE_TOKEN — required, ≥32 chars (ADR-272 §Security). */
  token: string;
  /** ATMUX_VOICE_PROVIDER — raw string; parsed at P4 boot wiring. */
  provider: string;
  /** ATMUX_VOICE_MODEL — optional provider-model override. */
  model?: string;
  /** ATMUX_VOICE_PORT — default 4390. */
  port: number;
  /** ATMUX_VOICE_HOST — default loopback (ADR-272 §Security layer 4). */
  host: string;
  /** ATMUX_VOICE_ORIGINS — comma-separated allowlist → string[]. */
  origins: string[];
  /** ATMUX_VOICE_TOOL_TIMEOUT_MS — default 20_000. */
  toolTimeoutMs: number;
  /** ATMUX_VOICE_MAX_RESULT_CHARS — default 2000. */
  maxResultChars: number;
  /** ATMUX_VOICE_READONLY — "1"/"true" (case-insensitive) → true. */
  readonly: boolean;
  /** ATMUX_VOICE_RESUME_GRACE_MS — default 90_000 (ADR-272 D8). */
  resumeGraceMs: number;
  /** ATMUX_VOICE_CONFIRM_TTL_MS — default 120_000 (ADR-272 D7). */
  confirmTtlMs: number;
...
export const VOICE_TOKEN_MIN_CHARS = 32;
 ...
 * than {@link VOICE_TOKEN_MIN_CHARS}; every other knob degrades to its
...
export function resolveVoiceConfig(
  ...
  const token = resolveOptionalString(flags?.token, env.ATMUX_VOICE_TOKEN);
  if (token === undefined) {
    throw new ConfigError({
      what: `ATMUX_VOICE_TOKEN is required (≥${VOICE_TOKEN_MIN_CHARS} chars) — the voice server refuses to start without one`,
      hint: TOKEN_HINT,
    });
  }
  if (token.length < VOICE_TOKEN_MIN_CHARS) {
    throw new ConfigError({
      what: `ATMUX_VOICE_TOKEN too short: ${token.length} chars (need ≥${VOICE_TOKEN_MIN_CHARS})`,
      hint: TOKEN_HINT,
    });
  }
  ...
  const origins =
    flags?.origins !== undefined
      ...
      : parseOriginsList(env.ATMUX_VOICE_ORIGINS);

  const model = resolveOptionalString(flags?.model, env.ATMUX_VOICE_MODEL);
  const assetsDir = resolveOptionalString(flags?.assetsDir, env.ATMUX_VOICE_ASSETS_DIR);

  const out: VoiceConfig = {
    token,
    provider: resolveString(flags?.provider, env.ATMUX_VOICE_PROVIDER, VOICE_DEFAULTS.provider),
    port: resolveNumber(flags?.port, env.ATMUX_VOICE_PORT, VOICE_DEFAULTS.port),
    host: resolveString(flags?.host, env.ATMUX_VOICE_HOST, VOICE_DEFAULTS.host),
    origins,
    toolTimeoutMs: resolveNumber(
      flags?.toolTimeoutMs,
      env.ATMUX_VOICE_TOOL_TIMEOUT_MS,
      VOICE_DEFAULTS.toolTimeoutMs,
    ),
    maxResultChars: resolveNumber(
      flags?.maxResultChars,
      env.ATMUX_VOICE_MAX_RESULT_CHARS,
      VOICE_DEFAULTS.maxResultChars,
    ),
    readonly: flags?.readonly ?? parseBooleanEnv(env.ATMUX_VOICE_READONLY),
    resumeGraceMs: resolveNumber(
      flags?.resumeGraceMs,
      env.ATMUX_VOICE_RESUME_GRACE_MS,
      VOICE_DEFAULTS.resumeGraceMs,
    ),
