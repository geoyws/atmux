...
// 2. **It is skippable and bounded.** `ATMUX_VOICE_SKIP_MODEL_CHECK=1`
...
//    `createVoiceLogger` on top of that. Three layers, because a boot
...
export const SKIP_MODEL_CHECK_ENV = "ATMUX_VOICE_SKIP_MODEL_CHECK";
...
export async function checkModelPin(deps: ModelCheckDeps): Promise<ModelCheckResult> {
  ...
  if (parseBooleanEnv(env[SKIP_MODEL_CHECK_ENV])) {
...
export function formatModelCheck(r: ModelCheckResult): string[] {
  ...
  switch (r.status) {
    case "ok":
      return [`voice: model check ok — ${pin} is in the provider's list of ${r.available} models`];
    case "skipped":
      return [`voice: model check SKIPPED (${r.detail}) — a retired model id will not be caught`];
    case "unreachable":
      return [
        `voice: model check could not run — ${r.detail}. Serving anyway; the pin ${pin} is UNVERIFIED`,
      ];
    default:
      return [
        `voice: ${RULE}`,
        `voice: ## MODEL PIN DRIFT — '${r.model}' is NOT in ${r.kind}'s model list (${r.available} models)`,
        ...
        ...(r.suggestions.length > 0
          ? [`voice: ## Closest available: ${r.suggestions.join(", ")}`]
          : ["voice: ## No similarly-named model was offered — check the provider's console."]),
        "voice: ## Fix: export ATMUX_VOICE_MODEL=<id>, or pass --model <id>.",
        `voice: ## Skip this check with ${SKIP_MODEL_CHECK_ENV}=1 (offline / dev only).`,
        `voice: ${RULE}`,
      ];
  }
}
