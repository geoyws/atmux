 ...
 * check, and `voice-e2e` is not a name an operator would ever pick.
 */
export const FAKE_TEAM_PREFIX = "voice-e2e-";
...
function refuse(what: string, hint: string): never {
  throw new ConfigError({ what: `voice-e2e isolation: ${what}`, hint });
}
