...
export interface VoiceTeamIndex {
  teams: VoiceTeamEntry[];
}
...
export async function buildTeamIndex(deps: BuildTeamIndexDeps = {}): Promise<VoiceTeamIndex> {
  const load = deps.loadCockpit ?? (() => loadCockpit());
  const cockpit = await load();
  const teams = enabledTeams(cockpit).map(
    (t): VoiceTeamEntry => ({ name: t.name, root: t.root, type: t.type }),
  );
  return { teams };
}

export type ResolveTeamResult =
  | { ok: true; team: VoiceTeamEntry }
 ...
 */
export function resolveTeamName(index: VoiceTeamIndex, spoken: string): ResolveTeamResult {
  ...
  const rungs: Array<(t: VoiceTeamEntry) => boolean> = [
    (t) => t.name === spoken,
