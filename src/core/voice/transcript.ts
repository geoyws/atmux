...
// `ATMUX_VOICE_READONLY` without it would mean the mutating surface goes
...
//
//   1. LOCATION — `~/.atmux/voice-logs/`. atmux's own state directory,
...
//      do that?" the morning after. `ATMUX_VOICE_TRANSCRIPT_RETENTION_DAYS`
//      lets the operator SHORTEN it (OQ-4 grants that explicitly).
//
//   3. OFF BY DEFAULT — writing requires `ATMUX_VOICE_TRANSCRIPTS=1`.
...
//      arrive in. (`src/core/voice/log.ts` is the STDERR sink and carries
...
export const VOICE_TRANSCRIPT_DIR_REL = ".atmux/voice-logs";
...
export const TRANSCRIPT_FILE_PREFIX = "voice-";
 ...
 */
export const TRANSCRIPT_FILE_RE = /^voice-[A-Za-z0-9_-]+\.jsonl$/;
...
/**
 * Resolve `~/.atmux/voice-logs`. `$HOME` first (the value every other
...
export function resolveTranscriptDir(opts: { env?: NodeJS.ProcessEnv } = {}): string {
  ...
  return join(home, VOICE_TRANSCRIPT_DIR_REL);
}

/**
 * `voice-<sanitized-session-id>.jsonl`.
 *
...
export interface VoiceTranscriptSink {
  ...
  record(ev: VoiceTranscriptEvent): void;
}
 ...
 * self-heals.
 */
export function createTranscriptSink(opts: CreateTranscriptSinkOpts): VoiceTranscriptSink {
  ...
  return {
    ...
    record(ev: VoiceTranscriptEvent): void {
      ...
      try {
        ...
        if (!failureLogged) {
          failureLogged = true;
          log(`voice: transcript write failed — ${e instanceof Error ? e.message : String(e)}`);
        }
      }
    },
  };
...
export function formatPruneResult(dir: string, r: PruneTranscriptsResult): string {
  return `voice: transcript prune ${dir} — removed ${r.removed}, kept ${r.kept}, skipped ${r.skipped}, errors ${r.errors}`;
}
...
export interface TranscriptPruneLoopOpts {
  /** Timer seam (the session's `VoiceTimers`; fake in tests). */
