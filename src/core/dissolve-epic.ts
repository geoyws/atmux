// ADR-232 §D1 + ADR-090 §dissolve-epic pure core (factored from
// src/verbs/team/dissolve-epic.ts).
//
// Owns the dissolve-epic pipeline as a structured-input function so
// both the CLI verb wrapper (src/verbs/team/dissolve-epic.ts) and the
// cross-cage dispatcher (src/core/orchd-dispatchers.ts ::
// dispatchDissolveEpic) can call it without re-walking argv. Same
// factoring pattern as performEpicMerge (src/core/epic-merge.ts) per
// ADR-091.
//
// Pipeline (unchanged from pre-factor verb body):
//
//   1. Caller-scope gate (driver-only).
//   2. Find epic-team in cockpit.json — walk sessions[] tree.
//   3. Resolve epic root + parent root from cockpit topology.
//   4. Pre-flight gates (skipped under skipChecks): all-tasks-done +
//      clean-worktree (matches epic-merge gate-pass cond).
//   5. defaultCageTeardown (softStop + killSession + killServer) for
//      the child cage.
//   5a. removeEpicViewerFromParentCage — symmetric to atmux start's
//       add.
//   6. pruneWorktree (`dirty: 'skip'` by default; force on
//      forcePrune | skipChecks).
//   6a. deleteMergedEpicBranch — closes "merged-but-not-deleted"
//       residue class (e-7a1014f9 §Fix #2).
//   7. Remove epic-team entry from parent's cockpit sessions[].
//   8. Mark parent's kanban EPIC row done.
//
// Out of scope: argv parsing (verb wrapper owns); cross-cage routing
// (orchd-dispatchers.ts owns); trunk-merge gate (ADR-091).
...
/** Structured input to {@link performDissolveEpic}. Replaces argv +
 *  flag-walking; the CLI verb wrapper parses these out of argv via
 *  `parseDissolveEpicArgs` (verb-file local). Zod schema is exported
 *  so the cross-cage dispatcher can validate event payloads before
 *  invoking the local-route. */
export const PerformDissolveEpicInput = z.object({
  epicId: z.string().min(1),
  skipChecks: z.boolean().default(false),
  forcePrune: z.boolean().default(false),
});
export type PerformDissolveEpicInput = z.infer<typeof PerformDissolveEpicInput>;

// ---------- Dep injection (test seam) ----------
...
/**
 * Pure-core dissolve-epic pipeline. Takes structured input (no argv)
 * so cross-cage dispatchers + the CLI verb wrapper share one entry
 * point. Returns `0` on success; throws {@link ConfigError} on
 * pre-flight refusal or topology-not-found.
 */
export async function performDissolveEpic(
  input: PerformDissolveEpicInput,
  ...
  if (found === null) {
    throw new ConfigError({
      what: `dissolve-epic: epic-team '${input.epicId}' not found in cockpit ${cockpitPath}`,
  ...
  const epicRoot = join(`${parentRoot}-epics`, input.epicId);
  ...
  if (!input.skipChecks && childTeam !== null) {
    ...
    logger.warn(
      `dissolve-epic: --skip-checks bypassing all-tasks-done + clean-worktree gates for '${input.epicId}' (operator owns the consequences per ADR-090 resolved-open #5)`,
    );
  }
  ...
  if (childTeam !== null) {
    ...
    try {
      ...
      logger.warn(
        `dissolve-epic: cage teardown failed for '${input.epicId}' — continuing with prune (${e instanceof Error ? e.message : String(e)})`,
      );
    }
  }
  ...
  if (await exists(epicRoot)) {
    const dirtyMode: "skip" | "force" = input.forcePrune || input.skipChecks ? "force" : "skip";
  ...
  if (parentEntry.sessions !== undefined) {
    parentEntry.sessions = parentEntry.sessions.filter(
      (s) => !(s.type === "epic-team" && s.name === input.epicId),
    );
  ...
  logger.log(`epic-team dissolved: ${input.epicId} (parent=${parentEntry.name ?? "<unnamed>"})`);
  return 0;
}
