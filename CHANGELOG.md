...
- `_bot` is a distinct typed send target. ADR-239's driver send-keys prohibition was still in force when this landed — *superseded 2026-09-08, when the operator revoked [ADR-239](docs/adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §D2; the `_bot` kind and its fail-closed worktree setup are unaffected* — and bot worktree setup fails closed instead of falling back to shared trunk.
...
> ⚠️ **The roster is the practical limit now — not the driver rule.** Delivery goes through `atmux send`, which addresses roster members, while the sweep enumerates from **tmux** windows, so a pane can be *reported* and not be *nudgeable*. Most `idle-residue` findings on the live fleet sit on `driver` / `driver-N` windows, because 14 of 15 enabled teams carry `members: []` and run their work in driver windows. **Driver panes are no longer refused:** the operator revoked [ADR-239](docs/adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §D2 on 2026-09-08, the `DriverSendKeysViolation` runtime guard and the up-front `isDriverPaneName` refusal are gone, and a driver pane takes input like any other pane whose state was observed first. The roster asymmetry is documented rather than hidden: closing it would mean either a roster-driven sweep (which reports "all clear" across a working fleet) or a delivery path that is not `atmux send` (which §D5 forbids).
