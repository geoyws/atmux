...
**ADR-282's guard and helper are kept, with three corrections:**
...
- **The `templates/` root's stated reason was false and hid a live blind spot.** The four new roots were justified as "all carry executable shell"; `templates/` carries **no shell at all**, and `templates/tmux/atmux.conf` — the file this series' own colour work edits — was **unscannable** because `.conf` was not in `SCAN_EXTS`. A `run-shell 'env > /tmp/x'` in a shipped conf would have sailed through. A tmux conf **does** execute shell (measured 2026-08-29, tmux 3.7c: a conf with `run-shell "echo … > <marker>"` wrote the marker), so `.conf` is now scanned, and a new leg enumerates the extensions actually present in the scanned roots and fails if an executable one is unscanned.
- **The matcher's claim is now a measured ratio, pinned by an executing test.** Re-enumerated first-hand over **36** genuine whole-environment-capture shapes: it catches **23** and misses **13**. Both halves are asserted, so the number cannot drift from the code. The missed 13 by class: **5 runtime-object** (`console.log(process.env)`, `JSON.stringify`, spread, `Object.entries`, snapshot — missed *by design*, since `process.env` is read legitimately across `src/`), **3 shell-builtin** (`export -p`, `declare -x`, `set`), **1 kernel-interface** (`/proc/self/environ`), **1 process-table** (`ps eww`), **1 quoted-shell-word** (`sh -c "env" > file`), **1 line-split-argv**, **1 runtime-assembly**. The alternate-dumper class was absent from the previous residual-gaps list entirely. It is **a tripwire for enumerated shapes, not a proof of absence.**
