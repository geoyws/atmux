...
### 🔒 Security — a test can no longer capture the whole environment ([ADR-282](docs/adr/282-never-collect-the-whole-environment-in-a-test.md))
...
- **New guard `tests/regression/no-unfiltered-env-dump.test.ts`** scans `src/` and `tests/` and fails the suite on `env`/`printenv` redirected to a path, or piped to anything that is not `grep`, naming the file, the line and the safe alternative. Comment lines are skipped (a comment does not execute); there is **no path exclusion list**, and the guard's own fixtures are assembled at runtime so none is needed. Mutation-proven both ways: reintroducing the 2026-08-28 shape turns it red, and it caught its own helper when `dumpEnvCommand` was mutated to stop filtering.
