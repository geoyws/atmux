...
### 🧭 Changed — every command-mode pane is shell-backed; `_medic` opens to `zsh -l`

**No pane atmux creates dies with its TUI any more.** A pane whose start command is a TUI or foreground process is killed by tmux the moment that process exits (remain-on-exit is off) — the recurring `_medic` disappearance. Every command-mode spawn now goes through one shared wrap, `src/core/tui-cmd.ts::shellFallbackCommand` (`sh -c '<cmd>; exec $SHELL -i'`, the shape ADR-239 §A5 already proved on driver panes): the `_superbot` scheduler window and `cockpit rotate medic`'s respawn drop to an interactive shell when their command exits instead of taking the window with them. The cockpit `_medic` window itself now opens to a plain `zsh -l` login shell (operator-direct, 2026-09-23) — never a claude TUI; claude is started by hand (c-alias wrapper, ADR-167) or via `cockpit rotate medic`, and `claudeAccount` / `tuiOverrides` stay on the config for the rotate respawn matrix. `buildMedicWindowCommand` returns `zsh -l` and the inline-env claude builder `buildClaudeWindowCommand` plus the deprecated `buildSuperdoctorWindowCommand` alias are gone. Cage driver / bot / member panes were already shell-backed (ADR-239 §A5 wrap, bot `exec $SHELL -i`, member send-keys) and are unchanged.
