...
### 📐 Proposed — atmux as an OpenCode plugin + headless Rust daemon ([ADR-262](docs/adr/262-atmux-opencode-plugin-and-daemon.md))

Two-layer architecture: thin TypeScript OpenCode plugin (bridge — registers `atmux_*` tools, hooks `session.idle`/`session.error`/`permission.ask`) + Rust daemon (`atmuxd` — owns SQLite state, Honker events, session lifecycle via `opencode serve` SDK). Zero tmux dependency. Dual-harness (same daemon serves opencode + Claude Code frontends). Supplements ADR-258 §D3.3.
