...
### 🔒 Security — the test runner's environment no longer carries the operator's credentials ([ADR-283](docs/adr/283-scrub-the-test-runner-environment.md))

**Three adversarial reviews returned DEFECTIVE on the commit below with nine MAJOR findings. The governing one: it shipped documentation claiming a guarantee the code did not deliver — the exact sin it convicted [ADR-277](docs/adr/277-cage-color-environment-scrub.md) of.** ADR-282 said its source guard meant "the class cannot come back" and that "taking any other route fails the suite". A reviewer drove sixteen whole-environment-capture shapes through that guard's own matcher and reported four caught. That report's arithmetic does not close, so the comparison was re-derived here against an enumeration written down in full: **of 21 shapes, the `bb47d0b6` matcher caught 9 and missed 12** — the bare relative redirect target it had already named as a known gap, `env -0`/`--null`/`printenv -0` in shell and argv form, command substitution, and every argv-array form including `Bun.spawnSync({ cmd: ["env"], stdout: "pipe" })`, the idiomatic TypeScript route, which the very file the guard protects already used four times for other commands. The widened matcher catches all 21 — which is not the claim it looks like, since it was widened *against* that enumeration and says nothing about a shape nobody wrote down.

- **`bun run test` (`scripts/test.ts`) is the entrypoint, and it builds the runner's environment from an allowlist.** The operator's `.zshrc` sources a git-crypt'd `.env` into every shell; **29** of that shell's ~125 variable names match a credential pattern. Remove them from the runner and **no test can leak them, whatever shape it uses** — a property of the environment, not of a regex, so there is nothing to evade. Measured on `geoywsMBP`: **21 variables through, 103 withheld.** Allowlist rather than denylist because a name pattern is a guess (`DATABASE_URL` carries a password and matches none of the obvious one); an unfamiliar name is now withheld rather than admitted. `ATMUX_TEST_ENV_PASSTHROUGH=NAME1,NAME2` is the escape hatch, and every admitted name is echoed to stderr. The receipt prints **counts, never names**.
- **A bare `bun test` is now REFUSED** on a box whose environment carries credential-shaped names, by a bunfig preload beside the existing cage guard, naming `bun run test` and the `ATMUX_TEST_ENV_OK=1` override. It refuses rather than scrubs **because it cannot scrub**: measured on bun 1.3.14, `Bun.spawn`/`Bun.spawnSync` without an explicit `env` use the environment as it stood at process start, so a `delete process.env.X` in a preload leaves `Bun.spawnSync({ cmd: ["env"] })` still dumping X. The two layers check each other — `scripts/test.ts` sets the marker the preload looks for **only** when its own output has zero credential-shaped names left. CI's test step goes through the same door.
- **The ADR-282 guard is kept, widened, and its claim cut to what it does.** New shapes caught: a bare relative redirect target (`env > out`, ADR-282's stated gap — closed by requiring a shell statement end and refusing a dumper preceded by `>`, which is what this repo's `flag > env > default` prose looks like), `env -0`/`--null` in shell and argv form, a dumper as the **last** argv element (`["env"]`, `["sh","-c","env"]`) while a prefix (`["env","-u","NO_COLOR","tmux"]`) and a single-variable read (`["printenv","TERM"]`) stay legal, and `$(env)`. New roots: `scripts/`, `bin/`, `templates/`, `plugins/` — all carry executable shell, and the first two are outside biome's scope so nothing else reads them. **Its own assertion was vacuous**: `scan()` returns `[]` both when nothing violates and when nothing was scanned, and emptying `SCAN_EXTS` left it green with a real violation planted on disk. Now pinned by per-root file-count floors and a leg that plants a violation in a `mktemp` tree and requires `scan()` to find it. It is documented as a **tripwire for enumerated shapes, not a proof of absence**, with the residual gaps listed.
- **`dumpEnvCommand` is capped at 8 variables and refuses a credential-shaped name** — without a cap the sanctioned helper builds the dump it exists to prevent, at a call site the guard reads as an ordinary helper call. Its `outPath` refused quotes only while being spliced next to `;`, `&`, backtick, `$(`, `>` and whitespace; it now takes a character allowlist. `parseEnvDump` keeps only the **first** sighting of each name and redacts values over 256 chars — a secret containing a newline followed by `TERM=` used to produce a line that looked like a legitimate assignment and was a fragment of the secret.
- **The credential pattern was matching `PATH`.** `/(TOKEN|SECRET|KEY|PASSWORD|PASSWD|PAT|WEBHOOK|CREDENTIAL|AUTH)/i` matched bare substrings, so `PAT` hit `PATH` and `KEY` hit `MONKEY` and `COMPATIBILITY`. Now: substring for the long unambiguous words (which is what catches `PGPASSWD`), `_`-segment anchored for the short collision-prone ones, and `AUTH(?:ORIZATION)?` so `AUTHORIZATION` matches while `GIT_AUTHOR_NAME` does not. Single-sourced — two copies of a security predicate are two answers to one question.
- **Seven tmux spawn sites, seven tests.** ADR-281's stated reason for a shared constant is "a future call site forgetting", and nothing detected that: a reviewer deleted `unsetEnv` from six of the seven and the full 10,404-test suite came back **byte-identical**, with `rg TMUX_CHILD_UNSET_ENV tests/` returning zero hits. `tests/unit/abstractions/tmux-child-env.test.ts` drives every site and asserts the environment that actually reached `Bun.spawn` — the effective env, not the option, so an `unsetEnv` still passed but no longer honoured fails too — plus a control leg requiring the variable to arrive without the policy. All ten mutants (seven sites, plus poke's three consumer lines) go red on exactly the right assertion. `TMUX_CHILD_ENV_ARGV` ↔ `TMUX_CHILD_UNSET_ENV` correspondence, previously prose, is now a test.
- **The biome include no longer regresses the CI gate.** `bunx biome check .` — what CI runs — went **156 → 162 errors** when ADR-282 added the two directories; the previous `LINT_EXIT=0` came from `bun run lint`, which is `biome lint .` only and skips the format and assist passes. The six (4 `format`, 2 `organizeImports`, all in pre-existing `tests/helpers/` files the include newly brought into scope) are fixed and the count is back to **156**. ⚠ **Stated plainly: `bunx biome check .` still exits 1 at 156, and did before any of this work** — all 156 are `format`/`organizeImports` in files untouched by `90db9831..HEAD`. A repo-wide formatting sweep is deliberately not bundled into a security change.
- **`describe.if(HAS_TMUX)` and `HAS_TMUX` itself.** The reviewer's diagnosis is half right and worth correcting: bun 1.3.14 **does** count a `describe.if(false)` block's tests in the skip total (measured: 9 tests, 9 skips), so they were never invisible — what was missing is a **gate**. Added a `@skip-reason` and an unconditional leg asserting `not (on CI and no tmux)`. Separately, `HAS_TMUX` probed the bare PATH (`command -v tmux`) while the code under test resolves via `resolveTmuxBin()` ([ADR-191](docs/adr/191-vendored-tmux-binary.md)), so the raw-tmux **control** legs could run a different binary from the one `createTmux` drives — both now use `resolveTmuxBin()`.
- **`templates/tmux/atmux.conf` gains `set-environment -g COLORTERM truecolor`, because ADR-281 said twice (§D2, §Consequences) — and `docs/ARCHITECTURE.md` a third time — that it already had one, and it did not.** Its only `set-environment` was `NO_COLOR`; its only `COLORTERM` occurrence was in a comment. The line existed solely in the operator's personal `~/.tmux.conf`, which atmux does not ship — so the compensating control ADR-281 leaned on when withdrawing the spawn-level injection did not exist in anything deployed. Fixed by making the claim true: the line sits after the ADR-277 scrub and **above** the ADR-171 `source-file`, so the scrub stays a default rather than a lock. Measured before shipping, tmux 3.7c: server-side `COLORTERM` set + client carrying none → `#{client_termfeatures}` = `bpaste,ccolour,clipboard,cstyle,focus,title`, **no `RGB`** — so the server half genuinely does not assert 24-bit capability on the operator's behalf, which was the whole reason the client half was withdrawn. On 3.7c it is **inert for the pane** (tmux sets `COLORTERM=truecolor` there itself, measured with and without the line); its stated value is the 3.6b case ADR-281 measured, which has **not** been reproduced here. Two regression legs keep it present and correctly ordered. **This error came from the instructions the previous round was given, not from that agent's own work.**
- **ADR-281 §D2's client-path measurement re-run and confirmed** after one of three reviewers could not reproduce it: no client `COLORTERM` → `bpaste,ccolour,clipboard,cstyle,focus,title`; `COLORTERM=truecolor` → the same plus `RGB`. Three things decide whether it reproduces and are now recorded in the ADR: the client needs a real pty (`script -q /dev/null`, read the feature string from outside via `list-clients -F`), `TERM` must be identical in both legs, and the server must carry no `COLORTERM` of its own. ⚠ Every measurement in ADR-281 and ADR-283 is **tmux 3.7c on macOS**; the vendored pin is **3.6a** and CI installs **3.4**, and neither has been measured.
- **A pre-existing race in `tests/unit/verbs/start.test.ts`, surfaced and fixed.** Two legs read `#{pane_current_command}` immediately after `runStart` with no settle, which is a race by construction: tmux reports the pane's foreground process at that instant, and a just-exec'd shell can still be reporting its launcher. Reproduced 2026-08-28 at **1 failure in 4 isolated runs** of that file under load. Now polled to a 2s deadline, returning whatever was last seen so a failure still prints the real command. Mutation-proven not to be a loosening: making a null-`tui` driver launch `sh` instead of `zsh` turns both legs red and reverting turns them green.
- **The full suite is unchanged**: 10327 pass / 8 skip / 2 todo / 67 fail / 5 errors at `bb47d0b6` and again through the entrypoint with the scrub as the only change (10404 tests, 350 files); **10367 pass / 8 skip / 2 todo / 67 fail / 5 errors** with everything in place (10444 tests, 352 files — the extra 40 tests are this change's own). The 67-name failure SET is identical to the baseline in both directions at both points. One benign difference: the scrub withholds `CLAUDECODE`, and bun's reporter suppresses `(pass)` lines when it is set, so a wrapped run prints ~12,700 lines where a bare one printed ~1,500.
...
### 🔒 Security — probes collect an allowlist, and a guard for the known capture shapes ([ADR-282](docs/adr/282-never-collect-the-whole-environment-in-a-test.md))
...
- **A redactor as the seatbelt, never the brake** — `parseEnvDump` replaces the value of any surviving name matching `/(TOKEN|SECRET|KEY|PASSWORD|PASSWD|PAT|WEBHOOK|CREDENTIAL|AUTH)/i`, matching the **name** only so it cannot itself leak. It is explicitly not the defence: a name pattern is a guess, and `DATABASE_URL` carries a password while matching none of it.
- **New guard `tests/regression/no-unfiltered-env-dump.test.ts`** scans `src/` and `tests/` and fails the suite on `env`/`printenv` redirected to a path, or piped to anything that is not `grep`, naming the file, the line and the safe alternative. ⚠ **This guard was described here and in ADR-282 as closing the class; it does not** — see the ADR-283 entry below, which retracts that claim and adds the layer that does hold. Comment lines are skipped (a comment does not execute); there is **no path exclusion list**, and the guard's own fixtures are assembled at runtime so none is needed. Mutation-proven both ways: reintroducing the 2026-08-28 shape turns it red, and it caught its own helper when `dumpEnvCommand` was mutated to stop filtering.
