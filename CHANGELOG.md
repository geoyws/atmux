...
### ✨ Added — split legacy/vendored tmux resolvers + `vendored-tmux-binary` doctor probe (ADR-191, e-162046c7)
...
- **Resolver helpers**: `src/core/resolve-tmux-bin.ts::resolveTmuxBin()` keeps ordinary/legacy calls on `ATMUX_TMUX_BIN` (operator override) → system `tmux` on PATH, while `resolveVendoredTmuxBin()` resolves only the explicit exact-3.7c vendored path and fails closed. Per-process memoization avoids re-probing the filesystem on every tmux spawn. Injectable env / existsSync / state seams mirror `resolveDefaultListenerBinary` for test parity. 100% line + func coverage in `tests/unit/core/resolve-tmux-bin.test.ts`.
...
- **Operator-facing**: `ATMUX_TMUX_BIN` and the fail-closed vendored path are documented in README §Configuration. The `build:install` extension landed in this batch; activating `/opt/atmux/` remains an explicit operator deployment step because it requires administrator authorization.
