...
### 🔁 Changed — cockpit window 1 renamed `_superdriver` → `_sd` (ADR-288 §D1)

**The operator's cockpit REPL window is now `_sd`** — the dotfiles ADR-009 pure-truncation shortform of `superdriver`, matching the `driver` → `d` lane rule and ADR-264's precedent of a shortform in a live tmux name (`atx`). The ADR-135 §D4 in-place rename shim gains two steps — `superdriver → _sd` and `_superdriver → _sd` — so the next `atmux cockpit reconcile` renames a live window once without touching its pane, and warns (instead of renaming) when a legacy spelling and `_sd` coexist. `_superdriver` / `superdriver` stay in the never-prune guard and the reserved operator-window set for one release cycle. `atmux cockpit rotate` gate 1 now reads `<session>:=_sd` (falling back to `<session>:=_superdriver` while the window is still un-renamed; both are tmux exact-match targets, since a prefix-matched `:_sd` resolves to a lone `_sd2` lane — the `_medic` capture / Ctrl-C / kill-window targets use `=` too); gate 4 keeps the `superdriver` key and additionally refuses `sd`, `_sd` and `_superdriver`. Role name, schema discriminator, brief and kb board are unchanged. See [ADR-288](docs/adr/288-superdriver-lane-shortform-and-multi-lane-cockpit.md).

### 🐛 Fixed — tmux `=` exact-match targets no longer bypass the never-send-keys-to-driver guard (ADR-239 §D2)

`extractWindowNameFromTargetString` returned `=driver` for a target written `sess:=driver`, so `DRIVER_PANE_NAME_RE` never matched and `DriverSendKeysViolation` was never raised for any exact-match target. The leading `=` is now stripped before classification (`sess:=driver` and `sess:=driver-2` throw again; covered by unit tests). Surfaced by the ADR-288 review because `atmux cockpit rotate` now addresses cockpit-role windows with `=` targets.

### ✨ Added — `_sd2` / `_sd3` superdriver lanes as declarative operator windows (ADR-288 §D2)

Additional superdriver lanes are `_sdN` (N ≥ 2, no `_sd1`), declared as ADR-279 `windows[]` entries in the operator's cockpit config — no code-reserved role, accepted by `validateOperatorWindowNames` (which rejects the malformed spellings `_sd0` / `_sd1` / zero-padded `_sd01` with a `ConfigError` citing ADR-288 §D2), never pruned, and **placed immediately after `_sd`, before `_medic` / `_superbot`** (ADR-288 §D5 — the medic slot becomes "after the last lane", other operator windows such as `_misc` keep their ADR-279 slot). Reconcile keeps the live order `_sd, _sd2, _sd3, _medic, _misc, <viewers>`; a cockpit still in the older `_sd, _medic, _sd2, …` order is corrected by the park-then-place pass with every window id preserved and without `--yes` — `_medic` now rides that pass, and neither the medic pass nor the dry-run planner treats a lane / operator window / `_superbot` in the medic slot as a kill victim. Each lane's Claude runs with `ATMUX_MEMBER=sdN`, acts on kb as `claude@sdN` on lane `sdN`, and works the same `superdriver` board under an operator-armed standing goal; all interaction is kb rows, dispatch is lease-guarded, lanes are orchestration-only and never edit the shared cwd. `atmux cockpit rotate` cannot target a lane (it is refused as an unknown team right after role classification — before the gates and before any handoff payload is written — exit 70, pane and the last team-driver handoff untouched; ADR-288 §D3). The medic slot, reorder base and dry-run planner now anchor on `_sd ?? _superdriver ?? superdriver`, so a cockpit whose rename shim failed keeps its REPL out of the reorder's move-with-kill path. `templates/briefs/superdriver.md` v6 carries the lane identity check and a §Multi-lane section.

### 🧹 Removed — `/atmux:bruh` and `/atmux:bruhloop` (ADR-288 §D4)
