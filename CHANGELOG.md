...
### 🔁 Changed — cockpit window 1 renamed `_superdriver` → `_sd` (ADR-290 §D1)

**The operator's cockpit REPL window is now `_sd`** — the dotfiles ADR-009 pure-truncation shortform of `superdriver`, matching the `driver` → `d` lane rule and ADR-264's precedent of a shortform in a live tmux name (`atx`). The ADR-135 §D4 in-place rename shim gains two steps — `superdriver → _sd` and `_superdriver → _sd` — so the next `atmux cockpit reconcile` renames a live window once without touching its pane, and warns (instead of renaming) when a legacy spelling and `_sd` coexist. `_superdriver` / `superdriver` stay in the never-prune guard and the reserved operator-window set for one release cycle. `atmux cockpit rotate` gate 1 now reads `<session>:=_sd` (falling back to `<session>:=_superdriver` while the window is still un-renamed; both are tmux exact-match targets, since a prefix-matched `:_sd` resolves to a lone `_sd2` lane — the `_medic` capture / Ctrl-C / kill-window targets use `=` too); gate 4 keeps the `superdriver` key and additionally refuses `sd`, `_sd` and `_superdriver`. Role name, schema discriminator, brief and kb board are unchanged. See [ADR-290](docs/adr/290-superdriver-lane-shortform-and-multi-lane-cockpit.md).

### 🐛 Fixed — tmux `=` exact-match targets no longer bypass the never-send-keys-to-driver guard (ADR-239 §D2)

`extractWindowNameFromTargetString` returned `=driver` for a target written `sess:=driver`, so `DRIVER_PANE_NAME_RE` never matched and `DriverSendKeysViolation` was never raised for any exact-match target. The leading `=` is now stripped before classification (`sess:=driver` and `sess:=driver-2` throw again; covered by unit tests). Surfaced by the ADR-290 review because `atmux cockpit rotate` now addresses cockpit-role windows with `=` targets.

### ✨ Added — `_sd2` / `_sd3` superdriver lanes as declarative operator windows (ADR-290 §D2)

Additional superdriver lanes are `_sdN` (N ≥ 2, no `_sd1`), declared as ADR-279 `windows[]` entries in the operator's cockpit config — no code-reserved role, accepted by `validateOperatorWindowNames` (which rejects the malformed spellings `_sd0` / `_sd1` / zero-padded `_sd01` with a `ConfigError` citing ADR-290 §D2), never pruned, and **placed immediately after `_sd`, before `_medic` / `_superbot`** (ADR-290 §D5 — the medic slot becomes "after the last lane", other operator windows such as `_misc` keep their ADR-279 slot). Reconcile keeps the live order `_sd, _sd2, _sd3, _medic, _misc, <viewers>`; a cockpit still in the older `_sd, _medic, _sd2, …` order is corrected by the park-then-place pass with every window id preserved and without `--yes` — `_medic` no…

### 🔁 Changed — the medic is a live cockpit member again (ADR-291)

**Operator decision (geoyws, 2026-09-07): keep the medic.** [ADR-291](docs/adr/291-medic-reinstated-as-cockpit-member.md) reinstates it and supersedes [ADR-212](docs/adr/212-retire-medic-lead-gated-rotation-simplify-honker-consumer-set.md) **§D1** (retire at cockpit W2), **§D5** (retire last, behind the Honker/orchd substrate that ADR-276 deleted on 2026-08-27, so the gate can never open) and **§D6** (the `medic-config-residue` probe, never implemented and now pointing the wrong way); ADR-212 §D2–§D4 / §D7 stand. No code path changed hands — reconcile has always created `_medic` from a `type: "medic"` roster entry with `enabled: true`, rotate has always rotated it, status has always probed it, and ADR-266 §KEEP had already declined to remove the window; only the docs said otherwise.

- **Identity (§D3).** `buildMedicWindowCommand` and `cockpit rotate`'s `buildClaudeRespawnCommand` now both start with `export ATMUX_MEMBER=medic && ` (via `src/core/tui-cmd.ts::envPrefix`, the seam every team pane already uses), so a reconcile-created and a rotated medic pane carry the same lane identity — kb actor `claude@medic`, `atmux claim` / `done` without `--as`. No `cd` and no trailing `exec zsh -i` (an interactive `❯` prompt would defeat rotate's `claudeUiGoneVerifier`).
- **Placement.** `_medic` sits after `_sd` and every `_sdN` lane (ADR-290 §D5) — unchanged mechanics, corrected documentation. ADR-290's two "the slot is normally empty" sentences are fixed and it carries a dated amendment.
- **Operating model (§D2).** The medic is a cockpit lane like `sd` / `sdN`: fleet + host health on the `medic` kb board on `@@hax` under an operator-armed standing goal, kb rows as the only interaction surface, no send-keys (`r-1376df29`), the DOING delegated to isolated worktrees, shared cwd read-only. The ADR-077 §D3 / `docs/medic.md` hard limits are unchanged. New brief at `templates/briefs/medic.md`.
- **Docs + briefs.** `docs/medic.md` gets a 2026-09-07 banner (its hourly-tick, `/loop /whip`, `__superdoctor__` and P0-send-keys sections are now marked ADR-077 substrate / superseded mechanics), and PRD / ARCHITECTURE / RUNBOOK-cockpit / help / `src/verbs/README.md` / the twelve role briefs / the bundled `cockpit-rebuild`, `sweep`, `team`, `bau`, `heads-up`, `session`, `whip` skills no longer state the role is retired. Medic moves from each brief's **Retired roles** clause to its **Cockpit-tier roles** list.
- **Still out of scope.** Ombudsman (kb attention `a-cce8a963` stays open for it), sentinel (retired per ADR-211), the `autoStart` send-keys auto-fire (it types the nonexistent `/loop /medic` and conflicts with `r-1376df29` — kept in tree, `autoStart: false` recommended, kb follow-up filed), and the medic window's cwd (kb `t-cce65c72`).

### 🧹 Removed — the last two `superdoctor` builder shims (ADR-291 §D3, closing ADR-266 §D2)

`buildSuperdoctorWindowCommand` (deprecated alias of `buildMedicWindowCommand`) and the `buildSuperdoctorCommand` field on `ResolveTeamWindowDeps` — with its `??` fallback at the reconcile call site — are deleted. ADR-266 §D2 expired the ADR-133 superdoctor→medic shim window for every other alias; these two were missed. Callers and test fixtures use `buildMedicWindowCommand` / `buildMedicCommand`. Storage-layer identifiers (`superdoctor_attempts`, `SuperdoctorAttemptsRepo`, `__superdoctor__`, the `superdoctor-self-heal-escalation` dedup key) are untouched — they need a schema-migration ADR per ADR-133 §Out of scope.

### 🧹 Removed — `/atmux:bruh` and `/atmux:bruhloop` (ADR-290 §D4)

Both skills are retired and **deleted** from `plugins/atmux/skills/`; `plugin.json` and the wizard's `SKILLS_TABLE` now enumerate **11** skills. They are superseded by the kb-row operating model (a standing goal per superdriver lane plus the `/atmux:sweep` and `/atmux:whip` cadences) and the operator confirmed they are no longer needed; `bruhloop` was pure sugar over `bruh`. The §0.7 eternal-improvement heuristics are inlined into `/atmux:sweep` and `/atmux:whip`; ADR-135 / ADR-167 / ADR-217 / ADR-279 carry dated amendment notes and their older body text is historical.
