...
- **No lane rotation.** `atmux cockpit rotate` cannot target a lane (ADR-290 §D3) and unconditionally refuses `superdriver` / `sd`. A lane is restarted by the operator by hand and its goal re-armed.
