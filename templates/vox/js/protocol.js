// atmux vox — browser mirror of the wire codec (ADR-272 D5).
//
// This file mirrors src/core/vox/frame.ts (binary frame codec) and the
// VOICE_CLOSE table of src/schema/voice.ts. The two numeric tables below
// are REGEX-EXTRACTED by tests/unit/core/vox/asset-protocol-drift.test.ts
...
export function encodeFrame({ flags, seq, payload }) {
  const out = new Uint8Array(VOX_PROTOCOL.HEADER_BYTES + payload.length);
  const view = new DataView(out.buffer);
  view.setUint8(0, VOX_PROTOCOL.MAGIC_PCM16_V1);
  view.setUint8(1, flags & 0xff);
  view.setUint16(2, seq & 0xffff, true); // uint16 seq, LITTLE-endian, offset 2
  out.set(payload, VOX_PROTOCOL.HEADER_BYTES);
  return out;
}
...
export function decodeFrame(buf) {
  if (buf.length < VOX_PROTOCOL.HEADER_BYTES) return { ok: false, reason: "short" };
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  if (view.getUint8(0) !== VOX_PROTOCOL.MAGIC_PCM16_V1) return { ok: false, reason: "bad-magic" };
  if ((buf.length - VOX_PROTOCOL.HEADER_BYTES) % 2 !== 0)
  ...
  return {
    ...
    turnEnd: (flags & VOX_PROTOCOL.FLAG_TURN_END) !== 0,
    synthetic: (flags & VOX_PROTOCOL.FLAG_SYNTHETIC) !== 0,
    payload: buf.subarray(VOX_PROTOCOL.HEADER_BYTES),
  };
}
