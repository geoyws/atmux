// atmux voice — WebAudio capture + playback engine (ADR-272 D5).
//
...
import { VOICE_PROTOCOL } from "./protocol.js";
...
export function createVoiceAudio({ onFrame, onRms }) {
  ...
  async function init() {
    if (ctx) return;
    ctx = new AudioContext({ sampleRate: VOICE_PROTOCOL.SAMPLE_RATE });
  ...
  function enqueuePcm(int16) {
    ...
    const buf = ctx.createBuffer(1, int16.length, VOICE_PROTOCOL.SAMPLE_RATE);
