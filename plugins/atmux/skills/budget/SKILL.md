...
After the verbatim table/JSON output, prepend a one-line operator-attention summary so the operator can scan-skim without re-reading the table. Same scheme as `/atmux:bau`, `/atmux:bruh`, `/atmux:session`, `/atmux:team`, `/atmux:tell-lead`.
