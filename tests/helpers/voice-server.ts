...
//
// Port note: `resolveVoiceConfig` deliberately refuses a non-positive
...
import type { VoiceRunnerKey } from "../../src/core/voice/tool-catalog.ts";
import type { VerbFn } from "../../src/core/verb-capture.ts";
import {
  buildVoiceDeps,
  startVoiceServer,
  type VoiceServeDeps,
  type VoiceServerHandle,
...
export const TEST_TEAM_INDEX: VoiceTeamIndex = {
  teams: [
...
export interface VoiceServerCtx {
  deps: VoiceServeDeps;
  handle: VoiceServerHandle;
 ...
 * — even if `body` throws.
 */
export async function withVoiceServer(
