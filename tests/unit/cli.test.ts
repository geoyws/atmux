...
describe("cli.main — version verb (3-form parity with bash)", () => {
  test("'version' → exit 0, prints 'atmux 0.4.0'", async () => {
    const { exit, stdout, stderr } = await captureMain(["version"]);
    expect(exit).toBe(0);
    expect(stdout).toBe("atmux 0.4.0\n");
    expect(stderr).toBe("");
  });

  test("'--version' alias → exit 0, prints 'atmux 0.4.0' (bash parity)", async () => {
    const { exit, stdout, stderr } = await captureMain(["--version"]);
    expect(exit).toBe(0);
    expect(stdout).toBe("atmux 0.4.0\n");
    expect(stderr).toBe("");
  ...
  test("'-V' alias → exit 0, prints 'atmux 0.4.0' (bash parity)", async () => {
...
describe("cli — bin/atmux-bun entrypoint integration", () => {
  ...
  test("`bin/atmux-bun version` exits 0 + prints 'atmux 0.4.0'", async () => {
