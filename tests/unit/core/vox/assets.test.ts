// Unit tests for src/core/vox/assets.ts — ADR-272 PWA asset routing.
//
// Pins:
...
//   - Default assets dir composes `resolveTemplatesDir() + "vox"`;
...
describe("route table", () => {
  test.each([
    ...
    expect(resolveVoxAsset(pathname, { assetsDir: DIR })).toEqual({
  ...
  test("the map is exactly these 11 routes", () => {
    expect(Object.keys(VOX_ROUTES).length).toBe(11);
  });

  test("/ aliases /index.html (same file path)", () => {
    const root = resolveVoxAsset("/", { assetsDir: DIR });
    const index = resolveVoxAsset("/index.html", { assetsDir: DIR });
    expect(root?.filePath).toBe(index?.filePath ?? "");
  });

  test("VOX_ROUTES map and entries are frozen", () => {
    expect(Object.isFrozen(VOX_ROUTES)).toBe(true);
    const entry = VOX_ROUTES["/"];
...
describe("explicit-map-only lookup (zero traversal surface)", () => {
  test.each([
    ...
    expect(resolveVoxAsset(pathname, { assetsDir: DIR })).toBeNull();
  });
});
...
describe("assets dir resolution", () => {
  ...
  test("default dir composes resolveTemplatesDir() + voice", () => {
    const expected = resolve(resolveTemplatesDir(), "vox", "css", "app.css");
    expect(resolveVoxAsset("/css/app.css")?.filePath).toBe(expected);
  });

  test("opts without assetsDir still uses the default dir", () => {
    const expected = resolve(resolveTemplatesDir(), "vox", "index.html");
    expect(resolveVoxAsset("/", {})?.filePath).toBe(expected);
  });

  test("default dir follows the ATMUX_TEMPLATES_DIR override seam", () => {
    process.env.ATMUX_TEMPLATES_DIR = "/opt/custom-templates";
    expect(resolveVoxAsset("/js/app.js")?.filePath).toBe("/opt/custom-templates/vox/js/app.js");
  });

  test("assetsDir override wins over the default", () => {
    process.env.ATMUX_TEMPLATES_DIR = "/opt/custom-templates";
    expect(resolveVoxAsset("/js/app.js", { assetsDir: DIR })?.filePath).toBe(
