// ADR-272 P4 — `src/core/vox/session.ts` per-connection state machine.
//
...
import type { VoxConfig } from "../../../../src/core/vox/config.ts";
import {
  encodeFrame,
  VOX_FLAG_TURN_END,
  VOX_FRAME_MS,
...
import type {
  ...
  ToolBridge,
  ToolBridgeHealth,
} from "../../../../src/core/vox/tool-bridge.ts";
import { VOX_TOOL_CATALOG } from "../../../../src/core/vox/tool-catalog.ts";
import type { VoxTranscriptEvent, VoxTranscriptSink } from "../../../../src/core/vox/transcript.ts";
...
class FakeTimers implements VoxTimers {
  now = 0;
  private seq = 0;
...
function baseConfig(over: Partial<VoxConfig> = {}): VoxConfig {
  return {
    token: TOKEN,
...
interface Harness {
  ...
  config: VoxConfig;
  bridge: ToolBridge;
  session: ReturnType<typeof createVoxSession>;
  deps: VoxSessionDeps;
...
function makeHarness(
  over: {
    ...
    openTranscript?: (sessionId: string) => VoxTranscriptSink;
  } = {},
): Harness {
  ...
  const shared =
    over.shared ?? createVoxSharedState({ clock: () => timers.now, graceMs: config.resumeGraceMs });
...
function audioFrame(bytes: number, seq = 0, turnEnd = false): Uint8Array {
  return encodeFrame({
    flags: turnEnd ? VOX_FLAG_TURN_END : 0,
    seq,
...
describe("provider connect options", () => {
  ...
  test("tools are the flat-schema catalog, assignable to the provider seam", async () => {
    ...
    expect(tools).toHaveLength(VOX_TOOL_CATALOG.length);
...
describe("uplink", () => {
  ...
  test("an empty TURN_END frame ends the turn without sending audio", async () => {
    ...
    await h.session.handlePhoneMessage(
      encodeFrame({ flags: VOX_FLAG_TURN_END, seq: 0, payload: new Uint8Array(0) }),
    );
  ...
  test("a malformed binary frame is counted and IGNORED — never closes", async () => {
    ...
    await h.session.handlePhoneMessage(new Uint8Array([VOX_MAGIC_PCM16_V1])); // short
    await h.session.handlePhoneMessage(new Uint8Array([VOX_MAGIC_PCM16_V1, 0, 0, 0, 1])); // odd
...
// tests/unit/core/vox/transcript.test.ts and the end-to-end verbs test.)
...
describe("transcript recording", () => {
  interface Recorder {
    ...
    readonly recorded: Array<VoxTranscriptEvent & { session: string }>;
  }

  function recorder(): Recorder {
    const opened: string[] = [];
    const recorded: Array<VoxTranscriptEvent & { session: string }> = [];
    return {
      opened,
      recorded,
      openTranscript: (sessionId: string): VoxTranscriptSink => {
  ...
  test("a RESUMED session keeps recording under the same session id", async () => {
    const rec = recorder();
    const shared = createVoxSharedState({ clock: () => 0, graceMs: 90_000 });
...
describe("downlink", () => {
  test("audio-out is framed SYNTHETIC with an advancing seq and announces speaking once", async () => {
    ...
    expect(a[0]).toBe(VOX_MAGIC_PCM16_V1);
...
describe("tool-call logging", () => {
  ...
  function toolLines(h: Harness): string[] {
    return h.logs.filter((l) => l.startsWith("vox: tool "));
  }
  ...
  test("a successful ungated call logs exactly one line in and one line out", async () => {
    ...
    expect(toolLines(h)).toEqual([
      "vox: tool 'team_status' invoked (no confirmation gate)",
      "vox: tool 'team_status' ok in 412ms",
    ]);
  });
  ...
  test("a failed call names the failure CLASS, not the failure text", async () => {
    ...
    await live(h);
    await callTool(h);
    expect(toolLines(h)[1]).toBe("vox: tool 'team_status' failed in 0ms (unknown_team)");
  ...
  test("a confirm-gated tool logs its gate on the way in, and 'previewed' on the way out", async () => {
    ...
    expect(toolLines(h)).toEqual([
      "vox: tool 'dispatch_task' invoked (confirm-gated)",
      "vox: tool 'dispatch_task' previewed in 0ms — NOT executed, awaiting the operator's confirmation",
    ]);
  ...
  test("redemption is distinguishable from the preview that preceded it", async () => {
    ...
    expect(toolLines(h)[1]).toBe("vox: tool 'dispatch_task' ok in 0ms (confirmation redeemed)");
  });
  ...
  test("a redeemed call that then FAILS carries both the class and the redemption", async () => {
    ...
    expect(toolLines(h)[1]).toBe(
      "vox: tool 'dispatch_task' failed in 0ms (verb_failed, confirmation redeemed)",
    );
  });
  ...
  test("an EXPIRED token is not reported as a redemption", async () => {
    ...
    expect(toolLines(h)[1]).toBe("vox: tool 'dispatch_task' failed in 0ms (confirm_expired)");
  });
  ...
  test("a gated tool rejected BEFORE the gate claims neither stage", async () => {
    ...
    expect(toolLines(h)[1]).toBe("vox: tool 'dispatch_task' failed in 0ms (bad_args)");
  });
  ...
  test("a tool the catalog does not hold is called out as such, not as ungated", async () => {
    ...
    // Under ATMUX_VOX_READONLY the messaging tools are ABSENT from the
    ...
    expect(toolLines(h)[0]).toBe("vox: tool 'kill_team' invoked (not in catalog)");
  });
  ...
  test("an unparseable envelope logs a class rather than logging nothing", async () => {
    ...
    await live(h);
    await callTool(h);
    expect(toolLines(h)[1]).toBe("vox: tool 'team_status' failed in 0ms (unparseable-envelope)");
  });
  ...
  test("an envelope that parses but names no error still logs a class", async () => {
    ...
    await live(h);
    await callTool(h);
    expect(toolLines(h)[1]).toBe("vox: tool 'team_status' failed in 0ms (unspecified)");
  });
  ...
  test("ARGUMENTS never reach the log — and the line is still diagnostic", async () => {
    ...
    expect(toolLines(h)).toEqual([
      "vox: tool 'tell_lead' invoked (no confirmation gate)",
      "vox: tool 'tell_lead' ok in 0ms",
    ]);
  ...
  test("an invented tool NAME cannot write an unbounded log line", async () => {
    ...
    expect(toolLines(h)[0]).toBe(
      `vox: tool '${absurd.slice(0, TOOL_NAME_LOG_MAX_CHARS)}' invoked (not in catalog)`,
    );
  ...
  test("a garbled failure CLASS cannot write an unbounded log line", async () => {
    ...
    await callTool(h);
    expect(toolLines(h)[1]).toBe(
      `vox: tool 'team_status' failed in 0ms (${SPOKEN.repeat(10).slice(0, TOOL_ERROR_CLASS_LOG_MAX_CHARS)})`,
    );
  ...
  test("the stale-result drop line shares the same name bound", async () => {
    ...
    await flush();
    const drop = h.logs.find((l) => l.startsWith("vox: dropping tool result"));
    // One helper caps every name that reaches a log line — the fix is at
    // the shared rendering point, not per-callsite.
    expect(drop).toBe(
      `vox: dropping tool result for '${absurd.slice(0, TOOL_NAME_LOG_MAX_CHARS)}' — the provider leg that issued it is gone (redial during execution)`,
    );
  });
...
describe("dial diagnostics", () => {
  ...
  test("the happy path is ONE line, and it names provider, model and attempt", async () => {
    ...
    await live(h);
    expect(h.logs).toEqual([
      `vox: provider ready — openai-realtime/gpt-realtime attempt 1/${REDIAL_MAX_ATTEMPTS} in 0ms`,
    ]);
  });
  ...
  test("the provider-error event is itself logged with its code, before the attempt fails", async () => {
    ...
    expect(errLine).toBe(
      "vox: provider error (openai-realtime/gpt-realtime) [beta_api_shape_disabled] The Realtime Beta API is no longer supported. Please use /v1/realtime for the GA API.",
    );
  ...
  test("a mid-session provider close logs the redial WITH its close code", async () => {
    ...
    expect(redial).toBe(
      "vox: provider closed mid-session (openai-realtime/gpt-realtime) code=1006 reason=network — redialing",
    );
  });
  ...
  test("a close with no reason renders the code alone, not a dangling 'reason='", async () => {
    ...
    expect(h.logs.find((l) => l.includes("closed mid-session"))).toBe(
      "vox: provider closed mid-session (openai-realtime/gpt-realtime) code=1006 — redialing",
    );
  });
  ...
  test("a codeless provider error logs the message alone, with no empty bracket", async () => {
    ...
    h.provider.lastLeg.emit({ type: "provider-error", message: "rate limited", fatal: false });
    await flush();
    expect(h.logs[0]).toBe("vox: provider error (openai-realtime/gpt-realtime) rate limited");
  });
  ...
  test("an oversized provider message is truncated before it reaches the log", async () => {
    ...
    await live(h);
    h.logs.length = 0;
    // Filler is "y", NOT "x": the log prefix is `vox: ` (ADR-274), so an
    // "x" filler makes `/x+/` match the prefix's own x and measure 1.
    const huge = "y".repeat(PROVIDER_ERROR_LOG_MAX_CHARS * 3);
    h.provider.lastLeg.emit({ type: "provider-error", message: huge, fatal: false });
    await flush();
    const ys = /y+/.exec(h.logs[0] as string)?.[0] ?? "";
    expect(ys).toHaveLength(PROVIDER_ERROR_LOG_MAX_CHARS);
  });
...
describe("park, resume and takeover", () => {
  test("suspend parks the leg and closes 1000; resume within grace reattaches", async () => {
    const timers = new FakeTimers();
    const shared = createVoxSharedState({ clock: () => timers.now, graceMs: 90_000 });
  ...
  test("an abnormal phone drop parks; a clean 1000 close releases", async () => {
    ...
    const clean = makeHarness({
      timers,
      shared: createVoxSharedState({ clock: () => timers.now, graceMs: 90_000 }),
    });
    await live(clean);
