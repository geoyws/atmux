...
import {
  ...
  TranscriptAssembler,
} from "../../../../../src/core/vox/e2e/drive.ts";
import {
  decodeFrame,
  VOX_BYTES_PER_FRAME,
  VOX_FLAG_TURN_END,
...
const PCM = new Uint8Array(VOX_BYTES_PER_FRAME * 2);
...
describe("uplink framing and pacing", () => {
  test("streams the PCM as wire frames with TURN_END on the last", async () => {
    ...
    if (first.ok) expect(first.flags & VOX_FLAG_TURN_END).toBe(0);
  });

  test("paces one frame per VOX_FRAME_MS — real time, not as fast as possible", async () => {
    ...
    expect(slept.filter((ms) => ms === VOX_FRAME_MS).length).toBeGreaterThanOrEqual(2);
    expect(new Set(slept)).toEqual(new Set([VOX_FRAME_MS]));
  });
  ...
  test("one second of speech is 25 frames at the wire rate", () => {
    ...
    expect(VOX_SAMPLE_RATE * 2 * 1) // one second of PCM16
      .toBe(VOX_BYTES_PER_FRAME * 25);
  });
});
...
describe("collection", () => {
  ...
  test("counts downlink audio payload bytes", async () => {
    const { encodeFrame } = await import("../../../../../src/core/vox/frame.ts");
