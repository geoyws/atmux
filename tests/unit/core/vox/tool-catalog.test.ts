// Unit tests for src/core/vox/tool-catalog.ts — ADR-272 D6 frozen v1
// tool surface.
//
// Pins:
...
import { z } from "zod";
import { NUDGE_ACTIONS } from "../../../../src/core/vox/nudge.ts";
import {
  ARGV_PROBE,
  type ArgvSlot,
  auditArgvSlots,
  ...
  type VoxRunnerKey,
  type VoxToolEntry,
} from "../../../../src/core/vox/tool-catalog.ts";
...
function entry(name: string): VoxToolEntry {
...
describe("catalog surface (ADR-272 D6, frozen)", () => {
  ...
  test("mutating on exactly the 4 messaging tools + pane_nudge", () => {
    const mutating = VOX_TOOL_CATALOG.filter((t) => t.mutating).map((t) => t.name);
  ...
  test("confirm on exactly dispatch_task + claim_task + pane_nudge", () => {
    const confirm = VOX_TOOL_CATALOG.filter((t) => t.confirm).map((t) => t.name);
    expect(confirm).toEqual(["dispatch_task", "claim_task", "pane_nudge"]);
  });

  test("every confirm tool is also mutating", () => {
    for (const t of VOX_TOOL_CATALOG.filter((t) => t.confirm)) {
      expect(t.mutating).toBe(true);
    }
  });

  test("runnerKey is null ONLY for list_teams", () => {
    const runnerless = VOX_TOOL_CATALOG.filter((t) => t.runnerKey === null).map((t) => t.name);
    expect(runnerless).toEqual(["list_teams"]);
  });

  test("runner keys map per the header module-map", () => {
    const map = Object.fromEntries(VOX_TOOL_CATALOG.map((t) => [t.name, t.runnerKey]));
    expect(map).toEqual({
      list_teams: null,
      fleet_overview: "topo",
...
describe("pane_nudge (ADR-273 D4) — the bound that lets it ship without OQ-1", () => {
  ...
  test("no OTHER catalog entry declares a preview — the hook stayed narrow", () => {
    const withPreview = VOX_TOOL_CATALOG.filter((t) => t.preview !== undefined).map((t) => t.name);
...
describe("flat-schema guard — EVERY entry derives a flat provider schema", () => {
  ...
  test.each(VOX_TOOL_CATALOG.map((t) => [t.name, t] as const))("%s derives flat", (_name, t) => {
 ...
 */
const RUNNER_PARSERS: Record<VoxRunnerKey, (argv: ReadonlyArray<string>) => unknown> = {
...
function stringArgNames(t: VoxToolEntry): string[] {
...
describe("structural argv-slot gate (ADR-272 D2 §Supplement)", () => {
  test("the sample table covers EXACTLY the catalog — a new tool fails here first", () => {
    expect(Object.keys(SAMPLES).sort()).toEqual(VOX_TOOL_CATALOG.map((t) => t.name).sort());
    expect(Object.keys(EXPECTED_SLOTS).sort()).toEqual(VOX_TOOL_CATALOG.map((t) => t.name).sort());
  });
  ...
  test.each(
    VOX_TOOL_CATALOG.map((t) => [t.name, t] as const),
  ...
  test("a `--` terminator is emitted ONLY for runners whose real parser honours it", () => {
    for (const t of VOX_TOOL_CATALOG) {
      ...
      expect(
        TERMINATOR_HONOURING_RUNNERS.has(t.runnerKey as VoxRunnerKey),
  ...
  test("TERMINATOR_HONOURING_RUNNERS is TRUE of the real parsers, not just asserted", () => {
    ...
    const probeArgv: Record<VoxRunnerKey, string[]> = {
      topo: ["--", "-x"],
      fleet: ["--", "-x"],
      status: ["--", "-x"],
      health: ["--", "-x"],
    ...
    for (const t of VOX_TOOL_CATALOG) {
      ...
      for (const [key, slot] of Object.entries(slots)) {
        ...
        for (const hostile of ["--no-ping", "--team-dir"]) {
          ...
          const parsed = RUNNER_PARSERS[t.runnerKey as VoxRunnerKey](argv);
          expect(
