...
// (templates/voice/js/protocol.js) is vanilla ESM outside the TS universe,
...
//
// Also pins route↔file integrity: every VOICE_ROUTES entry must resolve
// to a real file under templates/voice/ (catches a route or file rename
...
const ASSETS_DIR = resolve(import.meta.dir, "../../../..", "templates", "voice");
...
describe("protocol.js VOICE_PROTOCOL mirrors src/core/voice/frame.ts", () => {
  test("all eight constants match exactly", () => {
    expect(extractNumericTable(PROTOCOL_JS, "VOICE_PROTOCOL")).toEqual({
      MAGIC_PCM16_V1: VOICE_MAGIC_PCM16_V1,
      FLAG_TURN_END: VOICE_FLAG_TURN_END,
      FLAG_SYNTHETIC: VOICE_FLAG_SYNTHETIC,
      HEADER_BYTES: VOICE_HEADER_BYTES,
      SAMPLE_RATE: VOICE_SAMPLE_RATE,
      FRAME_MS: VOICE_FRAME_MS,
      SAMPLES_PER_FRAME: VOICE_SAMPLES_PER_FRAME,
      BYTES_PER_FRAME: VOICE_BYTES_PER_FRAME,
    });
  });
});
...
describe("every VOICE_ROUTES entry has a file on disk in templates/voice/", () => {
  const routes = Object.keys(VOICE_ROUTES);

  test.each(routes)("%s resolves to an existing file", (pathname) => {
    const resolved = resolveVoiceAsset(pathname, { assetsDir: ASSETS_DIR });
