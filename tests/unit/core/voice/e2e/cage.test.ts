...
import {
  ...
  plannedPaths,
  shellQuote,
} from "../../../../../src/core/voice/e2e/cage.ts";
import { TEAM_FIXTURES } from "../../../../../src/core/voice/e2e/fixtures.ts";
import { isUnder } from "../../../../../src/core/voice/e2e/isolation.ts";

const TEMP = "/tmp/atmux-voice-e2e-plan";
const UID = 1000;
...
describe("destroyCage", () => {
  ...
  test("a tmux failure during teardown is logged, not thrown", async () => {
    ...
    expect(lines.join("\n")).toContain("teardown of atmux-voice-e2e-alpha failed");
  });
