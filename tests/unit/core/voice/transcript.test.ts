// Unit tests for src/core/voice/transcript.ts — ADR-272 OQ-4 (resolved
...
//
// Pins:
//   - Location is `$HOME/.atmux/voice-logs` and has no env override.
...
async function scratch(): Promise<string> {
  return await mkdtemp(join(tmpdir(), "atmux-voice-transcript-"));
}
...
describe("resolveTranscriptDir", () => {
  test("resolves $HOME/.atmux/voice-logs — atmux's own state dir (ADR-268)", () => {
    expect(resolveTranscriptDir({ env: { HOME: "/home/op" } })).toBe(
      `/home/op/${VOICE_TRANSCRIPT_DIR_REL}`,
    );
    expect(VOICE_TRANSCRIPT_DIR_REL).toBe(".atmux/voice-logs");
  });
  ...
  test.each([
    ...
    expect(resolveTranscriptDir({ env })).toBe(join(homedir(), VOICE_TRANSCRIPT_DIR_REL));
  });
  ...
  test("reads process.env when no env is injected", () => {
    ...
    expect(resolveTranscriptDir()).toBe(
      join(process.env.HOME ?? homedir(), VOICE_TRANSCRIPT_DIR_REL),
    );
  });
  ...
  test("there is NO directory override — the only knobs are boolean + retention", async () => {
    ...
    const noisy = {
      HOME: "/home/op",
      ATMUX_VOICE_TRANSCRIPT_DIR: "/repo/product/logs",
      ATMUX_VOICE_LOG_DIR: "/repo/product/logs",
    };
    expect(resolveTranscriptDir({ env: noisy })).toBe(`/home/op/${VOICE_TRANSCRIPT_DIR_REL}`);
    // Resolved from this file, not from the cwd — the suite must not
    // depend on being launched from the repo root.
    const source = await readFile(
      resolve(import.meta.dir, "../../../../src/core/voice/transcript.ts"),
      "utf8",
    );
    expect(source).not.toContain("ATMUX_VOICE_TRANSCRIPT_DIR");
  });
});

describe("transcriptFileName", () => {
  test("carries the session id, prefixed and suffixed", () => {
    expect(transcriptFileName("01920af7-c0de")).toBe("voice-01920af7-c0de.jsonl");
  });

  test.each([
    ["../../etc/passwd", "voice-______etc_passwd.jsonl"],
    ["a/b", "voice-a_b.jsonl"],
    ["with space", "voice-with_space.jsonl"],
    ["dots.and.dots", "voice-dots_and_dots.jsonl"],
  ...
  test("an id that sanitizes to nothing still yields a prunable name", () => {
    expect(transcriptFileName("")).toBe("voice-session.jsonl");
  ...
  test("a pathological id is capped at 64 characters and stays prunable", () => {
    ...
    expect(name).toBe(`voice-${"z".repeat(64)}.jsonl`);
    expect(TRANSCRIPT_FILE_RE.test(name)).toBe(true);
  });
});
...
describe("createTranscriptSink", () => {
  test("writes ONE file per session with the id in its name, and the exact line content", async () => {
    ...
    try {
      ...
      expect(sink.path).toBe(join(dir, "voice-sess-abc.jsonl"));
      ...
      expect(await readdir(dir)).toEqual(["voice-sess-abc.jsonl"]);
    } finally {
  ...
  test("creates the file LAZILY — a session where nobody spoke leaves nothing behind", async () => {
    const dir = await scratch();
    try {
      const nested = join(dir, "voice-logs");
      const sink = createTranscriptSink({ sessionId: "silent", dir: nested });
      expect(sink.path).toBe(join(nested, "voice-silent.jsonl"));
...
describe("pruneTranscripts — the 7-day boundary", () => {
  test("keeps a file EXACTLY at the retention edge and removes one past it", async () => {
    ...
    try {
      const atEdge = await agedFile(dir, "voice-edge.jsonl", now, RETENTION_MS);
      const pastEdge = await agedFile(dir, "voice-past.jsonl", now, RETENTION_MS + 1000);
  ...
  test("a 7-day-old file survives and an 8-day-old file is deleted", async () => {
    ...
    const now = Date.parse("2026-08-15T00:00:00.000Z");
    try {
      const sevenDays = await agedFile(dir, "voice-seven.jsonl", now, 7 * DAY_MS);
      const eightDays = await agedFile(dir, "voice-eight.jsonl", now, 8 * DAY_MS);
      const fresh = await agedFile(dir, "voice-fresh.jsonl", now, 60_000);
      const result = await pruneTranscripts({ dir, retentionMs: RETENTION_MS, now: () => now });
      expect(result).toMatchObject({ removed: 1, kept: 2, errors: 0 });
      expect(await readdir(dir)).toEqual(
        expect.arrayContaining(["voice-seven.jsonl", "voice-fresh.jsonl"]),
      );
      expect(await readdir(dir)).not.toContain("voice-eight.jsonl");
  ...
  test("the window follows the CONFIGURED retention, not a hardcoded 7 days", async () => {
    ...
    const now = Date.parse("2026-08-15T00:00:00.000Z");
    try {
      await agedFile(dir, "voice-two-days.jsonl", now, 2 * DAY_MS);
...
describe("pruneTranscripts — deletes ONLY what it owns", () => {
  test("ancient non-transcript entries in the same directory are untouched", async () => {
    ...
    try {
      ...
      await agedFile(dir, "voice-mine.jsonl", now, ancient);
      await agedFile(dir, "notes.txt", now, ancient);
      await agedFile(dir, "voice-abc.jsonl.bak", now, ancient);
      await agedFile(dir, "voice-.jsonl", now, ancient); // no id → not ours
      await agedFile(dir, "voice-abc.json", now, ancient);
      await agedFile(dir, "voice-bad name.jsonl", now, ancient);
      const subdir = join(dir, "voice-subdir.jsonl"); // a DIRECTORY named like one
      await mkdir(subdir);

      const result = await pruneTranscripts({ dir, retentionMs: RETENTION_MS, now: () => now });
      expect(result).toMatchObject({ removed: 1, kept: 0, skipped: 6, errors: 0 });
...
describe("pruneTranscripts — never takes the server down", () => {
  test("a missing directory is normal, not an error", async () => {
    const result = await pruneTranscripts({
      dir: join(tmpdir(), "atmux-voice-does-not-exist-ever"),
  ...
  test("one undeletable file is counted and the sweep continues to the next", async () => {
    ...
    const fs: TranscriptFs = {
      list: async () => [
        { name: "voice-a.jsonl", isFile: true },
        { name: "voice-b.jsonl", isFile: true },
      ],
      mtimeMs: async () => 0,
      remove: async (path) => {
        if (path.endsWith("voice-a.jsonl")) throw new Error("EPERM");
        removed.push(path);
      },
    };
    const result = await pruneTranscripts({
    ...
    expect(removed).toEqual(["/x/voice-b.jsonl"]);
  });

  test("a file that vanishes mid-sweep is neither removed nor an error", async () => {
    const fs: TranscriptFs = {
      list: async () => [{ name: "voice-gone.jsonl", isFile: true }],
      mtimeMs: async () => null,
      remove: async () => {
...
describe("nodeTranscriptFs", () => {
  ...
  test("mtimeMs answers null for a missing file and rethrows anything else", async () => {
    expect(await nodeTranscriptFs.mtimeMs(join(tmpdir(), "atmux-voice-absent-file"))).toBeNull();
    const dir = await scratch();
    try {
...
describe("formatPruneResult + retentionMsForDays", () => {
  test("renders every counter and the directory", () => {
    expect(
      formatPruneResult("/home/op/.atmux/voice-logs", {
        removed: 2,
        kept: 5,
        skipped: 1,
        errors: 0,
      ...
      "voice: transcript prune /home/op/.atmux/voice-logs — removed 2, kept 5, skipped 1, errors 0",
    );
  });
