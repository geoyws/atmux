// Unit tests for src/core/voice/nudge.ts — ADR-273 D4/D5.
//
...
import type { PaneObservation, PaneVerdict } from "../../../../src/core/voice/fleet.ts";
import {
  classifyAfterNudge,
  describeVerdict,
  isNudgeAction,
...
describe("classifyAfterNudge (ADR-273 D5 — the survey classifier is NOT reused verbatim)", () => {
  ...
  test("...and the bare survey classifier really does disagree (the bug is real, not hypothetical)", async () => {
    const { classifyPaneObservation } = await import("../../../../src/core/voice/fleet.ts");
