// ADR-272 P4 — `src/core/voice/probe.ts` (the headless probe client).
//
...
import {
  ...
  toneFrames,
  tryReadServerType,
} from "../../../../src/core/voice/probe.ts";
import { UsageError } from "../../../../src/errors.ts";
import { withVoiceServer } from "../../../helpers/voice-server.ts";
...
describe("synthesizeTone", () => {
  test("produces the right sample count for the duration", () => {
    ...
    expect(pcm.length).toBe(VOICE_SAMPLE_RATE * 2); // 24000 samples × 2 bytes
    const half = synthesizeTone({ hz: 440, ms: 500 });
    expect(half.length).toBe(VOICE_SAMPLE_RATE);
  });
...
describe("toneFrames", () => {
  test("slices into wire frames with TURN_END on the last one only", () => {
    ...
    expect(frames).toHaveLength(
      Math.ceil((VOICE_SAMPLE_RATE * PROBE_TONE_MS) / 1000 / (VOICE_BYTES_PER_FRAME / 2)),
    );
    frames.forEach((f, i) => {
      const d = decodeFrame(f);
      expect(d.ok).toBe(true);
      if (!d.ok) return;
      expect(f[0]).toBe(VOICE_MAGIC_PCM16_V1);
      expect(d.seq).toBe(i);
      expect(d.turnEnd).toBe(i === frames.length - 1);
    });
    // Every full frame is exactly the nominal payload size.
    expect((frames[0] as Uint8Array).length - 4).toBe(VOICE_BYTES_PER_FRAME);
  });

  test("keeps a trailing partial frame and still marks TURN_END", () => {
    const pcm = new Uint8Array(VOICE_BYTES_PER_FRAME + 10);
    const frames = toneFrames(pcm);
  ...
  test("empty PCM still emits one bare TURN_END frame", () => {
    ...
    if (d.ok) {
      ...
      expect(d.flags & VOICE_FLAG_TURN_END).toBe(VOICE_FLAG_TURN_END);
    }
  });

  test("sequence numbers start where asked and wrap at uint16", () => {
    const frames = toneFrames(new Uint8Array(VOICE_BYTES_PER_FRAME * 3), 65534);
  ...
  test("a bad token is refused at the PRE-upgrade gate, never reaching hello", async () => {
    await withVoiceServer({}, async (ctx) => {
      const result = await runProbe({
        ...
        // verbs/voice.test.ts "a bad hello token closes 4401
