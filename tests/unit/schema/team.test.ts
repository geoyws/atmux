...
describe("TeamMartinetOverrides — defaults + bounds", () => {
  ...
  test("escalationConfidenceThreshold rejects out-of-range values", () => {
    expect(() => TeamMartinetOverrides.parse({ escalationConfidenceThreshold: -0.01 })).toThrow();
    expect(() => TeamMartinetOverrides.parse({ escalationConfidenceThreshold: 1.01 })).toThrow();
  });

  test("unknown keys rejected (.strict drift detection)", () => {
    expect(() => TeamMartinetOverrides.parse({ cadenceSecondz: 270 })).toThrow();
  });
