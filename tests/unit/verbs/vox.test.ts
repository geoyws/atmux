// ADR-272 P4 — `src/verbs/vox.ts`.
//
// The HTTP + WebSocket surface is exercised against a REAL `Bun.serve`
// on an ephemeral port with a fake provider (`tests/helpers/vox-server.ts`)
...
import { VOX_ROUTES } from "../../../src/core/vox/assets.ts";
import { encodeFrame, VOX_FLAG_TURN_END } from "../../../src/core/vox/frame.ts";
import {
  checkModelPin,
  type ModelCheckDeps,
  type ModelCheckResult,
} from "../../../src/core/vox/model-check.ts";
import { WEDGE_THRESHOLD_MULTIPLE } from "../../../src/core/vox/tool-bridge.ts";
import { VOX_TOOL_CATALOG, type VoxRunnerKey } from "../../../src/core/vox/tool-catalog.ts";
import {
...
import {
  ...
  waitFor,
  withVoxServer,
} from "../../helpers/vox-server.ts";

// ---------- parseVoxArgs ----------

describe("parseVoxArgs", () => {
  test("defaults to --serve", () => {
    expect(parseVoxArgs([])).toEqual({ action: "serve" });
    expect(parseVoxArgs(["--serve"])).toEqual({ action: "serve" });
  });
  ...
  test.each([
    ...
    expect(parseVoxArgs([flag])).toEqual({ action: action as VoxAction });
  });
  ...
  test("a repeated identical action flag is fine", () => {
    expect(parseVoxArgs(["--status", "--status"])).toEqual({ action: "status" });
  });

  test("two DIFFERENT action flags are refused rather than silently last-wins", () => {
    expect(() => parseVoxArgs(["--status", "--stop"])).toThrow(/conflicting actions/);
    expect(() => parseVoxArgs(["--serve", "--supervise"])).toThrow(UsageError);
  });
  ...
  test.each([
    ...
    expect(() => parseVoxArgs(argv as string[])).toThrow(re as RegExp);
  });
  ...
  test.each([
    ...
    expect(() => parseVoxArgs([flag, value])).toThrow(/must be a positive integer/);
  });

  test("an unknown arg is a usage error", () => {
    expect(() => parseVoxArgs(["--nope"])).toThrow(/unknown arg: --nope/);
    expect(() => parseVoxArgs(["bare"])).toThrow(/unknown arg: bare/);
  });

  test("voxArgsToFlags maps only what was supplied", () => {
    expect(voxArgsToFlags({ action: "serve" })).toEqual({});
    expect(
      voxArgsToFlags({
        action: "serve",
        port: 1,
    ...
    expect(voxArgsToFlags({ action: "serve", readonly: false })).toEqual({});
  });
});
...
describe("verb runner map", () => {
  test("covers every VoxRunnerKey the catalog declares", () => {
    const declared = new Set<string>(
      VOX_TOOL_CATALOG.map((e) => e.runnerKey).filter((k): k is VoxRunnerKey => k !== null),
    );
    const wired = new Set<string>(Object.keys(VOX_RUNNER_IMPORTERS));
    expect([...declared].sort()).toEqual([...wired].sort());
    // `list_teams` is the ONE core-direct read (runnerKey null).
    expect(VOX_TOOL_CATALOG.filter((e) => e.runnerKey === null).map((e) => e.name)).toEqual([
      "list_teams",
    ]);
  });

  test("every importer resolves to a real named export", async () => {
    for (const [key, importer] of Object.entries(VOX_RUNNER_IMPORTERS)) {
...
describe("boot wiring", () => {
  ...
  test("buildVoxDeps refuses without a token, BEFORE any provider work", async () => {
    ...
    ).rejects.toThrow(/ATMUX_VOX_TOKEN is required/);
    expect(providerBuilt).toBe(false);
  });

  test("buildVoxDeps refuses a short token", async () => {
    await expect(
      buildVoxDeps({
        env: { ATMUX_VOX_TOKEN: "too-short", OPENAI_API_KEY: "sk-x" },
        loadTeamIndex: async () => ({ teams: [] }),
      }),
    ).rejects.toThrow(/too short/);
  });

  test("buildVoxDeps refuses a missing OPENAI_API_KEY", async () => {
    await expect(
      buildVoxDeps({
        env: { ATMUX_VOX_TOKEN: TEST_VOX_TOKEN },
  ...
  test("buildVoxDeps refuses an unknown provider name", async () => {
    await expect(
      buildVoxDeps({
        env: {
          ATMUX_VOX_TOKEN: TEST_VOX_TOKEN,
          OPENAI_API_KEY: "sk-x",
          ATMUX_VOX_PROVIDER: "not-a-provider",
        },
        loadTeamIndex: async () => ({ teams: [] }),
      }),
  ...
  test("readonly REMOVES the mutating tools from the catalog (absent, not refused)", () => {
    ...
    expect(full).toHaveLength(VOX_TOOL_CATALOG.length);
    expect(ro).toHaveLength(VOX_TOOL_CATALOG.filter((e) => !e.mutating).length);
  ...
  test("pane_nudge is ABSENT under readonly — the input tool is unreachable until P7", () => {
    ...
    // ATMUX_VOX_READONLY=1, so the survey half can ship first. This is
  ...
  test("pane_send is NOT in the catalog — OQ-1 is still an operator decision", () => {
    ...
    expect(VOX_TOOL_CATALOG.some((e) => e.name === "pane_send")).toBe(false);
  });

  test("readonly deps carry the filtered catalog through to the session", async () => {
    const { deps } = await buildTestDeps({ env: { ATMUX_VOX_READONLY: "1" } });
  ...
  test("resolveVoxAssetsDir honours an explicit override and the templates default", () => {
    expect(
      resolveVoxAssetsDir({
        token: TEST_VOX_TOKEN,
...
describe("server helpers", () => {
  test("healthzBody reports provider + readonly, and carries NO secret", async () => {
    ...
    expect(json).not.toContain(TEST_VOX_TOKEN);
  });
  ...
  test("startupBanner names the config but NEVER a secret", async () => {
    ...
    expect(banner).not.toContain(TEST_VOX_TOKEN);
  });
});
...
describe("HTTP surface (real Bun.serve)", () => {
  ...
  test("/ws with a right token but a disallowed Origin is 403 (origin checked FIRST)", async () => {
    await withVoxServer({ env: { ATMUX_VOX_ORIGINS: "https://atmux.geoy.ws" } }, async (ctx) => {
  ...
  test("serves the PWA shell with the pinned mime + cache headers", async () => {
    await withVoxServer({}, async (ctx) => {
      ...
      for (const [pathname, entry] of Object.entries(VOX_ROUTES)) {
  ...
  test("no served asset leaks the token or the api key", async () => {
    await withVoxServer({}, async (ctx) => {
      for (const path of ["/", "/js/app.js", "/js/protocol.js", "/css/app.css", "/healthz"]) {
        ...
        expect(body, `${path} leaked the token`).not.toContain(TEST_VOX_TOKEN);
  ...
  test("a route whose file is missing on disk is a 404, not a 500", async () => {
    await withVoxServer(
      { env: { ATMUX_VOX_ASSETS_DIR: "/nonexistent-assets-dir" } },
      async (ctx) => {
...
describe("WebSocket surface (real Bun.serve)", () => {
  ...
  test("binary audio reaches the provider and TURN_END ends the turn", async () => {
    await withVoxServer({}, async (ctx) => {
      ...
      c.ws.send(encodeFrame({ flags: VOX_FLAG_TURN_END, seq: 1, payload: new Uint8Array(0) }));
  ...
  test("--max-frames bounds the serve and exits 0", async () => {
    ...
    deps.config.port = 0;
    const logs: string[] = [];
    const handle = startVoxServer({ deps, maxFrames: 2, log: (l) => logs.push(l) });
    try {
  ...
  test("--max-frames with NO log seam still stops cleanly (default no-op sink)", async () => {
    const { deps } = await buildTestDeps();
    deps.config.port = 0;
    const handle = startVoxServer({ deps, maxFrames: 1 });
    try {
      const c = openClient(`ws://127.0.0.1:${handle.port}/ws?token=${deps.config.token}`);
      await c.opened;
      c.ws.send(JSON.stringify({ type: "hello", v: 1, token: deps.config.token, mode: "ptt" }));
...
// the only recovery is `atmux vox --stop`. Throughout all of that,
// `/healthz` answered `{"ok":true,...}`.
//
// Vox runs unattended in a detached tmux session, so a health check
...
describe("healthz vs a wedged tool bridge (real Bun.serve, real HTTP)", () => {
  ...
  const FAST_TIMEOUT_ENV = { ATMUX_VOX_TOOL_TIMEOUT_MS: "30" };
  ...
  async function callTool(ctx: VoxServerCtx, c: ReturnType<typeof openClient>, name: string) {
    c.ws.send(JSON.stringify({ type: "hello", v: 1, token: ctx.token, mode: "ptt" }));
    expect(await waitFor(() => framesOf(c.texts, "ready").length > 0)).toBe(true);
  ...
  test("/healthz stays HTTP 200 while wedged — the body carries the verdict", async () => {
    // `atmux vox --status` treats any non-2xx as UNREACHABLE. A
  ...
  test("/healthz never carries a secret, wedged or healthy", async () => {
    await withVoxServer(
      ...
      async (ctx) => {
        ...
        expect(body).not.toContain(TEST_VOX_TOKEN);
        c.ws.close();
      },
    );
  });
...
describe("server diagnostics (real Bun.serve, real stderr sink)", () => {
  test("a provider dial failure reaches the server's log with the provider's error code", async () => {
    ...
    await withVoxServer({ provider }, async (ctx) => {
      const c = openClient(ctx.wsUrl);
      await c.opened;
      c.ws.send(JSON.stringify({ type: "hello", v: 1, token: ctx.token, mode: "ptt" }));
  ...
  // path is production's: session → `deps.log` → `createVoxLogger` →
    ...
    await withVoxServer({ provider }, async (ctx) => {
      ...
      ctx.provider.lastLeg.emit({
        ...
        message:
          ...
          ` (Authorization: Bearer ${TEST_OPENAI_KEY}; session token ${TEST_VOX_TOKEN})`,
        fatal: false,
      });
      ...
      expect(written).not.toContain(TEST_VOX_TOKEN);
...
// operator's own `~/.atmux/vox-logs/` out of the suite entirely.
...
async function scratchHome(): Promise<{ home: string; dir: string; cleanup: () => Promise<void> }> {
  const home = await mkdtemp(join(tmpdir(), "atmux-vox-home-"));
  return {
    home,
    dir: join(home, VOX_TRANSCRIPT_DIR_REL),
...
describe("transcript wiring", () => {
  test("OFF by default — buildVoxDeps hands the session NO sink factory", async () => {
    ...
    expect(deps.transcriptDir).toBe(`/home/op/${VOX_TRANSCRIPT_DIR_REL}`);
    expect(deps.transcriptRetentionMs).toBe(DEFAULT_TRANSCRIPT_RETENTION_DAYS * 86_400_000);
  });

  test("ATMUX_VOX_TRANSCRIPTS=1 builds a per-session factory under ~/.atmux/vox-logs", async () => {
    const { deps } = await buildTestDeps({
      env: { HOME: "/home/op", ATMUX_VOX_TRANSCRIPTS: "1" },
    });
    expect(deps.config.transcripts).toBe(true);
    const sink = deps.openTranscript?.("sess-xyz");
    expect(sink?.path).toBe(`/home/op/${VOX_TRANSCRIPT_DIR_REL}/vox-sess-xyz.jsonl`);
  });

  test("the retention window follows ATMUX_VOX_TRANSCRIPT_RETENTION_DAYS", async () => {
    const { deps } = await buildTestDeps({
      env: { ATMUX_VOX_TRANSCRIPT_RETENTION_DAYS: "2" },
    });
  ...
  test("the banner says whether speech is being recorded", async () => {
    ...
    const on = await buildTestDeps({ env: { ATMUX_VOX_TRANSCRIPTS: "1" } });
    ...
    expect(startupBanner(on.deps, handle)).not.toContain(TEST_VOX_TOKEN);
  });
});

describe("transcripts on disk (end to end)", () => {
  /** Drive one real session that speaks; returns its session id. */
  async function speak(ctx: VoxServerCtx): Promise<string> {
    const c = openClient(ctx.wsUrl);
    await c.opened;
    c.ws.send(JSON.stringify({ type: "hello", v: 1, token: ctx.token, mode: "ptt" }));
    expect(await waitFor(() => framesOf(c.texts, "ready").length > 0)).toBe(true);
  ...
  test("opted IN: the operator's speech lands in ~/.atmux/vox-logs, finals only", async () => {
    const scratch = await scratchHome();
    try {
      let sessionId = "";
      await withVoxServer(
        { env: { HOME: scratch.home, ATMUX_VOX_TRANSCRIPTS: "1" } },
        async (ctx) => {
          sessionId = await speak(ctx);
        },
      );
      const file = join(scratch.dir, `vox-${sessionId}.jsonl`);
      expect(await readdir(scratch.dir)).toEqual([`vox-${sessionId}.jsonl`]);
      ...
      expect(raw).not.toContain(TEST_VOX_TOKEN);
  ...
  test("opted OUT (the shipped default): NOTHING is written — no file, no directory", async () => {
    const scratch = await scratchHome();
    try {
      await withVoxServer({ env: { HOME: scratch.home } }, async (ctx) => {
...
describe("runTranscriptPrune", () => {
  test("reports the sweep on the log sink", async () => {
    ...
    try {
      ...
      expect(logs).toEqual([
        `vox: transcript prune ${scratch.dir} — removed 0, kept 0, skipped 0, errors 0`,
      ]);
    } finally {
      await scratch.cleanup();
    }
...
/**
 * The model-pin drift check, stubbed OFF for every `serveVox` call in
 * this file.
 *
 * `serveVox` runs it at boot, and the real one issues an HTTPS GET to
...
describe("serveVox", () => {
  ...
  test("logs the banner, honours SIGINT-style shutdown, and unregisters handlers", async () => {
    ...
    const code = await p;
    expect(code).toBe(0);
    expect(logs.some((l) => l.startsWith("vox: listening on 127.0.0.1:"))).toBe(true);
    expect(logs).toContain("vox: shutting down");
  ...
  test("a MISSING model prints the loud banner and the server STILL serves", async () => {
    ...
    expect(logs.some((l) => l.startsWith("vox: listening on"))).toBe(true);
...
describe("supervise", () => {
  test("is idempotent — an existing session is reported, never recreated", async () => {
    const spy = tmuxSpy([VOX_TMUX_SESSION]);
    const logs: string[] = [];
    expect(
      await superviseVox({
        tmux: spy.tmux,
  ...
  test("creates a DETACHED session running the crash-loop wrapper", async () => {
    ...
    expect(opts.name).toBe(VOX_TMUX_SESSION);
    expect(opts.detached).toBe(true);
    expect(opts.cwd).toBe("/root/work/src/atmux");
    expect(String(opts.shellCommand)).toContain("vox --serve");
    expect(logs[0]).toContain(VOX_TMUX_SESSION);
  });
  ...
  test("targets the OPERATOR'S DEFAULT tmux socket, not a cage socket", () => {
    ...
    expect(VOX_TMUX_SOCKET).toBe("default");
    expect(VOX_TMUX_SESSION).toBe("atmux-vox");
    expect(VOX_TMUX_SOCKET.startsWith("atmux-")).toBe(false);
  });
  ...
  test("the crash-loop wrapper backs off, breaks the circuit, and stops respawning", () => {
    ...
    expect(script).toContain("'/usr/local/bin/atmux' vox --serve");
...
// ---------------------------------------------------------------------
// ATMUX_VOX_BIN — the override that makes --supervise usable from a
...
describe("resolveSuperviseBin — precedence and fail-closed behaviour", () => {
  ...
  test("ATMUX_VOX_BIN beats the PATH resolution", () => {
    // The live failure this exists for: PATH resolves an INSTALLED
    // release that predates the `vox` verb, so the supervisor's wrapper
    // prints `unknown verb: vox`, exits 64, and crash-loops.
    expect(resolveSuperviseBin({ env: { ATMUX_VOX_BIN: REPO_BIN }, resolve })).toBe(REPO_BIN);
  });
  ...
  test("an EMPTY or whitespace-only value falls through — fails CLOSED", () => {
    // A fat-fingered `export ATMUX_VOX_BIN=` must degrade to the
    // current behaviour, never produce a wrapper that execs `'' vox`.
    for (const bad of ["", "   ", "\t", "\n"]) {
      expect(resolveSuperviseBin({ env: { ATMUX_VOX_BIN: bad }, resolve }), bad).toBe(PATH_BIN);
  ...
  test("an empty OVERRIDE still lets a good env var win — each layer is independent", () => {
    expect(resolveSuperviseBin({ override: "  ", env: { ATMUX_VOX_BIN: REPO_BIN }, resolve })).toBe(
      REPO_BIN,
    );
  });

  test("with no env seam it reads the real process.env", () => {
    const prior = process.env.ATMUX_VOX_BIN;
    process.env.ATMUX_VOX_BIN = "/probe/atmux";
    try {
      expect(resolveSuperviseBin({ resolve })).toBe("/probe/atmux");
    } finally {
      if (prior === undefined) delete process.env.ATMUX_VOX_BIN;
      else process.env.ATMUX_VOX_BIN = prior;
    }
  });
...
describe("vox --supervise honours ATMUX_VOX_BIN end to end", () => {
  test("the crash-loop script re-execs the overridden binary", async () => {
    const spy = tmuxSpy();
    const prior = process.env.ATMUX_VOX_BIN;
    process.env.ATMUX_VOX_BIN = "/root/work/src/atmux/bin/atmux-bun";
    try {
      expect(await vox(["--supervise"], { tmux: spy.tmux, log: () => {} })).toBe(0);
    } finally {
      if (prior === undefined) delete process.env.ATMUX_VOX_BIN;
      else process.env.ATMUX_VOX_BIN = prior;
    }
    const script = String((spy.created[0] as Record<string, unknown>).shellCommand);
    expect(script).toContain("'/root/work/src/atmux/bin/atmux-bun' vox --serve");
  });

  test("an explicit binPath override still wins over the env var", async () => {
    const spy = tmuxSpy();
    const prior = process.env.ATMUX_VOX_BIN;
    process.env.ATMUX_VOX_BIN = "/from/env";
    try {
      await vox(["--supervise"], { tmux: spy.tmux, log: () => {}, binPath: "/from/flag" });
    } finally {
      if (prior === undefined) delete process.env.ATMUX_VOX_BIN;
      else process.env.ATMUX_VOX_BIN = prior;
    }
...
describe("status", () => {
  ...
  const serverBody = (over: Partial<VoxHealthzBody> = {}): string =>
    JSON.stringify({
      ok: true,
      provider: "openai-realtime",
      readonly: true,
  ...
  ): Promise<VoxStatusReport> =>
    await voxStatus({
      tmux: tmuxSpy([VOX_TMUX_SESSION]).tmux,
      config: { ...cfg, ...over },
  ...
  test("probes the right URL and reports the SERVER's fields, not its own config", async () => {
    ...
    expect(formatStatusReport(report)).toContain(
      "vox: server: provider=openai-realtime  readonly=true",
    );
  });

  test("SECURITY: server readonly=false wins over a shell that exports readonly=true", async () => {
    // The dangerous direction. A shell with ATMUX_VOX_READONLY=1
    ...
    expect(lineOf(out, "vox: server: provider=")).toContain("readonly=false");
  ...
  test("server readonly=true wins over a shell that did not export the flag", async () => {
    ...
    expect(lineOf(formatStatusReport(report), "vox: server: provider=")).toContain("readonly=true");
  });
  ...
  test("provider comes from the server in BOTH directions of disagreement", async () => {
    ...
    expect(lineOf(formatStatusReport(a), "vox: server: provider=")).toContain(
      "provider=openai-realtime",
    );
    ...
    expect(lineOf(formatStatusReport(b), "vox: server: provider=")).toContain(
      "provider=gemini-live",
    );
  ...
  test("a wedged bridge is visible from --status, not only from a raw curl", async () => {
    ...
    expect(voxStatusExitCode(report)).toBe(0);
  });
  ...
  test("UNREACHABLE: local config is never asserted as the server's state", async () => {
    ...
    expect(out).toContain("vox: server state UNKNOWN — /healthz did not answer");
    // No line claims to describe the server …
    expect(out).not.toContain("vox: server: ");
    // … and the only line carrying the local values says so, loudly.
    expect(lineOf(out, "vox: local config (NOT the server):")).toBe(
      "vox: local config (NOT the server): provider=openai-realtime  readonly=true",
    );
    expect(voxStatusExitCode(report)).toBe(1);
  });
  ...
  test("MALFORMED: a body that is not a healthz body is reported, not crashed on", async () => {
    for (const body of [
      ...
      expect(out).not.toContain("vox: server: ");
      expect(out).toContain("vox: local config (NOT the server):");
      expect(voxStatusExitCode(report)).toBe(1);
    }
  });
  ...
  test("no line of any status report leaks the token", async () => {
    for (const body of [serverBody(), null, "garbage"]) {
      expect(formatStatusReport(await statusOf(body))).not.toContain(TEST_VOX_TOKEN);
    }
  });
  ...
  test("voxStatus with no probe seam uses the real HTTP fetch", async () => {
    const report = await voxStatus({
      tmux: tmuxSpy().tmux,
...
describe("stop", () => {
  ...
  test("is a no-op when no session exists", async () => {
    const spy = tmuxSpy();
    const logs: string[] = [];
    expect(await stopVox({ tmux: spy.tmux, log: (l) => logs.push(l) })).toBe(0);
...
describe("vox() entry", () => {
  ...
  test("--print-assets-dir prints the dir and exits 0 WITHOUT needing an api key", async () => {
    const out: string[] = [];
    const saved = { t: env.ATMUX_VOX_TOKEN, k: env.OPENAI_API_KEY };
    env.ATMUX_VOX_TOKEN = TEST_VOX_TOKEN;
    delete env.OPENAI_API_KEY;
    try {
      expect(await vox(["--print-assets-dir"], { out: (l) => out.push(l) })).toBe(0);
      expect(out).toHaveLength(1);
      expect(out[0]?.endsWith("/templates/vox")).toBe(true);
    } finally {
      if (saved.t === undefined) delete env.ATMUX_VOX_TOKEN;
      else env.ATMUX_VOX_TOKEN = saved.t;
      if (saved.k !== undefined) env.OPENAI_API_KEY = saved.k;
    }
  });

  test("--print-assets-dir honours ATMUX_VOX_ASSETS_DIR", async () => {
    const out: string[] = [];
    const saved = { t: env.ATMUX_VOX_TOKEN, a: env.ATMUX_VOX_ASSETS_DIR };
    env.ATMUX_VOX_TOKEN = TEST_VOX_TOKEN;
    env.ATMUX_VOX_ASSETS_DIR = "/opt/atmux/9.9/templates/vox";
    try {
      expect(await vox(["--print-assets-dir"], { out: (l) => out.push(l) })).toBe(0);
      expect(out[0]).toBe("/opt/atmux/9.9/templates/vox");
    } finally {
      if (saved.t === undefined) delete env.ATMUX_VOX_TOKEN;
      else env.ATMUX_VOX_TOKEN = saved.t;
      if (saved.a === undefined) delete env.ATMUX_VOX_ASSETS_DIR;
      else env.ATMUX_VOX_ASSETS_DIR = saved.a;
    }
  });

  test("bad argv throws UsageError before any wiring", async () => {
    await expect(vox(["--nope"])).rejects.toThrow(UsageError);
  });
  ...
  test("--stop routes to the graceful stop", async () => {
    const spy = tmuxSpy([VOX_TMUX_SESSION]);
    expect(await vox(["--stop"], { tmux: spy.tmux, log: () => {}, sleep: async () => {} })).toBe(0);
    expect(spy.killed).toEqual([VOX_TMUX_SESSION]);
  });

  test("--status exits 0 when up and 1 when down", async () => {
    const saved = env.ATMUX_VOX_TOKEN;
    env.ATMUX_VOX_TOKEN = TEST_VOX_TOKEN;
    try {
      const out: string[] = [];
      expect(
      ...
      if (saved === undefined) delete env.ATMUX_VOX_TOKEN;
      else env.ATMUX_VOX_TOKEN = saved;
    }
  });
  ...
  test("--status end-to-end reports the SERVER, even when the shell's env disagrees", async () => {
    ...
    const saved = { tok: env.ATMUX_VOX_TOKEN, ro: env.ATMUX_VOX_READONLY };
    env.ATMUX_VOX_TOKEN = TEST_VOX_TOKEN;
    env.ATMUX_VOX_READONLY = "1";
    try {
      const out: string[] = [];
      const code = await vox(["--status", "--provider", "gemini-live"], {
        tmux: tmuxSpy([VOX_TMUX_SESSION]).tmux,
        out: (l) => out.push(l),
        fetchHealthz: async () =>
          JSON.stringify({
      ...
      expect(code).toBe(0);
      const text = out.join("\n");
      expect(text).toContain("vox: server: provider=openai-realtime  readonly=false");
      expect(text).not.toContain("gemini-live");
      expect(text).not.toContain("readonly=true");
    } finally {
      if (saved.tok === undefined) delete env.ATMUX_VOX_TOKEN;
      else env.ATMUX_VOX_TOKEN = saved.tok;
      if (saved.ro === undefined) delete env.ATMUX_VOX_READONLY;
      else env.ATMUX_VOX_READONLY = saved.ro;
    }
  });
  ...
  test("--status with an unreachable server does not print the shell's env as fact", async () => {
    ...
    try {
      ...
      expect(code).toBe(1);
      const text = out.join("\n");
      expect(text).toContain("vox: server state UNKNOWN");
      expect(text).not.toContain("vox: server: ");
      // `readonly=true` appears exactly once, and only after the label
      // that says it is NOT the server.
      expect(text).toContain("vox: local config (NOT the server): ");
      expect(text.indexOf("readonly=true")).toBeGreaterThan(
        text.indexOf("vox: local config (NOT the server): "),
      );
    } finally {
      if (saved.tok === undefined) delete env.ATMUX_VOX_TOKEN;
      else env.ATMUX_VOX_TOKEN = saved.tok;
  ...
  test("--serve applies driver scope, builds deps and runs until aborted", async () => {
    ...
    const abort = new AbortController();
    const logs: string[] = [];
    let built: VoxServeDeps | null = null;
    const p = vox(["--serve", "--max-frames", "5"], {
      applyScope: () => {
        scoped = true;
      },
    ...
    expect(await waitFor(() => logs.some((l) => l.startsWith("vox: listening")))).toBe(true);
  ...
  test("--serve threads --port/--provider/--readonly through to config resolution", async () => {
    const abort = new AbortController();
    const p = vox(["--serve", "--port", "4399", "--provider", "openai", "--readonly"], {
      applyScope: () => {},
      buildDeps: async (opts) => {
...
describe("default output sinks", () => {
  ...
  test("serveVox defaults log→stderr and registers REAL process signal handlers", async () => {
    // `logToStderr` leaves `createVoxLogger`'s sink at its production
    ...
    const { out, err } = await captureStd(async () => {
      ...
      const p = serveVox({ deps, signal: abort.signal, checkModel: NO_MODEL_CHECK });
      await flush();
    ...
    expect(err).toContain("vox: listening on 127.0.0.1:");
    expect(err).toContain("vox: shutting down");
    // stdout is capture-owned while a tool's verb runs — nothing may land there.
    expect(out).toBe("");
  });

  test("vox() defaults --print-assets-dir to stdout", async () => {
    const saved = process.env.ATMUX_VOX_TOKEN;
    process.env.ATMUX_VOX_TOKEN = TEST_VOX_TOKEN;
    try {
      const { out } = await captureStd(async () => {
        expect(await vox(["--print-assets-dir"])).toBe(0);
      });
      expect(out.trim().endsWith("/templates/vox")).toBe(true);
    } finally {
      if (saved === undefined) delete process.env.ATMUX_VOX_TOKEN;
      else process.env.ATMUX_VOX_TOKEN = saved;
    }
  });

  test("vox() defaults its diagnostics to stderr", async () => {
    const spy = tmuxSpy();
    const { out, err } = await captureStd(async () => {
      expect(await vox(["--stop"], { tmux: spy.tmux })).toBe(0);
    });
    expect(err).toContain("no 'atmux-vox' session to stop");
    expect(out).toBe("");
  });

  test("stopVox's default sleep really waits between the SIGINT and the kill", async () => {
    const spy = tmuxSpy([VOX_TMUX_SESSION]);
    const started = Date.now();
    expect(await stopVox({ tmux: spy.tmux, log: () => {} })).toBe(0);
...
describe("production default wirings", () => {
  ...
  test("buildVoxDeps with no loadTeamIndex seam reads the real cockpit roster", async () => {
    const dir = `${process.env.TMPDIR ?? "/tmp"}/atmux-vox-cockpit-${process.pid}-${Date.now()}`;
    ...
    await Bun.write(
      ...
      JSON.stringify({
        ...
        cockpitSession: "atx",
        sessions: [
          { type: "team", name: "vox-fixture-team", root: "/root/work/src/atmux", enabled: true },
          {
            type: "team",
            name: "vox-disabled-team",
    ...
    try {
      const deps = await buildVoxDeps({
        env: { ATMUX_VOX_TOKEN: TEST_VOX_TOKEN, OPENAI_API_KEY: TEST_OPENAI_KEY },
        makeProvider: () => new FakeProvider(),
      });
      // Disabled sessions are filtered out by `enabledTeams`.
      expect(deps.teamIndex.teams.map((t) => t.name)).toEqual(["vox-fixture-team"]);
...
describe("assets dir resolution (V-1)", () => {
  test("dev-mode resolves <repo>/templates/vox and the files are really there", async () => {
    const { deps } = await buildTestDeps();
    expect(deps.assetsDir.endsWith(resolve("templates", "vox"))).toBe(true);
...
// ---------- `atmux voice` deprecated alias (ADR-274 D2) ----------
//
// SUNSET(v0.9.1): delete this block together with the alias.
//
// These cases are built to DISAGREE. An alias test where both names
// merely "work" proves nothing — it passes just as happily against a
// build with no alias at all, because the canonical name already works.
// So each case pins something only the alias can produce: the stderr
// notice, its ABSENCE on the canonical verb, and the fact that the alias
// does not write the old name into anything it generates.
describe("`atmux voice` — deprecated alias for `atmux vox`", () => {
  const env = process.env;

  /** Runs `fn` with a valid token in the environment, then restores it. */
  ...
  test("the alias warns, and the notice names BOTH verbs, the ADR and the sunset", async () => {
    const logs: string[] = [];
    await withToken(async () => {
      await voice(["--print-assets-dir"], { out: () => {}, log: (l) => logs.push(l) });
    });
    expect(logs).toContain(VOICE_VERB_DEPRECATION);
    expect(VOICE_VERB_DEPRECATION).toContain("atmux voice");
    expect(VOICE_VERB_DEPRECATION).toContain("atmux vox");
    expect(VOICE_VERB_DEPRECATION).toContain("ADR-274");
    expect(VOICE_VERB_DEPRECATION).toContain("v0.9.1");
  });

  test("the CANONICAL verb emits no deprecation notice — no warning tax", async () => {
    const logs: string[] = [];
    await withToken(async () => {
      await vox(["--print-assets-dir"], { out: () => {}, log: (l) => logs.push(l) });
    });
    expect(logs).not.toContain(VOICE_VERB_DEPRECATION);
    expect(logs.some((l) => l.includes("deprecat"))).toBe(false);
  });

  test("argv is forwarded untouched — identical stdout and exit code", async () => {
    const viaAlias: string[] = [];
    const viaCanonical: string[] = [];
    await withToken(async () => {
      expect(
        await voice(["--print-assets-dir"], { out: (l) => viaAlias.push(l), log: () => {} }),
      ).toBe(0);
      expect(
        await vox(["--print-assets-dir"], { out: (l) => viaCanonical.push(l), log: () => {} }),
      ).toBe(0);
    });
    expect(viaAlias).toEqual(viaCanonical);
    expect(viaAlias[0]?.endsWith("/templates/vox")).toBe(true);
  });

  test("errors behave identically — bad argv still throws UsageError", async () => {
    await expect(voice(["--nope"])).rejects.toThrow(UsageError);
  });

  test("the alias does NOT perpetuate itself: --supervise still execs `vox --serve`", async () => {
    // The shim keeps an old muscle-memory invocation working; it must not
    // write the old name into a wrapper that then re-execs it on every
    // crash-loop restart for the life of the session.
    const spy = tmuxSpy();
    expect(
    ...
    const opts = spy.created[0] as Record<string, unknown>;
    expect(String(opts.shellCommand)).toContain("'/usr/local/bin/atmux' vox --serve");
    expect(String(opts.shellCommand)).not.toContain("voice --serve");
  });
});
