...
| `ATMUX_VENDORED_TMUX_BIN`            | `/opt/atmux/current/bin/tmux`                | Driver-only cockpit resolver override. `driverOnly: true` routes through this fail-closed vendored path; ordinary atmux calls remain on the legacy resolver and this path never falls back to system PATH. |
