...
| `ATMUX_TMUX_BIN`                     | system `tmux` on PATH                        | Legacy/live tmux resolver override for every atmux call that still spawns the host plane (ADR-191). Use this when the wrapper wants a specific host tmux binary; ordinary atmux processes still fall through to PATH when unset. |
| `ATMUX_VENDORED_TMUX_BIN`            | `/opt/atmux/current/bin/tmux`                | Prepared seam for the future vendored plane only. The repo does not auto-route ordinary atmux calls through this path yet; it falls back only to the canonical vendored binary and never to system PATH. |
