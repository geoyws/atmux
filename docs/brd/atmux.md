...
**Status**: proposed
**Date**: 2026-08-06
**Amended**: 2026-08-16 — BR9 added (the vox operator interface, [ADR-272](../adr/272-voice-operator-interface.md) / [ADR-274](../adr/274-atmux-vox-rename.md)), with the §5 non-goal 1 boundary it sits inside and the §7.5 risk it introduces.
...
2. **PR review noise.** atmux state drifting through diffs makes every product PR harder to read, and the reviewers paying that cost never opted into atmux. [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §S3 states the target directly: *"No commit noise in product repos. Reviewers in sopx/unum/rentx never see atmux changes drifting through PRs."*
...
**Status.** DESIGN MET, ENFORCEMENT MANUAL. All atmux state lives in the operator's personal dotfile tree at `~/work/journals/.sb/_dotfiles/atmux/<repo-key>/`, symlinked into each managed repo's `.atmux/`; Node `fs` follows symlinks transparently so no code change was needed ([ADR-239](../adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §Supplement-2026-05-26 line 226, [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §Supersession-2026-05-26 §S1–§S2). This repo's `.gitignore` line 1 is `.atmux/*` with the citing stanza. Backup is `dotfiles push` at operator cadence ([ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §S3), which replaced the per-repo pre-commit hook proposed in that ADR's now-HISTORICAL D1–D6. The gap is BR5.

**Governing decisions.** [ADR-239](../adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §Supplement-2026-05-26, [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §Supersession-2026-05-26.
...
**Acceptance signal.** Each of BR1–BR7 and BR9 states a command or query whose output settles it. Each is written to be falsifiable, and each names the seam it would be checked from.

**Status.** PARTIAL — BR2/BR3/BR4 signals are checkable today with the commands stated above, as is BR9's first leg (its second leg is a phone check and cannot be settled headlessly); BR1/BR5/BR6/BR7 signals depend on mechanisms that are `proposed`, not shipped ([ADR-267](../adr/267-durable-agent-continuity-contract.md), [ADR-268](../adr/268-managed-repo-state-isolation-enforcement.md), [ADR-269](../adr/269-recursive-branch-ledger.md)).

### BR9 — The operator can read the fleet without a terminal

**Requirement.** The operator's read of what the fleet is doing does not depend on him sitting at a keyboard. Away from the desk — walking, in a lift, in a car — he can ask what needs his attention and get an answer that carries the evidence behind it.

**Rationale.** §2.2 item 4, from the other side. BR1 removes the operator from the *continuity* path; BR9 addresses the path he remains on and cannot be removed from. Under [ADR-260](../adr/260-manual-orchestration-mode-default.md) manual orchestration is the fleet default — measured at 20 of 20 `team.json` files in [ADR-271](../adr/271-sqlite-sole-store-rust-orchd-coordinator.md) — so the operator and the lead LLMs **are** the coordination mechanism. An operator who cannot be reached is therefore not a missing convenience; he is a missing scheduler, and every lane awaiting a decision queues against the same ceiling §2.2 item 4 names. Until 2026-08-14 the availability was zero by construction rather than by accident: every operator verb assumed a keyboard and a shell, and atmux deliberately had no HTTP surface at all ([ADR-261](../adr/261-issue-sync-external-tracker-ingestion.md) §Context, which pays outbound-polling-only for that property).

**Acceptance signal.** Two, and the second is the one that makes the first worth having. (a) `atmux vox --status` reports whether the supervised session is live and names the posture it is running under, read from the running server rather than assumed. (b) Asked from a phone, *"what needs my attention?"* returns a ranked list in which every item carries the evidence that classified it, and the empty case returns an explicit aggregated all-clear rather than silence — so *"nothing needs you"* is distinguishable from a sweep that failed. Same argument as BR7's clean-tree half: a survey that cannot report clear cannot be believed when it reports attention.

**Status.** PARTIAL, deliberately. The capability ships as `atmux vox` and is deployed **read-only** (`ATMUX_VOX_READONLY=1`) — it reads the fleet and changes nothing, with the mutating tools absent from the catalog handed to the provider and independently refused at the bridge. The read half of this requirement is therefore met and the unblock half is not; moving work along by voice is a separate, deliberate step, and so is typing into a wedged agent's pane ([ADR-273](../adr/273-voice-fleet-triage-and-pane-input.md) D4/D5, gated on an operator decision). Nor does the interface volunteer anything: there is no proactive narration, so it answers when asked and never raises a hand. Renamed from `atmux voice` by [ADR-274](../adr/274-atmux-vox-rename.md); both pre-rename surfaces (`atmux voice`, and `ATMUX_VOICE_*` as a fallback where the `ATMUX_VOX_*` equivalent is unset) keep working with a warning until v0.9.1. Surface detail — tools, flags, posture — is [docs/PRD.md](../PRD.md) §3.7 and `docs/RUNBOOK-vox.md`, not this document (§1.1).

**Governing decisions.** [ADR-272](../adr/272-voice-operator-interface.md) (the interface: every tool is an argv-built `atmux` verb call, and the server acts with driver scope), [ADR-273](../adr/273-voice-fleet-triage-and-pane-input.md) (fleet triage — survey half built, pane input not), [ADR-274](../adr/274-atmux-vox-rename.md) (the name, and the two dated shims it owes), [ADR-260](../adr/260-manual-orchestration-mode-default.md) (manual orchestration default — why operator reachability is load-bearing at all), [ADR-233](../adr/233-cron-auto-install-disabled-trust-orchd.md) (operator-started, nothing at boot), [ADR-266](../adr/266-shim-sunset-policy-and-first-sweep.md) §D1 (sunset markers on the rename's shims).
...
The operator asked on 2026-08-06 whether the project is *"all over the place."* On the evidence, the **mission is coherent and the surface is not.** R1/R2/R3 all sit inside the stated vision; BR4's design was fully reasoned in May 2026 ([ADR-239](../adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) / [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md)); BR6's problem was correctly analysed in April 2026 ([ADR-035](../adr/035-per-member-branch-recursive-ops.md)). What has accumulated is **unretired surface and unmaintained top-level documentation**, which is a real risk to this BRD because it makes the authority chain in §1.2 harder to follow:
...
- [docs/PRD.md](../PRD.md) §1.2 principle 2 read *"State lives on disk in JSON / markdown... greppable, diffable"* — superseded twice over, by [ADR-126](../adr/126-sqlite-state-store.md) (SQLite at `.atmux/state.db`) and by [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §Supersession-2026-05-26 (state relocated to the operator's dotfile tree). **Corrected 2026-08-06** in the same batch; the durability claim was retained because it was never the stale part.
...
BR4's solution puts all atmux state in the operator's dotfile tree with `dotfiles push` as the snapshot mechanism at operator cadence ([ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §S3, replacing that ADR's per-repo pre-commit hook). Consequences accepted with it: (a) state is only as durable as that cadence, and nothing verifies the cadence ran; (b) a managed repo cloned onto a second machine has **no** atmux state until the dotfile tree is present there, because `.atmux/` is a symlink into it; (c) BR2's durability guarantee covers agent death, not dotfile-tree loss. No measure in §6 currently observes snapshot freshness — that is a gap, not a solved problem.

### 7.5 Vox is the first capability that reaches in from outside

Every other surface in this document is reachable only from a terminal on the operator's own host. BR9's is not: it is a WebSocket endpoint, and the server invokes verbs with `ATMUX_CALLER_SCOPE=driver` ([ADR-272](../adr/272-voice-operator-interface.md) §D3), so **whoever reaches that socket is the driver.** This is a risk of a different shape from the rest of the document — BR4 is about state leaking *out* to colleagues; this is about capability reaching *in*.

The mitigations are layered rather than singular ([ADR-272](../adr/272-voice-operator-interface.md) §Security): an authenticating proxy in front, a `≥32`-character token compared timing-safely before the WebSocket upgrade, an `Origin` allowlist, a loopback bind so only the proxy can reach the port, and the read-only posture the feature ships in. Provider credentials never leave the host — the phone talks to the operator's box, never to the AI provider. Residual risk accepted plainly: defence-in-depth is not a proof, and clearing the read-only flag retires the layer that is currently doing the most work.
...
| *"keep all plans and intents in atmux so that the git repo can be clean of our artifacts"* | BR4 | [ADR-239](../adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §Supplement-2026-05-26, [ADR-244](../adr/244-per-repo-pre-commit-kanban-decisions-snapshot.SUPERSEDED.md) §Supersession-2026-05-26 |
...
**Later ask — BR9.** BR9 does not trace to the 2026-08-06 ask, and the table above is deliberately that day's trace rather than a running log. It traces to a separate operator-direct ask on 2026-08-14, recorded in [ADR-272](../adr/272-voice-operator-interface.md) §Driver-ref — a spoken interface to the fleet from a phone, for the operator away from the desk — and to the 2026-08-16 rename in [ADR-274](../adr/274-atmux-vox-rename.md). Both ADRs carry `Status: proposed`, so the §1.2 rule applies to BR9 exactly as it does to the rows above, even though the mechanism happens to be shipped and deployed.
