# ADR-060: SQLite for `.atmux/` state, JSON archive-only
...
**Supersedes / extends:** ADR-005 (JSON+lock model) — narrows the JSON scope; ADR-057 D3a/D3c (lock-TTL + atomic-write) — becomes mostly obsolete for migrated state, retained for the JSON files that stay.
...
1. `flock` advisory locks on a sidecar `.lock` file (ADR-005)
...
- **Not** a multi-process coordination overhaul. SQLite's transactions + WAL handle the cases that ADR-005 + ADR-057 D3 were patching. Other coordination concerns (member↔lead, cron↔interactive) are unchanged.
...
`openDatabase(path)` runs pending migrations on open. ADR-016 (schema-version-deferred-until-v2) is superseded by this for SQLite-managed state; markdown + team.json continue without versioning.
...
The Phase 3 parity matrix (ADR-026/027) currently does byte-exact / JSON-aware diffs on `.atmux/kanban.json`. Post-ADR-060, the parity harness gains a SQLite-aware comparator: opens both `state.db` files, diffs row sets per table (set-difference + row-level diff for matching IDs). The channel-mask config (ADR-027) extends to `db.tasks`, `db.epics`, `db.stories` channels. Existing JSON channels deprecate as files migrate.
...
Post-burn-in. Once all teams have `state.db` + parity harness shows zero divergence + bash decommission complete (ADR-013 Phase 5 finished), `core/kanban.ts` drops the JSON write fallback. Tracked as v1.2.x.

## Cross-references

- ADR-005: JSON + locking model (predecessor)
- ADR-014: v2 verb redesign (parallel — SQLite migration is Phase-5 / v1.1.x; verb redesign is Phase-6 / v2)
- ADR-016: schema-version-deferred (superseded for SQLite-managed state)
- ADR-026 / ADR-027: parity matrix + channel-mask (extends to SQLite channels)
- ADR-057 D3a/D3c: lock-TTL + atomic-write (mostly obsolete for migrated state)
- ADR-058: multi-tier fallback chain (paused; resumes post-cutover, orthogonal to ADR-060)
