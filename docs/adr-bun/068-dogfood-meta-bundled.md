# ADR-068: Dogfood-meta — complaints lane, cross-team targeting, nomenclature taxonomy fix, hot-reload, live-status, rename-safe identity (bundled)
...
- **ADR-011 (hot-reload)** — partial supersession + extension. Existing `brief-reload` / `config-reload` / `verify-libs` infra is sufficient for verb-table refresh; this ADR adds schema-migration-on-touch + read-side compat-shim infra. `brief-versions.json` convention preserved.
- **ADR-bun/021 (coordination-runtime-contract)** — direct rename blast radius. The HC#1 rename targets the verb-contract path constants (`driver-inbox.md` / `lead-outbox.md`). ADR-021 is **amended** (not superseded): verb names + path constants update; verb behaviour preserved. ADR-021's mode matrix (driver / solo / lead / no-team) untouched.
- **ADR-bun/060 (sqlite-state-store)** — schema extension. New `complaints` table per ADR-060 migration discipline (PRAGMA `user_version`; idempotent INSERT-IF-NOT-EXISTS).
- **ADR-007 (pull-kanban)** — preserved. Per-member task-queue shape continues; only the file-name word changes (`inboxes/<name>.json` → `tasks/<m-id>.json`). Lane semantics + `claim --next` cross-lane fallback untouched.
- **ADR-027 (team rename verb + topology invariant)** — extended. New `atmux team rename-member <old> <new>` extends ADR-027's atomic-rename-or-rollback model with member scope. Archive-don't-rewrite discipline applies.
...
- Display layer prints `displayName` (fallback to `name`); archive markdown is **not rewritten** (per ADR-027 archive-don't-rewrite discipline). Live refs follow the ID.
...
**New verb**: `atmux team rename-member <old> <new>` — atomic, alias-augmenting, no migration. Extends ADR-027's transactional shape (rename-lock → team.json → registry alias append → lock clear; rollback in reverse on partial failure).

Existing `atmux team rename <old> <new>` (ADR-027) extends the same way: registry alias append on top of the existing flow.

### D3 — SQLite complaints schema

New table in `.atmux/state.db` per ADR-060 migration discipline:
...
PRAGMA `user_version` bumped per ADR-060 OQ-3. Schema-add via idempotent migration on next state-touching `atmux` invocation (HC#3 hot-reload).
...
Building on ADR-011's existing `brief-reload` / `config-reload` / `verify-libs` infra:
...
ADR-011's deferred verbs (`swap-tui`, per-claim brief snapshot) remain deferred — not load-bearing for this ADR.
...
| `cage` | The isolated workspace each team gets, complete with its own door. | Per-team tmux session under unique `tmuxTmpdir` socket (ADR-018). |
...
| `claim` | "I'll take this one, boss." | Member pulls a task from todo, marks in-progress + assigns to self (ADR-007). |
| `done` | The mandatory status update. | Mark claimed task complete; auto-dispatches commit-Task to gitter (ADR-013/057). |
...
- **OQ-3 (low)**: Cross-team complaints initial substrate — file-write vs ADR-032 socket-pubsub. → **file-write substrate; socket-pubsub migration is non-breaking** (registry-keyed routing the same either way).
