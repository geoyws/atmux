# ADR-018: Integration contract with `/coordination:*` Claude skills plugin
...
| Invariant | Skills expect | Bash atmux | atmux-bun (post-ADR-017) |
|---|---|---|---|
...
The ADR-017 prefix drop is the only **deliberate** divergence. The `~/.claude/teams/<team>/...` family of paths was simply never implemented on the atmux side because atmux predates the Driver Mode skills and chose to keep all coordination state inside `.atmux/` per-project.
...
ADR-017 stands. The operator-stated preference for `<emoji><member>` over `__<team>__<emoji><member>` is explicit and recent; reverting to satisfy the skill would be the wrong tradeoff.
...
- **ADR-017 stands** — the operator's window-naming preference is preserved; the integration cost is paid by sidecar files rather than UI ergonomics.
...
### A. Revert ADR-017's prefix drop
...
- ADR-013 (WIP-bash deferral) — governs cross-language drift; the symmetric position to take here.
- ADR-017 (window naming — drop `__<team>__` prefix) — the originating divergence this ADR mediates.
