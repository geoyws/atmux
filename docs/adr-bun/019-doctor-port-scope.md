# ADR-019: `doctor` verb (V-24) — port scope + deferred bash-only checks
...
- They detect leaks from **Phase-5-deferred features** (cage-prefix leak, caged-windows-outside-cage, supervisor-liveness, cockpit, daily-driver-prefix-leak) that the TS port doesn't ship per ADR-013 § "Phase 5 deferral."
...
Porting all 22 in TS would add hundreds of lines of code that test artifacts the TS port doesn't produce. The bash-side checks aren't going away (atmux-bash stays live during burn-in per ADR-011), so operators who need those checks run `atmux doctor` (bash) on the same machine and get the full report.
...
| `tmux-tmpdir` | ❌ deferred | Phase-5 cage tmpdir invariant. Re-enable when ADR-013 § Phase-5 lands. |
...
- **Bash atmux remains the authority** for the bash-only checks during burn-in (ADR-011). Operators run `atmux doctor` (bash) for the full battery; `atmux-bun doctor` (TS) for the in-scope set. Both work side-by-side.
...
- ADR-009 (test strategy) — coverage gate applies to in-scope checks; deferred checks aren't tracked code at all.
- ADR-011 (cutover protocol) — bash atmux is the authority during burn-in.
- ADR-013 (WIP-bash deferral) — Phase 5 super-* / cage / cockpit are deferred; doctor checks for those features follow.
- ADR-018 (`/coordination:*` integration) — V-25 whip-watchdog cron integration; the deferred `crontab` + `cron-orphans` doctor checks land alongside.
