# ADR-005: JSON + locking model

**Status:** accepted — **narrowed by [ADR-060](060-sqlite-state-store.md)**: kanban (`tasks/epics/stories`), inboxes, and per-feature state moved to `.atmux/state.db` (SQLite, WAL). This ADR remains authoritative for `team.json`, JSONL append-logs, the markdown surface, and bash atmux's full state surface.
**Date:** 2026-05-04
**Owner:** architect
...
**Reviewer rule:** `JSON.parse(...)` is forbidden in `src/core/*` and `src/verbs/*`. The only place `JSON.parse` is allowed is `src/abstractions/json.ts`. Custom lint enforces (per ADR-003).

**Schema-mismatch policy:** parse failures throw `SchemaError` (ADR-006) carrying:
...
- Run 5 times; zero failures = pass; any failure = ADR-005 reopened
...
- Schema drift becomes a typed crash, not a silent persistence of garbage. Closes one of bash atmux's three most-painful bug classes (per ADR-001 Pressure 3).
...
- ADR-006 (error handling) — `SchemaError`, `LockError`, `LockTimeoutError`, `FsError` defined here.
...
Revisit at v3 if file-locking continues to bite. v2 (ADR-014) keeps JSON.
...
- ADR-001 Pressure 3 (no static typing across JSON boundary — this ADR closes it)
- ADR-003 (module taxonomy — `json.ts` and `lock.ts` are abstractions)
- ADR-006 (error handling — SchemaError / LockError / LockTimeoutError / FsError)
- ADR-009 (test strategy — fixture factories use the same Zod schemas)
- ADR-011 (cutover — cross-language lock compatibility is a cutover requirement)
