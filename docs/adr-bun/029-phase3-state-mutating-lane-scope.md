...
|------|----------------------|--------------|------------------|
| `reply` rows 7–8 | `--from lead "test msg"` / `--from w1 "test msg from member"` | **F11 (TS-port semantic drift) RED at HEAD as of 2026-05-06 17:35 MYT.** Probe-verified: bash `lib/reply.sh:64-69` awk emits trailing newline on `lead-outbox.md` (43 byte / 56 byte short / pre-seeded variants); TS `src/verbs/reply.ts:insertOpenEntry` regex consumes trailing whitespace then writes entry without trailing newline (42 byte / 55 byte). Diff is 1-byte trailing `0a`. Per ADR-027 §4 anti-semantic-mask rule a stateAfter regex on the file's final newline would broadly absorb other content; rejected. Up-impl marked `t-fc18d1f6` done with F10/F12/F14 staged but **F11 untouched** (per up-impl 17:07 MYT lead-outbox reply: "F11 was already GREEN at HEAD" — but state-impl re-probe at HEAD @17:07 + @17:35 contradicts: still RED, 1-byte gap). State-impl surfaced via `atmux send up-impl` + `atmux reply` at commit-G time; tracking issue is the F11 fix alone, NOT a row-design change. | When F11 fix lands in `src/verbs/reply.ts:insertOpenEntry` (1-line — the up-impl-authored fix sketch in `t-fc18d1f6` body: change regex from match-and-consume-trailing-whitespace to non-consuming, with replacement appending entry+newline). Saved repro patch at `/tmp/adr-029-reply-rows-blocked.diff` (46 LOC) per up-impl. State-impl re-applies the 2 reply rows in a follow-up commit `test(parity): 2 reply rows with markdown line-append (INSERT, ADR-029 rows 7-8 — post-F11)` once the fix commit lands. **No infra change needed in matrix.ts / compare.ts** — the path-suffix stdout mask + TIMESTAMP_MASK already cover all other surfaces; only the F11-driven trailing-newline gap is blocking. |
| `tell-lead` `--from` flag | (originally row 9–10 axis) | **Spec-error correction.** ADR-029 §1 originally listed `--from w1` as the discriminative axis on rows 9–10. Probe-verified at HEAD: neither `lib/tell.sh:16` nor `src/verbs/tell-lead.ts:86` USAGE accepts `--from` — bash silently consumes as message body (`tell-lead --from w1 "test ask"` writes `- [HH:MM MYT] --from w1 test ask\n`); TS rejects with `usage: atmux tell-lead <msg...>` (exit 64). Both sides legitimately diverge on stderr (bash dies on no-tmux-window AFTER appending; TS dies on usage BEFORE appending), but the divergence isn't a bug — it's a non-existent flag axis. | Drop the `--from` axis. Single-arg `<msg>` is the canonical bash-frozen shape; rows 9–10 use that. NO follow-up; this is a closed correction, not a deferral. |
...
6. ~~**Commit F — `test(parity): 2 reply rows with markdown line-append (INSERT, ADR-029 rows 7-8)`**~~: **DEFERRED post-F11.** Probe 2026-05-06 17:35 MYT confirms `src/verbs/reply.ts` still emits 1-byte trailing-newline gap on `lead-outbox.md` vs bash `lib/reply.sh:64-69`. Up-impl marked `t-fc18d1f6` done with F10/F12/F14 staged but F11 untouched (per up-impl 17:07 MYT lead-outbox reply). State-impl re-applies rows 7–8 in a follow-up commit `test(parity): 2 reply rows with markdown line-append (INSERT, ADR-029 rows 7-8 — post-F11)` once F11 fix lands in `src/verbs/reply.ts:insertOpenEntry`. Repro patch saved at `/tmp/adr-029-reply-rows-blocked.diff` (46 LOC) per up-impl. See deferred-row table above for full handle.
7. **Commit G — `test(parity): 2 tell-lead rows with markdown + tmux side-effect (INSERT, ADR-029 rows 9-10)`**: rows 9 + 10 land. **Spec-error correction:** ADR-029 §1 originally listed `--from w1` axis on rows 9–10; probe-verified neither bash `lib/tell.sh` nor TS `src/verbs/tell-lead.ts` accepts `--from`. Single-arg `<msg>` is the canonical bash-frozen shape; rows 9–10 use that shape (probe-confirmed byte-equal — short 23b / long 88b on `driver-inbox.md`). Lifecycle preset has no panes, so both sides die on no-tmux-window AFTER the file write — Pattern A error-rendering prefix mask + `exitCode: true` mask (ADR-006 sysexits) cover the divergence.
