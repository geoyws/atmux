# ADR-025: `SendTarget` discriminated union — type-system enforcement of "no send-keys to driver pane"
...
ADR-026 cage variants and super-driver ADRs (045/046) are likely to add new pane kinds (e.g. `cockpit-viewer`, `cage-attach`, `super-driver-relay`). The migration path is:
...
- **Caller refactor (R-6).** This ADR designs the contract; the refactor that updates `sendKeys` / `pasteBuffer` signatures + the 3 callers + tests lands in a separate `refactor(abstractions): tmux sendKeys + pasteBuffer accept SendTarget (R-6, ADR-025)` commit.
...
- **More pane kinds when topology lands.** ADR-026 cage and super-driver variants will widen `SendTarget`'s union — additive, no contract break.
