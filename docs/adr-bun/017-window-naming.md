# ADR-017: tmux window naming — drop `__<team>__` prefix
...
1. **Disambiguate** atmux-spawned windows from operator-created tmux windows on the same session — useful when atmux ran in single-session mode (ADR-013) where the driver, members, and the operator's own working windows could share one tmux session.
...
Bash atmux at HEAD `2aadc3f` retains the prefixed form. ADR-013 §"Phase 5 deferral" governs cross-language drift while the bash side stays live in production. Port-back to bash piggybacks on the next bash-side window-naming change (no urgency — the prefix still works, just suboptimally).
...
- ADR-003 (module taxonomy — common is the right home for this helper)
- ADR-013 (WIP bash deferral — governs cross-language drift)
