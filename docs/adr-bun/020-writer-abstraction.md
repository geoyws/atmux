# ADR-020: `Writer` abstraction + shared `core/io.ts` for verb stdout/stderr injection
...
4. **Commit D — `docs(adr,plan): R-5 done — ADR-020 + flip §6.2 status`**: this ADR file + flip R-5 row in `PLAN.md`.
