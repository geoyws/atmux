# ADR-064: Complete the driver-role port
...
1. **Port `lib/driver.sh` → `src/verbs/driver.ts`**. Two subcommands:
   - `brief-driver` — read-only ≤30-line state recovery. Single
     ...
     task list (ADR-060: bash's `jq <kanban.json>` becomes a typed
...
review brief. ADR-064 itself is in scope for review-pre-land sweep.
...
`.archive-bash-atmux-20260507/lib/driver.sh` per ADR-013). The
...
2. **OQ2 — 60-char message ceiling on `driver note`**. Default: keep
   ...
   condense; mirrors decisions.md's discipline (ADR-008).
