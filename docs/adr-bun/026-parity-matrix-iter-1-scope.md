...
| State-mutating verb happy-path rows (`task add` / `dispatch` / `inbox` / `done`) using `lifecycle` preset | ❌ iter-2 | Iter-1 in-scope at ADR authoring; relabeled `❌ iter-2` during commit-4 prep (2026-05-05). Probe at `/tmp/parity-probe` confirmed: bash `atmux task add 'test'` writes `kanban.json` with `id="t-XXXXXXXX"` (8-hex random) + `createdAt` Unix-epoch — both differ on every run. `compare.ts:176-189` has NO state-after JSON-field masking → any byte-different `fs.<file>.parsed` pushes `Divergence`. Plus `index.test.ts` limitation noted at `version.test.ts:46-50`: parallel `runVerb` against same fixture races state writes — needs per-side fixture cloning. The original "matrix dispatch is sufficient for the deterministic happy-path" claim was wrong: state-mutating verbs are NON-deterministic on happy-path output, not just on error paths. **Re-enable handle:** ADR-027 (parity channel-mask contract — covers BOTH error-rendering for the 4 row-6 verbs AND state-after JSON-field masks for state-mutating verbs) + `compare.ts` mask infrastructure + `index.test.ts` per-side fixture cloning. Iter-2 lead-off. |
...
| Reconciliation of bash↔TS error-rendering divergence (init / start / send / add-member) | ❌ iter-2+ | Probe at `/tmp/parity-probe` (whip-impl, 2026-05-05) confirmed real divergence on the no-team config-error path: bash emits `💥 atmux no team.json at <p> — run 'atmux init' first`/exit 1; TS emits `atmux: config: no team.json at <p> (hint: run 'atmux init' first)`/exit 78 (BSD `EX_CONFIG`). Activating the 4 standalone `test.todo` skeletons (init / start / send / add-member) via matrix rows would surface non-empty `Divergence[]` on stderr-byte + exit-code channels and fail the iter-1 commit's CI gate. Surfaced this turn; lead routed to driver as architectural meta-decision (mirror-bash's lossy `exit 1`-for-all-errors vs accept divergence + per-channel parity-mask). **Re-enable handle:** per-verb reconcile-and-flip commit — `fix(verbs): <verb> — bash-parity error rendering + matrix row` — each commit decides canonical exit-code semantics for that verb's error class + amends ADR-006 if BSD sysexits is dropped. Owners assigned post-iter-1 by lead per verb-porter familiarity. |
...
- **State-mutating verb row additions** — original commit-4 plan; commit-4 prep relabeled the row to `❌ iter-2` (see Decision §row 3 + this commit's body for the discovery). Iter-2 lead-off after ADR-027 + `compare.ts` masks + `index.test.ts` per-side fixture cloning land.
...
4. **Commit D — `docs(adr-bun): ADR-026 update — state-mutating non-determinism + iter-2`**: this update — relabel state-mutating row from `✅ iter-1` → `❌ iter-2`, add re-enable handle pointing at ADR-027 + `compare.ts` masks + `index.test.ts` per-side fixture cloning, append iter-1 actual delivery summary to Consequences. Original commit-4 plan (4 matrix rows) deferred to iter-2 lead-off.
...
- `f86f3da` — `docs(adr,plan): ADR-026 — parity matrix iter-1 scope (refs ADR-009 §3)` — this ADR.
- `8e82ed2` — `feat(parity): wire PARITY_MATRIX with version + unknown-verb (2 rows; init/start/send/add-member deferred per ADR-026)` — matrix-driven dispatch active for 2 verbs.
...
- This commit — `docs(adr-bun): ADR-026 update — state-mutating non-determinism + iter-2` — captures commit-4-prep discovery and relabels row 3.

### Iter-2 lead-off entry list

1. ADR-027 author — parity channel-mask contract (covers BOTH error-rendering masks for `init` / `start` / `send` / `add-member` per row 6 AND state-after JSON-field masks for state-mutating verbs per row 3).
2. `compare.ts` mask infrastructure — per-channel masking hooks per ADR-027 (timestamp regex + JSON field-elision for `id` / `createdAt`).
