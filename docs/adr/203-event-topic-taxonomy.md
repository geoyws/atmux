...
> ⚠ **SUPERSEDED IN EFFECT 2026-08-27 by [ADR-276](276-orchd-retirement-and-atmux-scope.md).** The topic vocabulary remains declared in `src/schema/events.ts`, but the task/epic-topic consumer fleet this ADR anticipated is retired with orchd. Surviving consumers: gitter + lane-router + the slimmed event-subscription registry (complaint consumer, lead-stall watchdog), all drained by `committer --drain`. The auto-push subscription died with its only emitter (`epic.merged` has none). Kept as history.
