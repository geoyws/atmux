...
## §Amendment 2026-05-24 — `emit()` auto-detects `honkerLoaded` from `getHonkerState(db)` (NOTIFY-skip class of latent bugs)

The §Amendment 2026-05-22 (I) "`emit()` now publishes to Honker stream when loaded" wiring required every caller to thread `opts.honkerLoaded: true` through every `emit()` call site. Callers that forgot the flag — or recovery scripts / ad-hoc tooling that emit() directly without going through a wrapper — silently degraded to durable-INSERT-only: the event landed in the events table, but no NOTIFY published to `_honker_notifications`, so daemons listening via `watchEvents()` (the native-listener path) never woke. The cron-backstop drain catches them eventually, but the substrate's wake-up promise (~100ms latency per §Amendment 2026-05-22 §2) silently downgrades to the 10-min cron cadence.

**Trigger event**: 2026-05-24 gitter bootstrap wedge-recovery session. The gitter member spawned, found two fan-ins (`atmux-geoyws-docs` +9, `atmux-geoyws-reviewer` +2) wedged at `ready_to_merge` (the wedge that ADR-134 §Amendment 2026-05-24 closes structurally). Wrote a synth-event recovery script — `bun -e "emit(db, {topic:'task.done', ...})"` — directly against the events.ts API to wake the daemon. Three emits landed durably; the daemon (subscribed via native-listener) never woke; orchd subscribers that polled the events table DID see them and advanced their offsets, so the gitter consumer's `withIdempotency` loop subsequently saw them but never got the wake-up. Net: a recovery mechanism that should have been instant required two more troubleshooting cycles to surface the NOTIFY-skip root cause.

**Resolution**: `emit(db, payload, opts)` now computes `const shouldNotify = opts.honkerLoaded ?? (getHonkerState(db)?.loaded ?? false)`. When `opts.honkerLoaded` is undefined (the omitted-default), it falls back to the cached honker state for this DB handle. Callers that ran `bootHonker(db)` upstream — which includes every production caller via the standard `committer --daemon` / `--drain` / `--sweep` boot path — automatically get NOTIFY without changing any call site. Callers that never bootHonker'd (test fixtures, pure durable-INSERT paths) see no behavior change because `getHonkerState(db)` returns `null` → `loaded=false` → no NOTIFY attempt.

**Explicit-override semantics preserved**: passing `opts.honkerLoaded: false` still suppresses NOTIFY (force-off, useful for tests that exercise the durable-only path); passing `opts.honkerLoaded: true` still force-attempts NOTIFY (useful for test harnesses with custom DB lifecycles where bootHonker wasn't called but the substrate is known-loaded). The `??` chain consults `opts.honkerLoaded` FIRST so explicit overrides always win over auto-detect.

**Carve-out — `honker_stream_publish()` absence still tolerated**: the existing inner `try/catch` around the SELECT continues to swallow "no such function" errors. Auto-detect changes WHEN the publish is attempted, not how failures propagate. A DB that booted into fallback (`{loaded: false, fallbackReason: ...}`) still skips the publish attempt entirely — auto-detect honors the fallback flag.

**Reviewer surface** — call-site simplification is followup territory: `src/core/kanban.ts:275-276` + `:466` build conditional `emitOpts` from `getHonkerState(db)?.loaded` and pass `{honkerLoaded: true}` only on the true branch. Both can drop the conditional + always call `emit(db, payload)` with no opts (or with only their UUID/clock injection seams) — emit() will auto-detect identically. Defer the call-site cleanup to its own commit to keep this amendment's blast radius scoped to the abstractions/events.ts default.

**Filed via** gitter wedge-recovery session 2026-05-24 09:00–10:00 MYT (atmux team auto-merge bootstrap). Companion to ADR-134 §Amendment 2026-05-24 (sweep IN_FLIGHT_STATES post-rebase-break recovery) — both filed in the same session by the same root cause (recovery paths silently lost the wake-up signal).
