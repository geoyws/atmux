...
- ADR-183 §Decision — deploy-completeness probe-class precedent (sibling pattern; the `code-shipped-not-wired` probe in this ADR is the generalization of ADR-183's specific probe across all wedge classes per the same probe-class registry contract).
