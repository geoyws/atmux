...
>
> **§Decision-anchor #4** — **Version-lock v2 — promote ADR-162 §3 warn-probe to refusal at session-start.** ADR-162 §3 shipped a warn-probe that runs on `atmux doctor` and emits `🟡 host tmux too old (got X, want ≥Y), bundled tmux recommended` without blocking. The vendored `aca` / `aco` cockpit path now opts into `resolveVendoredTmuxBin()` and refuses any implicit fallback to host PATH. Ordinary atmux paths stay on `resolveTmuxBin()` and keep the legacy host-PATH behavior; `atmux doctor` / `atmux status` remain read-only probes. `ATMUX_USE_HOST_TMUX=1` remains a legacy-resolver debug escape only. The distinct explicit `ATMUX_VENDORED_TMUX_BIN` override is an operator-controlled path; the shipped wrapper sets it to `/opt/atmux/current/bin/tmux`.
>
...
- **Operator-installed tmux replacement.** Historical/superseded as an automatic universal rollback story. The vendored resolver never falls back to host PATH. Its explicit `ATMUX_VENDORED_TMUX_BIN` override can deliberately select another executable for debugging, but the shipped `aca` / `aco` wrapper pins `/opt/atmux/current/bin/tmux`; normal vendored operation stays fail-closed until that bundle is restored.
