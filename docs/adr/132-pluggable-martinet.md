...
Impls live in `src/abstractions/martinets/{claude,minimax,cursor,kimi}.ts`. Escalation classifier (formal pane-state → NudgeAction mapping) lives in `src/core/martinet-escalation.ts`. Fleet-wide tick loop lives in `src/verbs/martinet.ts`.
...
**Cadence**: per-team via `team.json::martinetOverrides.cadenceSec` (per-impl defaults: claude=270, minimax=300, cursor=270, kimi=600). Fleet-wide tick runs at `min(per-team cadence)` — typically 270s. The tick loop is fired from cockpit W3 via `/loop /martinet` (same skill-driven pattern superdoctor uses in W2).

**Cage sharing**: the Martinet cage is provisioned at cockpit rebuild (not per-team `atmux start`). One cage per unique `martinet` value across enabled teams — i.e. if every team is `minimax`, one shared MinimaxMartinet cage handles all teams in W3. Mixed-backend fleets (some teams `claude`, others `minimax`) instantiate one cage per backend; the fleet-wide tick loop dispatches the right impl on each team's iteration.

### (D4) Four initial impls — `claude` baseline + 3 non-Claude variants
...
The Martinet runs in its own cage with its own runtime token budget — **independent of the Claude Max budget** that ADR-049 budget-pause guards. A team with `martinet: "minimax"` continues whipping during a Claude lead's budget-pause window. This is the load-bearing operational win: **the team's commit cadence becomes decoupled from the Claude lead's budget cycle** for the mechanical-observation tier.
...
- **T7** (e2e) lands the operator-runbook snippet (how to flip a team to MinimaxMartinet) in `docs/RUNBOOK-*.md` in the same commit.
...
- **Token-side win**: replaces ~300k Opus tokens/hour/team with ~30k MiniMax / Cursor tokens/hour/team for the mechanical pass + ~15-30k Opus tokens/hour for escalations only. Order-of-magnitude reduction in Claude budget burn per team. With cockpit-level shared cages, the per-fleet (not per-team) MiniMax cost is amortised across all teams using `minimax` backend.
- **Cage-side cost**: one cage per unique martinet backend in cockpit W3. Tier 3 impls (minimax, kimi) need one dedicated Linux user each — provisioned at cockpit rebuild via ADR-050 script. atmux dogfood opt-in costs 1 user (`minimax-agent`) + 1 W3 cage at cockpit rebuild time.
...
Whether `team.json::martinet` field changes should hot-swap the running backend (e.g. flip a team from `claude` to `minimax` without rebuilding cockpit) or require an `atmux cockpit rebuild` cycle.
...
| T3 | `src/abstractions/martinets/minimax.ts` + Tier-3 cage provisioning + unit tests | T2 | be |
| T4 | `src/abstractions/martinets/cursor.ts` + `src/abstractions/martinets/kimi.ts` + unit tests | T2 | be (parallelizable with T3) |
...
| T7 | e2e — synthetic cockpit with MinimaxMartinet + 1hr cycle + lead-token-burn assertion (<30k Opus/hr) | T3, T5, T6 | test |
...
Sub-task IDs: T1 = `t-0a889489` (this Task); T8 = `t-fb5e4c1f` (added 2026-05-14 reshape). T2-T7 IDs land alongside in the EPIC sub-task block.

Reviewer flips this ADR Proposed → Accepted in a follow-up commit per the EPIC's acceptance gate.

## Out of scope

- **Cross-model consensus** — multi-Martinet voting on actions (one MiniMax in W3, one Kimi in W4 for divergence detection). Defer to Phase 3 if model-diversity in observation surfaces concrete value beyond single-observer.
...
- **Custom Martinet impls beyond the initial 4** — operator-provided plug-ins via a registry of impl files. Defer to a follow-up ADR if a fifth impl surfaces (e.g. Anthropic Haiku-as-martinet when a different cost/latency tradeoff materialises).
