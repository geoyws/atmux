# ADR-106: WIP-bash deferral (Phase 5 scope)
...
- **`team-migrate-to-cage.sh`** — migration script for the cage-socket topology refactor (ADR-109 in bash repo).
...
- v2 (Phase 6) closure has a clear definition: WIP-as-of-Phase-5-snapshot ported + verb redesign per ADR-107 done.

**Negatives:**
- v1 ships missing modules that some operators may already use in unfinished form on their bash atmux installs. Mitigation: bash binary stays as `atmux-legacy` for the 4-week rollback window (ADR-104), and operators using WIP-bash modules just keep using `atmux-legacy <verb>` until v2.
...
- ADR-104 (cutover protocol — `atmux-legacy` rollback window covers operators using WIP-bash modules)
- ADR-107 (verb design debt — Phase 6 sequencing depends on Phase 5 finishing first)
