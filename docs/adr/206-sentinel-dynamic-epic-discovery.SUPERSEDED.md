...
**Primary**: (A) parent state.db epics table query. The kanban is the single source of truth for "what work exists" already (per decision number 076, no local ADR file exists, + ADR-126); reading epic-team membership from the same store keeps the abstraction layer thin. Cost is well under budget. Failure mode (DB unreachable) is the rarest of the four.
