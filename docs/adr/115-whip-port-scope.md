...
| Auto-stop check (`_atmux_whip_check_auto_stop`) | ❌ deferred | Phase-5 super-driver concern — auto-stops on cost-budget breach. TS port lacks the budget-policy machinery. Re-enable handle: historical decision number 046 (no surviving ADR file) (super-driver) or whichever Phase-5 ADR scopes auto-stop. **2026-05-07 (t-a3a0e5b1):** the bash-side `_atmux_whip_check_auto_stop` on atmux-geoyws now carries an ADR-052 §Whip-integration eternal-improvement intercept (pre-stop check on `.atmux/state/eternal-improvement.json::active`; if not active, invokes `atmux improve --idle-fallback --default-budget` and returns 0 on success). When the TS port re-enables the auto-stop check, the intercept must be ported alongside (it lives in the same function, not a separate helper). |
