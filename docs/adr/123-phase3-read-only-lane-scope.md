# ADR-123: Phase 4a parity matrix iter-3 read-only lane scope (refs ADR-119, ADR-120)
...
Iter-2 (ADR-120) closed ADR-119 rows 3 + 6 — 4 state-mutating happy-path rows (`task add` variants, `lifecycle` preset, INSERT class) + 4 error-rendering rows (`init` / `start` / `send` / `add-member` with the iter-2 3-pattern stderr mask). `PARITY_MATRIX` now has 10 active rows; `compare.ts` channel-mask infrastructure + per-side fixture cloning are operational; iter-2 mask vocabulary covers two divergence classes: error-rendering (style) + state-after (random IDs / epoch).

Phase 4a (per HANDOFF "PHASE 4 RUSH" + lead-outbox 2026-05-06 ~11:16 MYT) parallelises five matrix-expansion lanes: cron-fired (ADR-121), state-mutating UPDATE/DELETE (ADR-122), **read-only (this ADR — ADR-123)**, lifecycle (ADR-124), and error-class (ADR-125). Each lane scopes its own per-verb output channels + mask classes before row commits land, mirroring ADR-119's iter-1 carve-out shape.
...
| `dashboard` | interactive loop | stdout (full-screen redraw), exit on signal | screen-clear ANSI (`\e[2J`, `\e[H`), tick-driven sleep, SIGINT-trap exit message → not single-shot testable through current runner contract | **DEFER iter-4+** (separate dashboard-loop harness ADR — analogous to ADR-121 cron-runner) |
...
ADR-120 §"Out of plan" §"Mask classes beyond error-rendering + state-after" anticipated incremental class additions. This ADR adds two:

1. **`ADR-123 read-render class`** — divergent rendering of read-only output where neither side is "wrong" but the surface bytes differ. Symptoms: column-width disagreement, conditional-emission disagreement (one side always-emits, other side gates on a count). Mask: anchored regex that elides the divergent line/segment on both sides. Banned: regex broad enough to absorb semantic content (per ADR-120 §4).

2. **`ADR-123 timezone-render class`** — divergent timestamp rendering where bash uses host-local tz and TS pins UTC (per ADR-105 + cost.ts:411-416 explicit UTC pin "so test output is deterministic regardless of host timezone"). Symptoms: `YYYY-MM-DD HH:MM:SS` substrings differ by host UTC-offset hours. Mask: regex eliding the `YYYY-MM-DD HH:MM:SS` substring on both sides. Per ADR-120 §4 cite-locality, mask cites this class in inline `// reason:` comment on each entry.

Both classes integrate with existing `ChannelMask` shape (matrix.ts:46-51); no `compare.ts` infrastructure changes required.

## Decision

ADR-123 read-only lane scope is **9 matrix rows** across 4 verbs (`status`, `doctor`, `inbox`, `cost`) — each row carries its mask class inline + `// reason:` cite per ADR-120 §4. `dashboard` contributes ONE error-path row only; the interactive loop is deferred to a separate dashboard-loop harness ADR.
...
### Mask schema authoring per ADR-120 §3 + §4

Each row's `mask:` block carries inline `// reason:` cites referencing one of:
- `(ADR-120 error-rendering class)` — for prefix / path / hint masks shared with iter-2 rows.
- `(ADR-120 state-after class)` — N/A for this lane (no state-mutating rows).
- `(ADR-123 read-render class)` — for `status` row 1 header-line + driver-inbox-line elision.
- `(ADR-123 timezone-render class)` — for `cost` row 6 since-render elision.
- `(ADR-099 BSD sysexits)` — for `exitCode: true` skips, identical to iter-2.
...
### NOT in scope of THIS commit (per ADR-115 / ADR-118 / ADR-119 craftsmanship pattern)

- **`compare.ts` infrastructure changes.** ADR-123's two new mask classes (read-render, timezone-render) are **labels** consumed by the existing `ChannelMask.stdout` / `ChannelMask.stderr` regex infrastructure. No new `ChannelMask` keys; no new helpers in `compare.ts`. ADR-120's `applyChannelMask` already covers both classes.
- **Matrix row authoring.** This ADR pins the 9-row set; the actual `PARITY_MATRIX = [...]` lines land in commit 2 (`test(parity): 4 read-only happy-path rows (status/doctor/inbox/cost, ADR-123 rows 1-6)`) and commit 3 (`test(parity): 4 read-only error rows (inbox/dashboard/cost no-team, ADR-123 rows 7-10)`).
- **Dashboard interactive-loop harness.** Per the deferred-row table; iter-4+ via a separate dashboard-loop ADR (analogous to ADR-121's cron-runner pattern). Re-enable handle: when iter-4+ adds a `tests/parity/dashboard-loop-runner.ts` with `maxFrames` injection + signal-based shutdown probe, `dashboardLoop` becomes parity-testable.
- **JSON-canonical-stdout mask class.** Defers `status --json` / `inbox --json` / `doctor --json` rows. Re-enable handle: when a `ChannelMask.stdoutJson?: true` flag (or equivalent) lands in `compare.ts`, parsing both sides' stdout as JSON + canonical-comparing — analogous to the `fs` channel's existing JSON treatment. ADR-120 amendment + new helper. Iter-4 lead-off candidate.
...
- **ADR-099 amendment.** TS keeps BSD sysexits + structured-tag stderr; bash side stays frozen. No exit-code reconciliation.
- **Bash-side rendering changes.** Bash status header widths, bash discord-hint phrasing, bash inbox `cat "$f"` JSON shape — all frozen per ADR-119 §"NOT in scope". The mask absorbs the divergence harness-side.
...
| `status --json` / `inbox --json` / `doctor --json` | iter-4 JSON-canonical-stdout mask class (ADR-120 amendment) |
...
1. **Commit A — `docs(adr-bun): ADR-123 — phase3 iter-3 read-only lane scope (refs ADR-119, ADR-120)`**: this ADR file. Reviewer gates against ADR-119 + ADR-120 to verify it's a scope carve-out + mask-class extension (not an infrastructure rewrite). Two new class labels added without `compare.ts` changes.
2. **Commit B — `test(parity): 4 read-only happy rows (status/doctor/inbox/cost, ADR-123 rows 1-6)`**: rows 1–6 added to `PARITY_MATRIX`; new mask classes cited inline. Reviewer's `mask:` grep verifies all class labels present.
3. **Commit C — `test(parity): 4 read-only error rows (inbox/dashboard/cost no-team, ADR-123 rows 7-10)`**: rows 7–10 added; mask vocabulary identical to iter-2 (no new patterns).
4. **Commit D (optional, conditional on iter-3 surface findings) — `docs(adr-bun): ADR-123 update — N rows iter-3 actual delivery`**: post-landing summary mirroring ADR-119 §"Iter-2 actual delivery" shape.

Each commit standalone-passes typecheck + 100% coverage gate (the parity tests themselves are excluded from the lcov denominator per ADR-102 §2). Reviewer gates each commit per the 8-check protocol (PLAN.md §9). Reviewer scans this ADR against ADR-119 + ADR-120 to verify it's a scope + class-label addition, not a shape redesign.
...
- **Two new mask classes (read-render, timezone-render) extend ADR-120 vocabulary** without requiring `compare.ts` changes. Both consumed by the existing `applyChannelMask` regex path. iter-4+ adds JSON-canonical-stdout / row-set when their re-enable handles fire.
...
- **`dashboard` interactive loop deferral** preserves PLAN.md §14 functional-parity target without forcing single-shot harness contortions. The error-path row exercises `requireTeam` precondition agreement — sufficient for ADR-112 V-24-style "in-scope subset" coverage.
- **`status` text + `inbox` text are byte-equal under `NO_COLOR`** (the runner already sets it). The read-render class fires only on `status`'s header + driver-inbox conditional, not on `inbox` — narrow-scope mask per ADR-120 §4.
- **`cost` text mode rejects host-tz drift via timezone-render mask** rather than reconciling rendering. ADR-105 (time-and-timezone) keeps TS at UTC for determinism; bash keeps host-local for operator-friendliness; the harness absorbs the divergence.
...
**Landed:** 8 of 10 planned rows + 2 fix commits to enable them. PARITY_MATRIX grew 32 → 40 rows (commit-B-time baseline; cron-impl's parallel ADR-121 work also added rows in the same window). 8 rows × 100% green at @3064ce3.
...
| 4 | `inbox` | `lead` | `lifecycle` | ⏭ deduplicated — already covered by ADR-122 commit H rows 11-12 | (n/a — pre-existing in matrix) |
| 5 | `cost` | `--json` | `lifecycle` | ⏸ deferred to iter-4 — JSON-render `0.0000` vs `0` divergence on `totalUsd` not maskable per ADR-120 §4 anti-semantic-mask rule | (deferred) |
...
- **`15139eb` `fix(parity): doctor probe cleanup + loadKanban noLock (ADR-123)`** — closes F-readonly-1 + F-readonly-2:
  ...
  - F-readonly-2: TS `loadKanban` wrapped `updateJson` with default `withLock`, leaving `.atmux/kanban.json.lock` sidecar that bash never created (lib/status.sh:85-91 + lib/kanban.sh use direct unlocked `jq` reads). Fix: pass `noLock: true` per ADR-098 §"single writer per file" + ADR-122 F2/F9 precedent (commit `4e2e103`) — atomicWrite at writer guarantees readers see pre/post state, never partial.

### Commit-attribution gap

ADR-123 happy rows 1, 2, 3, 6 were intended to land in a discrete `test(parity): 4 read-only happy rows (status/doctor/cost, ADR-123 rows 1-3+6)` commit but instead got swept into cron-impl's `76bd071 feat(parity): golden-file harness mode` SHA via a worktree git-add collision (other porter ran `git add -A` while my matrix.ts edits were unstaged). The matrix entries are correct, only the commit attribution is off-trail. Surfaced via `atmux reply` for lead awareness; no history rewrite — the rows are green and the commit body is clear from the matrix-entry comments alone (the `// ADR-123 commit B:` block self-documents).
...
- **`error-rendering` class (ADR-120, reused)** — fires on rows 7, 8, 9, 10. Two narrow extensions:
