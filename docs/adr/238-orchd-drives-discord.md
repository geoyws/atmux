...
- ADR-068 (bash to ts cutover — no surviving ADR file) — bash discorder cutover history; the `[whip-progress]` / `[whip-heartbeat]` template-name byte-parity decision this ADR finishes reversing per ADR-237 §D4.
