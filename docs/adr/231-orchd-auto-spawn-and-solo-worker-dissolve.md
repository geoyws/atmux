...
> **⚠ PARTIALLY SUPERSEDED by [ADR-233](233-cron-auto-install-disabled-trust-orchd.md) — 2026-05-24 (§D4 cron-backstop only; `orchd --sweep` cron line retired). Remainder canonical: event-driven `epic.ready` / `epic.unblocked` subscribers + solo-worker dissolve semantics stay live.**
