...
- Original bun-port rationale (retained, not duplicated here): `docs/adr-bun/057-stall-prevention.md` (referenced by the retired `HANDOFF.md` operator section; the file is no longer in tree post-bash-decommission per commit `2696539`, but the rationale is reconstructed verbatim above).
