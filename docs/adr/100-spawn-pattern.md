# ADR-100: Subprocess spawn pattern (`Bun.spawn` wrapper)
...
| `flock` | 2 | concurrent-write protection (ADR-098 obsoletes) |
| `mktemp` | 5 | atomic-write tmp file |
| `date` | 8 | timestamps (ADR-105 obsoletes) |
| `command -v` | 7 | doctor `which`-style checks |

After the port:
- `jq` is gone (parsed JSON in-memory via ADR-098).
- `flock`, `mktemp`, `date` move into TS abstractions (ADR-098, ADR-105).
- `tmux`, `curl`, `command -v` remain external shellouts. `command -v` is not a real shell-out — Bun has `Bun.which()`. So it goes too.
- The Discord webhook send goes through `~/.claude/skills/whip/scripts/ping-discord.sh` (per ADR-101), which is itself a shell script the TS code spawns.
...
This is the foundation that ADR-097 (tmux), ADR-101 (discord), and any future shellout-using abstraction sits on top of. ADR-099 declares the error classes `SpawnError` + `SpawnTimeoutError`; this ADR specifies the API that throws them.
...
Logger uses `time.formatMyt()` from `src/abstractions/time.ts` (per ADR-105) for log line timestamps. No `Date()` usage in `spawn.ts` itself except via `Bun.nanoseconds()` for duration measurement (which is monotonic + locale-blind).
...
- Every external invocation goes through one place. ADR-099 R4 (no `Bun.spawn` outside this file) is mechanically enforceable.
...
- ADR-097 (tmux) consumes `spawn` for every subcommand wrap.
- ADR-101 (discord) consumes `spawn` for the Discord shell-script call.
...
Considered. Works in Bun, more familiar to Node devs. Rejected because `Bun.spawn` has cleaner async semantics (`proc.exited` is a real Promise, no event-listener dance) and integrates with Bun's runtime monitor. We're committed to Bun (ADR-095); using its native primitive is the consistent choice.
...
Rejected. Per-subsystem wrappers belong at the abstraction layer (`tmux.ts`, `discord.ts`). The spawn wrapper is one layer below — it doesn't know about subsystems. Putting tmux-specific argv-construction in `spawn.ts` would violate ADR-096 (abstractions don't know about each other's domains).
...
- ADR-095 Pressure 2 (silent error swallow; bash spawns are the primary instance)
- ADR-096 (module taxonomy — `spawn.ts` is one of the 8 abstractions; everyone shells out through it)
- ADR-097 (tmux abstraction — consumer)
- ADR-099 (error handling — declares `SpawnError`, `SpawnTimeoutError`)
- ADR-101 (discord webhook — consumer)
- ADR-105 (time + timezone — logger uses `time.formatMyt`)
