# ADR-099: Error handling discipline
...
| **Optional read with default** | `readJsonOr(path, schema, fallback)` (per ADR-098) | Helper makes intent visible at call site. |
...
### Reviewer rules summary (per ADR-099 + ADR-096)
...
- **R3.** No `JSON.parse` outside `src/abstractions/json.ts`. (ADR-098)
- **R4.** No `Bun.spawn` outside `src/abstractions/spawn.ts`. (ADR-100)
- **R5.** No `new Date().toLocale*` outside `src/abstractions/time.ts`. (ADR-105)
...
- **R11.** Every new `src/**/*.ts` file MUST carry an `// ADR-NNN: <title>` comment within the first 20 lines, naming the ADR that governs its design. Files with genuinely no architectural decision (a pure helper, a barrel re-export) carve out via an explicit `// ADR: pure helper, no design decision` (or similar) one-line comment. Excluded paths: `src/types/generated/**`, `tests/**`, `**/*.fixtures.ts`. Reviewer originally proposed by tester in ADR-102 §3.5; promoted here so all R-rules live in one place.

**Enforcement split — ADR captures rules, foundation-porter implements enforcer.** This ADR is the *spec* for the rules R1–R7 + R11. The mechanical enforcer is `scripts/lint-discipline.ts`, which is a **Phase 1 deliverable owned by foundation-porter**, not Phase 0 scope. The ADR ratifies the rule set so reviewers can hand-grade against it during the gap; once `lint-discipline.ts` lands, those rules fold into the automated 8-check.

The custom lint script (`scripts/lint-discipline.ts`, foundation-porter Phase 1 task) bundles R1, R2, R6, R7, R11. Biome's `noRestrictedImports` (configured in Phase 0 `biome.json`) covers R3, R4, R5. ADR-101 R8/R9/R10 (Discord-specific) are type-level enforced (literal-union `template:`) + runtime-validated (bullet shape) inside `src/abstractions/discord.ts` itself; no `lint-discipline.ts` regex needed for those.

**Single source of truth.** Other ADRs that need to reference R-rules (e.g. ADR-102 §3.5 reviewer regex enforcement) link to this section rather than re-state the regex inline — keeps the regexes from drifting across docs. If a future ADR needs to *narrow* an exception (e.g. additional path allowlist), the narrowing lives in the consuming ADR with an explicit "extends ADR-099 R-N" cross-ref.
...
- ADR-100 (spawn): SpawnError + SpawnTimeoutError throw shape consistent with this hierarchy.
- ADR-101 (discord): DiscordWebhookError throw shape consistent.
- ADR-105 (time): no error class needed; abstraction is pure.
...
- ADR-095 Pressure 2 (silent error swallow culture, the failure this ADR closes)
- ADR-096 (module taxonomy — `src/errors.ts` is the universally-importable leaf)
- ADR-098 (json+lock — declares SchemaError, LockError, LockTimeoutError)
- ADR-100 (spawn — declares SpawnError, SpawnTimeoutError)
- ADR-101 (discord — declares DiscordWebhookError)
