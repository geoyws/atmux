...
### 2026-09-02 — gate-1 source window `_superdriver` → `_sd`; never-rotate key; `/bruh` references are historical (ADR-288)

[ADR-288](288-superdriver-lane-shortform-and-multi-lane-cockpit.md) §D1 renames cockpit window 1 to **`_sd`**, so gate 1 (§Pre-flight gate matrix row 1) now captures `<cockpit_session>:=_sd` (tmux exact-match target — a prefix-matched `:_sd` would resolve to a lone `_sd2` lane), falling back to `<cockpit_session>:=_superdriver` when that capture is empty, for the deprecation window only; gate-2 capture, Ctrl-C and kill-window on the `_medic` role window use the `=` form as well, team viewer targets do not. Gate 4 (row 4) keeps the role name **`superdriver`** as its canonical key — the `gate-4-never-rotate-superdriver` identifier, the Discord template and the audit row's `sessionName` are unchanged — and additionally refuses the shortform `sd` and both window literals `_sd` / `_superdriver`, still ignoring `--force`. ADR-288 §D3 records the measured behaviour for the new `_sdN` lanes: they are not rotate targets — `classifyRole` files them as `team-driver` and the verb refuses right after classification, before gates 1–3 and before any handoff payload is written, with `team '<name>' not found in cockpit.json` (exit 70, pane and `__cockpit__/team-driver/handoff.md` untouched). The §Ordering invariant is unchanged for real targets: the handoff is still written before Ctrl-C. The `/bruh` skill this ADR's §Context, §Consequences and §Related cite (Rung A–D escalation chain, §3a manual fallback) was **retired and deleted per ADR-288 §D4**; those references are now historical. The Rung A–D ladder remains a valid description of this verb's own scope (Rung C).
