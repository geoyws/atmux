...
Per-team sockets remain on the existing cage-tier `-S <team-root>/.atmux/tmux/tmux-0/default` path per ADR-058 (cage tier isolation — no surviving ADR file) — unaffected by ADR-162. The `_-prefix` window-name format from this ADR's §D2 / §D3 is preserved verbatim on the new socket; ADR-162 §Decision-anchor #3's `automatic-rename off` in `templates/tmux/atmux.conf` is what protects the `_-prefix` contract from tmux's auto-rename stomping.
