...
| 282 | Never collect the whole environment in a test — allowlist at the source, guard the class | 2026-08-28 | proposed |
| 283 | Scrub the test runner's environment — a property of the environment, not a pattern over code (amends ADR-282's coverage claims + ADR-281's `atmux.conf` claim) | 2026-08-28 | proposed |
