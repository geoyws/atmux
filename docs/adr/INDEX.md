...
> Generated 2026-05-24, refreshed 2026-06-05 (MYT) — extended through ADR-256 + backfilled 222/223. Refreshed again 2026-08-06 — extended through ADR-269: backfilled the five missing rows 262–266 (the index stopped at 261) and added the 2026-08-06 operator-ask batch (267 continuity / 268 managed-repo state isolation / 269 recursive branch ledger). Refreshed again 2026-08-14 — extended through ADR-272: added 272 (voice operator interface) and backfilled the missing 271 row (ADR-271 §D9 item 7 deferred its own index edit to avoid a concurrent clobber). Refreshed again 2026-08-15 — extended through ADR-273 (voice fleet triage + pane input). Refreshed again 2026-08-16 — extended through ADR-274 (`atmux voice` → `atmux vox`); ADR-272's row keeps ADR-272's own title, which ADR-274 §D1 deliberately does not rename. Refreshed again 2026-08-18 — extended through ADR-277 (cage colour-environment scrub) and backfilled the missing 276 row (the index stopped at 275). Refreshed again 2026-08-28 — extended through ADR-282: added 281 (tmux child-environment scrub at the spawn seam; amends ADR-277) and 282 (never collect the whole environment in a test). ⚠ **ADR-283 was PROPOSED and WITHDRAWN on 2026-08-29 — no such file exists and none is missing; the next ADR written takes 284.** Its central guarantee was falsified by measurement and its `bun test` refusal collided destructively with `autoMerge.revertOnFail`; the retraction is recorded in ADR-281 §Retraction 2026-08-29 and ADR-282 §Retraction 2026-08-29. That number was taken on 2026-08-30 by ADR-284 (uniform cockpit window sizing), which this refresh adds. Refreshed again 2026-08-30 on merging `origin/atmux-geoyws-hax-final-20260829`: that branch had **independently assigned ADR-281** to the cooperative `_bot` / `_superbot` offer protocol, colliding with the NO_COLOR spawn-seam ADR-281 already accepted here. The collision was resolved on merge in favour of the incumbent — the NO_COLOR ADR keeps 281 (it is cross-cited by ADR-277, ADR-282, `templates/tmux/atmux.conf` and the regression suite) and the bot/superbot ADR was renumbered to **285**; **the next ADR written takes 290.** ⚠ **Concurrent reservation batch on 2026-09-03: ADR-286 and ADR-287 remain intentionally occupied by parallel writers, while ADR-288 and ADR-289 are now landed and accepted here; the next free number remains 290.** ⚠ **ADR-281's status was corrected from `accepted` to `proposed` on 2026-08-28** — it had been self-accepted by the implementing agent in the same commit as its own code, which `/CLAUDE.md` §"Binding discipline" #4 does not permit. **ADR-270 is a deliberate gap in the sequence — no such ADR exists and none is missing.** Live ADRs only; superseded ADRs at the bottom for trace.
...
| 288 | Driver window worker/attention pane pair — three-driver floor, command-null attention launch, and later-slice contract | 2026-09-03 | accepted (operator-direct 2026-09-03) |
| 289 | HAX KB is the sole continuity authority for agent work | 2026-09-03 | accepted (operator-direct 2026-09-03) |
...
- [267](267-durable-agent-continuity-contract.md) — Durable agent-continuity contract — plan/intent is written as you go, not captured on the death-bed — superseded by ADR-289
