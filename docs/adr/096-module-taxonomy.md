# ADR-096: Module taxonomy (abstraction / core / domain)
...
ADR-095 commits us to TS-on-Bun; ADR-130 commits us to a `src/` tree. This ADR defines **what kinds of code go where**, and how they may depend on each other.

The bash codebase has no enforced layering. Every `lib/*.sh` is sourced into a single global namespace; there is no static check that `lib/whip.sh` doesn't reach into helper variables defined deep inside `lib/discord.sh`. The result is the godfile drift documented in ADR-095 Pressure 1: `whip.sh` (1305 LOC) and `doctor.sh` (1467 LOC) accumulated responsibilities because the namespace allowed it.
...
|---|---|---|
| `tmux.ts` | the `tmux` CLI | One method per subcommand bucket (ADR-097) |
| `json.ts` | parse/serialize JSON files atomically | Wraps `Bun.file().json()` + atomic write; calls into `src/schema/<file>` (ADR-098) |
| `lock.ts` | file-locking | `proper-lockfile` or `node:fs.flock` (ADR-098) |
| `fs.ts` | filesystem primitives | mkdir, rename, exists, glob — typed wrappers over `node:fs/promises` |
| `time.ts` | clock + formatters | `now()`, `formatMyt()`, `formatDuration()` (ADR-105); test-injectable `setNow` |
| `spawn.ts` | subprocess invocation | `Bun.spawn` with timeout + stderr capture (ADR-100) |
| `http.ts` | HTTP requests | thin wrapper over Bun `fetch` with retry + JSON helpers |
| `discord.ts` | Discord webhook send | Named-template enforcement + chunking (ADR-101) |

**Rules:**
- No abstraction imports another abstraction. Exception: any abstraction may import from `spawn.ts` if it shells out (e.g. `tmux.ts` uses `spawn.ts`; `discord.ts` uses `spawn.ts` to call the Discord shell script per ADR-101).
- Every external-IO failure surfaces as a typed `AtmuxError` subclass (per ADR-099). No silent swallows.
...
The 4-module list is the **starting set**. ADR-130 allows new core modules over time. Rule: a new core module justifies itself only if 2+ verbs would import from it; otherwise the helper stays inside the single verb that needs it.
...
(7 of the 30 user-visible verbs are aliases routed at the dispatcher level: `broadcast` → `send`, `tell-lead` → `tell`, `outbox` → `reply`, `task` → `kanban`, `done` → `claim`, `rotate-lead` → `rotate`, `resume` → `pause`. Per ADR-103.)
...
- `src/errors.ts`: `AtmuxError` base + tagged subclasses (per ADR-099). Imported by everyone. No incoming dependencies on app code.
...
- Layer is enforced at PR time, not at architectural-discipline time. The "godfile drift" failure mode of bash atmux (Pressure 1, ADR-095) cannot recur because growth in `verbs/whip.ts` cannot reach into private state of `verbs/dispatch.ts` — the import graph forbids it.
...
- v2 redesign (ADR-107) lands inside `src/verbs/` only: subcommand layouts (`src/verbs/task/{add,list,claim,done}.ts`) preserve the rule "no sideways verb imports" because each leaf still owns its own logic.
...
- Aliases force a choice: do we have `verbs/broadcast.ts` (file) or do we route `broadcast` to `verbs/send.ts` at the dispatcher? Decision: route at dispatcher per ADR-103. Each `src/verbs/*.ts` file is a distinct *implementation*, not a distinct *user-visible verb*.
...
- ADR-097 (tmux abstraction interface) — fills in the contract for `src/abstractions/tmux.ts`.
- ADR-098 (JSON + locking) — fills in `src/abstractions/json.ts` + `src/abstractions/lock.ts`.
- ADR-099 (error handling) — defines the error hierarchy that every layer throws.
- ADR-100 (spawn pattern) — defines `src/abstractions/spawn.ts`, the only place `Bun.spawn` is allowed.
- ADR-101 (Discord webhook) — defines `src/abstractions/discord.ts` named-template enforcement.
- ADR-103 (CLI dispatcher) — formalises alias routing in `src/cli.ts`.
...
Considered for v1. Rejected because it's noise at the v1 surface (most verbs are <200 LOC and don't justify a folder). v2 (ADR-107) introduces folders only where the subcommand redesign makes them natural (`src/verbs/task/*.ts`, `src/verbs/member/*.ts`). v1 stays files-only.
...
- ADR-095 Pressure 1 (godfile drift, the failure this taxonomy prevents)
- ADR-130 (project layout — directory shape that holds this taxonomy)
- ADR-097, ADR-098, ADR-099, ADR-100, ADR-101 (abstraction-specific contracts)
- ADR-103 (CLI dispatcher — verb file resolution + alias routing)
- ADR-107 (v2 verb redesign — preserves this taxonomy, restructures `src/verbs/`)
