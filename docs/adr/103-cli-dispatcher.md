...
1. **Alias routing.** 7 aliases: `broadcast`, `tell-lead`, `outbox`, `task`, `done`, `rotate-lead`, `resume`. ADR-096 routes them at dispatcher level so each maps to a verb file without duplicating logic.
2. **v2 subcommand structure.** ADR-107 introduces `task <sub>` / `member <sub>` in Phase 6. The dispatcher must accommodate this without a rewrite — i.e. v1 ships with a structure that allows nested commands later without churning the dispatcher core.
3. **`--help` / `--version` / unknown-verb error.** Standard plumbing.
4. **Exit codes.** Per ADR-099 — UsageError → 64, ConfigError → 78, etc. Dispatcher's job to translate.
...
(`reportError` is the implementation from ADR-099.)
...
3. **Lazy verb imports are critical for cold start.** ADR-095 cites Bun cold-start ~25ms. Dynamic `import("./verbs/${verb}.ts")` ensures only the verb you ran is loaded — `whip` doesn't pay for `dashboard`'s code. Frameworks vary on this; citty is good, commander+oclif eagerly require everything.
4. **v2 nested subcommand support is straightforward.** When ADR-107 lands `task <sub>`, the dispatcher path becomes:
...
`readVersion()` reads `package.json` via `Bun.file` + JSON.parse-via-schema (per ADR-098; package.json gets its own minimal schema). Output format matches bash: `atmux v0.4.0` (no leading `atmux`, no SHA).
...
Exit code 64 (EX_USAGE) per ADR-099.
...
ADR-107's v2 redesign restructures verbs:
...
- Adding a new exit-code-bearing error class (e.g., `LockTimeoutError` → 75 BSD `EX_TEMPFAIL`) requires only:
  ...
  2. Add tag → exit code in `exitCodeForTag()` (ADR-099 §exit-code table).
...
- ADR-107 (Phase 6): add `SUBCOMMAND_VERBS` set + `loadSubVerb` helper. Confirm migration path holds.
...
This is essentially what we're doing. The "alternative" is to add a `commander`/`citty` layer for no benefit. ADR-103's decision is to keep the bash shape.

## References

- PLAN.md §2 (verb list at frozen scope), §10 (CLI framework TBD note)
- ADR-095 (cold-start budget — lazy imports matter)
- ADR-130 (project layout — `src/cli.ts` + `bin/atmux-bun` shim)
- ADR-096 (module taxonomy — `cli.ts` is the only module that may dynamic-import)
- ADR-099 (error handling — UsageError, exit codes)
- ADR-107 (v2 verb redesign — preserves the dispatcher, adds nested-subcommand handling)
