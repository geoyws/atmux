...
Zod sketch (full implementation lands in T2 at `src/schema/host-registry.ts`; this is the contract):

```ts
import { z } from "zod";

export const HostRegistryEpicRow = z
  .object({
    epicId: z.string().regex(/^e-[0-9a-f]{8}$/),
    project: z.string().min(1),
    parentRoot: z.string().min(1),
    epicRoot: z.string().min(1),
    tmuxSocketPath: z.string().min(1),
    memberCount: z.number().int().nonnegative(),
    spawnedAt: z.number().int().positive(),
    lastCommitAt: z.number().int().positive().nullable(),
    lastPaneActivityAt: z.number().int().positive().nullable(),
    priority: z.enum(["normal", "emergency"]).default("normal"),
    queuedBy: z.string().min(1),
  })
  .strict();

export const HostRegistryQueueRow = z
  .object({
    queueId: z.string().regex(/^q-[0-9a-f]{8}$/),
    project: z.string().min(1),
    spawnArgs: z.array(z.string()).min(1),
...
export const HostRegistry = z
  .object({
    version: z.literal(1),
    hostCap: z.number().int().positive().default(8),
    perProjectSoftCap: z.number().int().positive().default(3),
    epics: z.array(HostRegistryEpicRow).default([]),
    queue: z.array(HostRegistryQueueRow).default([]),
  })
  .passthrough();
...
export const HostConfig = z
  .object({
    ...
    dormancy: z
      .object({
        commitMin: z.number().int().positive().default(240),
        paneMin: z.number().int().positive().default(60),
        autoRetireEnabled: z.boolean().default(false),
      })
      .strict()
      .default({}),
    queue: z
      .object({
        ...
        fullPolicy: z.enum(["hard-refuse", "evict-oldest"]).default("hard-refuse"),
      })
      .strict()
      .default({}),
  })
...
export type HostRegistry = z.infer<typeof HostRegistry>;
export type HostConfig = z.infer<typeof HostConfig>;
```

`.strict()` on rows catches typo'd field names; `.passthrough()` on the top-level admits forward-compatible additions (sentinel hooks, telemetry fields) without a schema bump.
