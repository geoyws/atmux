...
- **`src/core/boot-claude.ts`** — `bootClaudeMember()` orchestrates: (a) capture-pane sentinel check for already-booted (tokens count > 0); (b) poll capture-pane up to 30s for TUI ready (`❯` glyph or `tokens` footer); (c) `tmux send-keys` a single-line boot prompt (`"Read /tmp/atmux-brief-generic-{team}.md and your role brief if your role appears in templates/briefs/, then bootstrap as that team member. Your role is {member}."`) with `enter:true`; (d) accept the post-`C-m` verify when the composer is empty OR, only for non-forced cold/start bootstrap, when capture already proves the boot is live (`bootSignalLive` = tokens moved or active-thinking); `forceBootPrompt:true` stays composer-empty-only because `/clear` can leave stale scrollback; (e) retry the send-keys once on first-attempt miss; (f) on full failure, surface to `lead-outbox.md` via `renderBootFailureNotice` so the operator sees the undead pane on next review.
- `bootClaudeMember()` now special-cases Claude Code's exact first-run folder-trust modal before readiness: it accepts the preselected `Yes, I trust this folder` row once with bare `Enter`, performs a second capture immediately before the Enter send so a vanished modal returns without a key, returns the helper's final capture to the outer loop so a deadline-edge composer can still proceed, does not treat the modal's `❯` row as readiness, fails closed with `folder-trust-not-cleared` if the exact modal persists after that single acceptance, and lets near-match/generic permission prompts age out as `tui-not-ready` instead of auto-accepting them.
...
Tests in `tests/unit/core/boot-claude.test.ts` pin sentinel / ready-poll / exact folder-trust accept-once / retry / both-attempts-fail / send-keys-verb-failure / capture-error scenarios.
