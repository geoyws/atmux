# ADR-104: Side-by-side cutover protocol
...
- ADRs accepted (link to `docs/adr/`)
- Migration notes for operators (none expected — verb names + args identical to bash by ADR-107)
- Known deferrals (Phase 5 WIP modules — link to ADR-106)
...
- ADR-102 (parity harness shape — the source of "zero divergence" verdict)
- ADR-106 (WIP-bash deferral — keeps Phase 5 separate from cutover)
- ADR-107 (verb design debt — confirms v1 ships at 1:1 verb parity, so cutover doesn't surprise operators with renames)
