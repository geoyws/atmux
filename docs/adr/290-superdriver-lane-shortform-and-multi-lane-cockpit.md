# ADR-290: Superdriver lane shortform — cockpit window 1 `_superdriver` → `_sd`, `_sdN` multi-lane cockpit, and retirement of `/atmux:bruh` + `/atmux:bruhloop`
...
**Numbering note (recorded, not hidden):** this ADR was drafted as 287 in the `atmux-sd-lanes-geoyws-driver-1` worktree on 2026-09-02; it was renumbered to 288 the same day because the main checkout had already taken 286 and 287. On 2026-09-03 the main checkout and the driver-pair branch (`c6acab48`) independently took **288** for `288-driver-window-worker-attention-pane-pair.md` and **289** for `289-kb-authoritative-agent-continuity.md`, colliding with this document's number. kb attention `a-b25d1166` (2026-09-04) named the collision. On 2026-09-07 this ADR moved to **290** (the next free number), with every citation on this branch moving with it in the same commit. Every `ADR-290` reference in code, tests and docs on this branch means this document.
...
3. `_superdriver → _sd` (ADR-135 §D2 spelling → ADR-290 shortform)
