# ADR-107: Verb design debt — deferred v2 redesign (Phase 6)
...
**Follow-up tickets:**
- ADR-107a (Phase 6 kickoff): subcommand framework choice — does the CLI dispatcher (ADR-103) accommodate `task <sub>` / `member <sub>` cleanly, or does v2 need a refactor?
- ADR-107b (Phase 6 design): `member rm` semantics — does removing a member archive their inbox? hand off their open tasks? ask first?
- ADR-107c (Phase 6 design): deprecation warning UX — stderr line on every old-verb call? once-per-day? how do scripts opt out?
