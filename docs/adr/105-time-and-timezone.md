# ADR-105: Time + timezone handling (MYT discipline, UTC internals)
...
- All timestamps in JSON state files are **epoch milliseconds** (number). Schemas (ADR-098) declare them as `z.number().int().positive()` with field name suffix `*At` or `*Ms` (e.g. `createdAt`, `lastSeenAt`, `bootedMs`).
...
- Migrating: bash atmux uses `date +%s` (epoch seconds) in some places. ADR-098 schema validators multiply by 1000 on read if the value is < 10^11 (heuristic for "this looks like seconds, not ms"); writes always use ms going forward.
...
ADR-102 (test strategy) declares `bun:test` lifecycle resets between tests — `resetNow()` is on the test author, not auto-magic.
...
These regexes are part of `scripts/lint-discipline.ts` (ADR-099). CI fails any commit that violates.

### Logger integration

`spawn.ts` (ADR-100) and other logger-using code prefix log lines with `formatMytFull()`. Log shape:
...
- Test authors must remember to call `resetNow()` in `afterEach`. ADR-102 declares this in the test-author guide; missed reset shows as flaky time-dependent test.
...
- ADR-096 (module taxonomy — `time.ts` is one of 8 abstractions)
- ADR-099 (error handling — no time-related errors; `time.ts` is pure)
- ADR-100 (spawn pattern — logger uses `formatMytFull` for log line timestamps)
- ADR-101 (discord — header uses `formatMyt`)
- ADR-102 (test strategy — `setNow` / `resetNow` test pattern)
