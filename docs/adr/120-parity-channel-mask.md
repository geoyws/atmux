# ADR-120: Parity channel-mask contract (Option B per George 2026-05-05)
...
Phase 3 iter-1 (ADR-119) shipped the parity matrix foundation — `PARITY_MATRIX` populated for `version` + `unknown-verb` (2 rows green, both deterministic byte-equal cases) plus the `lifecycle` fixture preset. Iter-1 commit-2 prep (`8e82ed2`) and commit-4 prep (`33125ec`) probed at `/tmp/parity-probe` surfaced **two distinct classes of bash↔TS divergence** that block straightforward matrix-row activation for the remaining 8 ADR-119-tracked verbs.

### Class 1 — error-rendering style (init / start / send / add-member, ADR-119 row 6)
...
- TS emits `atmux: config: no team.json at <p> (hint: run 'atmux init' first)` to stderr / exits `78` (BSD `EX_CONFIG` per ADR-099).

Same semantic outcome (both refuse + emit a no-team hint), different stylistic surface (emoji-tag vs structured-tag prefix; `1` vs `78`). ADR-099 is the canonical decision — TS keeps BSD sysexits; bash side stays frozen.

### Class 2 — state-after non-determinism (task / dispatch / inbox / done, ADR-119 row 3)
...
Both classes are **stylistic** divergence (rendering / random IDs / timestamps), not **semantic** divergence (state mutation, business logic, authz, missing tasks). The parity gate's purpose is catching the latter, not the former. ADR-102 §3 already establishes timestamp-mask precedent for stdout (e.g. `\[\d+ms\]` durations). What's missing is a uniform per-row, per-channel mask config that lets each row declare its noise channels inline + cite the divergence class.

George's call (2026-05-05 ~20:35 MYT, Option B): **mask the noise, don't reconcile the rendering.** ADR-099 stands; bash-side error rendering is frozen; TS keeps structured-tag + sysexits. The harness adapts.
...
- **`exitCode`**: when `mask.exitCode === true`, skip the channel entirely (no `Divergence` regardless of value). Cited reason: `bash exit 1` vs `TS exit 78` (ADR-099 BSD sysexits).
- **`stdout` / `stderr`**: when `mask` is present, apply `String.replace(pattern, "")` (or empty-string substitution) to BOTH sides before byte-equal. Mirrors existing timestamp-mask precedent (ADR-102 §3).
...
2. Reference the ADR class — `(ADR-120 error-rendering class)` or `(ADR-120 state-after class)`.
...
{ verb: "task", args: ["add", "test"], fixturePreset: "lifecycle",
  ...
  mask: {
    stateAfter: {
      "kanban.tasks[*].id": /^t-[0-9a-f]{8}$/,        // reason: 8-hex random per invocation (ADR-120 state-after class)
      "kanban.tasks[*].createdAt": /^\d{10,}$/,        // reason: Unix epoch per invocation (ADR-120 state-after class)
    },
  },
},
```
...
{ verb: "init", args: ["--dry-run"], fixturePreset: "minimal",
  ...
  mask: {
    exitCode: true,                                    // reason: bash exit 1 vs TS exit 78 (BSD sysexits per ADR-099)
    stderr: /^(💥 atmux \S+ |atmux: \S+: )/,           // reason: bash emoji-tag vs TS structured-tag prefix (ADR-120 error-rendering class)
  },
},
```
...
- Reference an undefined divergence class (must be `ADR-120 error-rendering class` or `ADR-120 state-after class`, or a future class added by ADR-120 amendment).
...
- **State-mutating happy-path matrix rows.** ADR-119 row 3's 4 verbs (`task add` / `dispatch` / `inbox` / `done`) — land in commit 4 with state-after masks per §3 above.
- **Error-rendering matrix rows.** ADR-119 row 6's 4 verbs (`init` / `start` / `send` / `add-member`) — land in commit 5 with exitCode + stderr masks per §3 above.
- **ADR-099 amendment.** ADR-099 stands. TS keeps BSD sysexits + structured-tag stderr. Bash side stays frozen (out of scope per ADR-119 §Scope).
...
- **New mask classes.** Iter-2 covers error-rendering + state-after. Future classes (e.g. SHA-bearing log lines, environment-variable-injected paths) land via ADR-120 amendment + new class label.

## Migration plan (this ADR's commit chain)

1. **Commit A — `docs(adr,plan): ADR-120 — parity channel-mask contract (Option B per George)`**: this ADR file + PLAN.md §7 backlog row.
...
4. **Commit D — `test(parity): 4 state-mutating happy-path rows with masks (task/dispatch/inbox/done)`**: ADR-119 row 3 resolved.
5. **Commit E — `test(parity): 4 error-rendering rows with masks (init/start/send/add-member, ADR-119 row 6)`**: ADR-119 row 6 resolved.
6. **Commit F — `docs(adr-bun): ADR-119 — rows 3 + 6 done after iter-2 mask landings`**: ADR-119 update mirroring `33125ec` shape; rows 3 + 6 → ✅ done; §Consequences extended with iter-2 actual delivery.
...
- **Cron-aware mask harness.** Cron-fired scenarios (whip / report / decisions-digest / groom) need a frozen-clock injector (ADR-119 row 6 deferred). When that lands, the timestamp-mask channel may move from per-row regex to harness-level clock injection — separate ADR.
...
- **ADR-099 stands.** TS keeps BSD sysexits + structured-tag stderr. No per-verb mirror-bash reconciliation commits needed (the original ADR-119 row 6 plan).
...
- **Iter-2 delivers 8 new rows + harness extensions in 6 commits.** ADR-119 rows 3 + 6 close together; iter-3+ inherits a working mask vocabulary for new divergence classes.
