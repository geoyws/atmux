...
- **Tests**: covered in `rust/atmux-orchd/src/main.rs` `#[cfg(test)] mod tests` (landed by [ADR-256](256-orchd-rust-hardening.md) — this claim was a forward-reference until then; see ADR-256 §"ADR-249 coverage reconciliation"). `singleton_lock_relative_and_absolute_collide_for_same_db` + `singleton_lock_distinct_teams_never_collide` + `singleton_lock_path_for_missing_parent_falls_back_without_panicking` exercise the pure path-derivation collision matrix; `singleton_lock_second_acquire_refuses` exercises the acquire/refuse path (a same-process double-`flock` on one lockfile — the deterministic, non-flaky equivalent of "a second orchd against the same DB exits 5", since the `Err` it asserts is exactly what `main()` maps to `ExitCode::from(5)` + the `🔴 orchd singleton guard · refusing to start` log) and proves the kernel-releases-on-fd-close self-healing.
