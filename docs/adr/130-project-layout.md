...
ADR-095 commits us to a single-binary TypeScript-on-Bun port. Before any code lands, we need a directory layout that:
...
2. Enforces ADR-096's three-layer module taxonomy (abstractions / core / verbs) by physical separation, so reviewer's "abstractions don't import from core" rule has a directory boundary to lean on.
...
6. Supports v2's nested-subcommand redesign (ADR-107, Phase 6) without churning paths — the v2 split is verb-internal, not directory-internal.

The bash project today has `lib/*.sh` flat (27 files at frozen worktree HEAD), `tests/unit/*.bats`, `tests/e2e/lifecycle.bats`, `bin/atmux`. We deliberately do NOT mirror this layout — bash's flat namespace is one of the reasons we're porting (ADR-095 Pressure 1).
...
│   ├── cli.ts             # Top-level dispatch (per ADR-103)
│   ├── errors.ts          # AtmuxError base + tagged subclasses (per ADR-099)
│   ├── abstractions/      # Pure IO wrappers; no domain knowledge (per ADR-096)
│   │   ├── tmux.ts        # ADR-097
│   │   ├── json.ts        # ADR-098 (Zod parse/serialize at boundary)
│   │   ├── lock.ts        # ADR-098 (file-lock primitive)
│   │   ├── fs.ts
│   │   ├── time.ts        # ADR-105 (MYT formatter, duration formatter)
│   │   ├── spawn.ts       # ADR-100 (Bun.spawn wrapper)
│   │   ├── http.ts        # Bun fetch wrapper
│   │   └── discord.ts     # ADR-101 (named-template enforcement)
...
│   │                      # 7 aliases routed at dispatcher level — see ADR-103)
│   │   └── (filled in Phase 2)
...
- **Mirror rule.** `tests/unit/<path>/<file>.test.ts` ↔ `src/<path>/<file>.ts`. No collision, no orphan tests, no orphan source files. CI fails if a `src/**/*.ts` file (excluding generated/barrel) has no matching `tests/unit/**/*.test.ts`. ADR-102 owns the exact regex.
- **e2e tests** are file-per-verb but run against the *built binary*, not the source modules. They live at `tests/e2e/<verb>.test.ts`. The 11-step `lifecycle.bats` becomes `tests/e2e/lifecycle.test.ts` (1× cold-start non-idempotent — ADR-102 captures the docstring template).
...
- ADR-107's v2 redesign collapses some verbs into subcommands (`task claim` etc.); that becomes a folder shape (`src/verbs/task/claim.ts`) without disturbing the rest.
...
Returning the exit code (per ADR-096). The dispatcher in `src/cli.ts` resolves verb → file → run.
...
- `src/abstractions/json.ts` is generic JSON IO (atomic write, read+parse helpers); it imports from `src/schema/` rather than embedding any specific schema. This keeps ADR-098's "no `JSON.parse` in domain" rule enforceable per-schema.
...
- Reviewer's three-layer dependency rule (ADR-096) gates by directory: `src/abstractions/` MUST NOT import from `src/core/` or `src/verbs/`; `src/core/` MUST NOT import from `src/verbs/`. Static check via biome import-restriction or a small custom lint.
- Test mirror rule lets the coverage gate name-and-shame missing tests at PR time, not in mysterious lcov gaps.
- v2 redesign (ADR-107) lands by introducing `src/verbs/<group>/<sub>.ts` folders without renaming abstractions or core. Path stability across major versions.
...
- ADR-096 (module taxonomy) — formalise the dependency rule + lint that enforces it.
- ADR-102 (test strategy) — declare the exact mirror-rule regex and the `no-test` opt-out marker.
...
Rejected. Reproduces the bash flat-namespace problem (Pressure 1 from ADR-095). The whole point of the module taxonomy (ADR-096) is to make a typed module boundary; flattening defeats that boundary.
...
- ADR-095 (motivation for the port — Pressure 1 godfile drift drives the abstractions/core/verbs split)
- ADR-096 (module taxonomy — formalises the layered import rule that this layout enforces by directory)
- ADR-102 (test strategy — declares the mirror rule regex + `no-test` opt-out)
- ADR-103 (CLI dispatcher — verb file resolution lives there)
- ADR-107 (v2 verb redesign — preserves this layout; redesign happens within `src/verbs/`)
