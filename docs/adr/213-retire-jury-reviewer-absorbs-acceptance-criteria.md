...
**Status**: Accepted — ratified by driver 2026-05-21 (Jury role retires entirely; ADR-204 fully superseded; reviewer absorbs AC verification per §D2 — stories.extra.acceptance_criteria[] schema decision PERSISTS; pre-work AC ratification dropped per §D3 — reviewer auto-rejects empty/vague AC at signoff = same insurance; §D4 ADR-144 §Amendment 2026-05-21 jury-states reverted, original review state stands, gitter refuses merge without review signoff per existing flow; §D5 e-5a5110d0 EPIC dissolved 2026-05-21 same operator session — kanban EPIC walked to done; e-honker-jury consumer EPIC dropped from ADR-202 §D12, TOPICS registry drops 5 jury-prefixed topics 39→34; §D6 brief sweep — reviewer brief grows §AC-verification, planner brief grows §AC-discipline, jury brief deleted in cleanup-EPIC; §D7 doctor probe jury-config-residue merges into consolidated retired-role-config-residue probe; §OQ recommendations as-written; sibling simplification to ADR-211/212/214 — operator stance: atmux is too bloated, simplify by retiring overlapping roles where existing actors can absorb the function)
**Date**: 2026-05-21
