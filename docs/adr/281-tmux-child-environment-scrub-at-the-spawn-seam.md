...
> ⚠ **CORRECTED 2026-08-28, re-measured 2026-08-29.** This paragraph originally read "The operator's own `~/.tmux.conf` and `templates/tmux/atmux.conf` **keep** their `COLORTERM truecolor` line, deliberately and unchanged", and §Consequences said the same. **`templates/tmux/atmux.conf` had no such line.** Its only `set-environment` was `NO_COLOR`; its only `COLORTERM` occurrence was inside a comment. The line existed solely in the operator's personal `~/.tmux.conf`, which this repository does not ship — so the compensating control this section leaned on did not exist in anything atmux deploys. The line is in the shipped conf now, after the ADR-277 scrub and above the ADR-171 `source-file`, with two regression legs keeping it there.
>
> **What the line is, stated as measured rather than as hoped** (geoywsMBP, tmux 3.7c, 2026-08-29, fresh `mktemp` sockets, legs differing only by the line): it is a **cheap defensive default**, not a fix for any pane fault measured in this repository. It is **inert for the pane** — a pane on a server whose environ carries `COLORTERM=` empty reports `COLORTERM=truecolor` with the line and without it, because tmux sets the variable per pane itself. It is **safe** — with it set server-side and the client carrying no `COLORTERM`, `#{client_termfeatures}` came back `bpaste,ccolour,clipboard,cstyle,focus,title`, byte-identical to the no-line leg and carrying **no RGB**; adding `COLORTERM` to the CLIENT's environment is what adds `RGB`. That asymmetry is the whole of §D2's argument and it reproduces.
>
> ⚠ **A causal story previously attached to this line is WITHDRAWN as fabricated.** Three places asserted the line existed "for the tmux 3.6b servers ADR-281 measured with `COLORTERM=` empty, where the pane does inherit the frozen value". **No such measurement was ever made** — this ADR says the opposite two paragraphs above ("nothing in this ADR has been measured on either"), and `grep -rn '3\.6b' docs/ templates/ src/ tests/ CHANGELOG.md` found only the pointer sentences citing each other. What IS measured is a **server-environment survey**, not pane inheritance: of 47 live servers on this box, five carried `COLORTERM=` empty in their global environment and two carried `NO_COLOR=1`, and the affected servers report `TERM_PROGRAM_VERSION` 3.6b while healthy ones report 3.7c. Whether a pane on such a server inherits that empty value is **unmeasured and unmeasurable here** — no 3.6b binary is available. The error came from the instructions the implementing agent was given, not from its own work.
...
## Supersession 2026-09-01

The historical empty-config proof above stays as history. Current live-server tests load the canonical atmux conf via `getAtmuxTmuxConfPath()`, so the live guidance is no longer `/dev/null`. The spawn-seam invariant is covered by `tests/unit/abstractions/tmux-child-env.test.ts` using the spawn recorder/`true` and no tmux server. `tests/regression/atmux-conf-no-color-scrub.test.ts` keeps the real-conf behavior and its negative control through the documented user override layer.
...
- ~~**Nothing in this change writes to a tmux conf.** `~/.tmux.conf` and `templates/tmux/atmux.conf` keep their `COLORTERM truecolor` lines untouched and deliberately.~~ **CORRECTED 2026-08-28** — `templates/tmux/atmux.conf` had no such line to keep, so this bullet described a compensating control that did not exist in anything atmux ships. It does now: the line was added, after the ADR-277 scrub and above the ADR-171 `source-file`, as a measured-safe default (see the §D2 note). What stands unchanged: server-level `COLORTERM` is fine, only atmux's process-level injection is withdrawn, and the operator's own conf still loads last and still wins.
- Guarded by `tests/regression/atmux-conf-no-color-scrub.test.ts`, which keeps the real-conf behavior leg and the documented user-override negative control. The 2026-08-28 empty-config proof with `-f /dev/null` is historical; the live harness now uses the canonical atmux conf path. The no-conf spawn-seam invariant is covered by `tests/unit/abstractions/tmux-child-env.test.ts` via the spawn recorder and `true`, with no tmux server started. That is the shape the conf-only guard structurally could not test, since every pre-existing leg passed `-f CONF_PATH` and so could only ever prove the conf was correct, never that it arrived.
...
| `zsh -c` | 10 | **0** |
| `zsh -lc` (login) | 11 | **0** |
| `zsh -lic` (login + **interactive**) | **81** | **25** |

**Reproduced independently 2026-08-29** with a script that emits only integers (`wc -l`, `grep -c` — no path in it can print a name or a value), seeding `env -i` with `HOME`/`PATH`/`TERM`/`LANG` and counting against this repository's own `SENSITIVE_NAME_RE`:

| child shell | names | credential-shaped |
|---|---|---|
| `sh -c` | 7 | **0** |
| `zsh -c` | 9 | **0** |
| `zsh -lc` | 9 | **0** |
...
The absolute counts differ from the row above (a different seed set and a different credential pattern), and the finding is the same one and is decisive: **every non-interactive form carries zero credential-shaped names, and adding the single `-i` flag brings in dozens.**
