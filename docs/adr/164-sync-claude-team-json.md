...
`.atmux/team.json` is the **canonical** team roster atmux reads for every verb (`dispatch`, `send`, `rotate`, `start`, `stop`, etc.). It is fresh, schema-validated (per historical decision number 005 (no surviving ADR file)), and in lock-step with the live cage.
...
- **Drift detection** surfaces hand-edits between syncs (operator added a brief; sync detects file changed; refuses without `--force`). This is the same shape as historical decision number 054 (no surviving ADR file) §"drift detection" — pattern reuse, not new mechanism.
...
- historical decision number 054 (no surviving ADR file) — typed-config + drift detection pattern reused here.
