...
OQ-A from this ADR's §Open questions is RESOLVED (split chosen). See ADR-226 §Open questions for OQ-B-equivalent (deprecation alias lifetime, carried forward) + new OQ-3 (`spawnPressureDeferred` reset semantics) + new OQ-4 (orchd-domain observability events).
...
1. **OQ-A: Should ADR-224 §Phase 2 split into a sibling ADR at Phase 2 dispatch?** — **RESOLVED 2026-05-23 (planner, t-64bbc474)**: split to ADR-226. ADR-225 was claimed by sibling EPIC e-cf8a6195 (`epic-dependencies-and-is-ready-toggle.md`) in the interim; the split lands at ADR-226 instead of the originally-pinned 225. See §D4 above for the consumer-vs-substrate rationale.
