...
**Status**: Accepted (Phase 1) — ratified by driver 2026-05-23 (Phase 1 rename shipped via merge `f6b078b`: src/verbs/orchd.ts + src/core/orchd-window.ts + rust/atmux-orchd/ + ORCHD_WINDOW + atmux start incremental-mode auto-rename + relayd deprecation alias + reviewer trunk-signoff `d0a808f`). Phase 2 auto-spawn loop status: **proposed (deferred:** gated on sibling EPIC `e-cf8a6195` `is_ready`+`depends_on` schema landing). Phase 3-5 (auto-merge / auto-dissolve / throttle-queue) tracked in sibling ADR-226/227/228 under EPIC `e-a946af69`.
**Date**: 2026-05-22
...
### D4 — Phase 2 auto-spawn loop — SPLIT to [ADR-231](231-orchd-auto-spawn-and-solo-worker-dissolve.md) (decided 2026-05-23, OQ-A resolved; renumbered from ADR-226 → ADR-231 per OQ-PLANNER1 reconcile against parent trunk's concurrent ADR-226 'auto-merge subscriber')

Phase 2 semantics (subscribers, autoSpawn config, dedup, cron backstop, failure recovery, solo-worker dissolve) live in **ADR-231** (originally drafted as ADR-226 at commit 029ae9c; renumbered 2026-05-23 14:00 MYT after parent trunk's ADR-226 'orchd auto-merge subscriber' shipped concurrently via sibling EPIC e-a946af69 commit 8d75360). Split rationale per the heuristic in this section's prior wording: §Phase 2 introduced a new top-level concept (consumption pattern joining ADR-224 §D6 registry seam + ADR-225 eligibility substrate). Sibling ADR-225 (epic dependencies + `is_ready` toggle) landed on parent trunk at commit 4870833, making it possible to write the consumer ADR concretely instead of speculatively — that tipped the heuristic.

**One-line summary of ADR-231 for readers staying inside ADR-224**:

- orchd Phase 2 subscribes to `epic.ready` + `epic.unblocked` (NOT the original `epic.added` sketch — ADR-225 events align with eligibility transitions, no wasted spawn-epic refusal cycles).
- Combined predicate: `is_ready=1` AND all `depends_on` done (delegates to ADR-225 `epicIsEligible()`) AND `extra.autoSpawn.enabled=true` (per-epic explicit OR per-team `team.json::autoSpawn.defaults[]` regex match against title) AND `spawned_at IS NULL` (dedup).
- Spawn handler invokes `atmux team spawn-epic --epic <eid> --roster <r>`; on success SET spawned_at. On failure: hard-failure flag, host-pressure transient defer (≥3 → separate flag), eligibility-race silent.
- `task.done` subscriber dissolves solo-workers when all owned tasks done (closes ADR-221 §Phase 2).
- Cron `atmux orchd --sweep` (default 5min) is the backstop walker; reuses handlers, doesn't duplicate logic.
- Handler-as-data pattern: orchd.ts stays <100 LOC; handlers live in `src/core/orchd-{spawn,dissolve,sweep}.ts` and register by appending to `ORCHD_SUBSCRIPTIONS`.

OQ-A from this ADR's §Open questions is RESOLVED (split chosen). See ADR-231 §Open questions for OQ-B-equivalent (deprecation alias lifetime, carried forward) + new OQ-3 (`spawnPressureDeferred` reset semantics) + new OQ-4 (orchd-domain observability events).
...
1. **OQ-A: Should ADR-224 §Phase 2 split into a sibling ADR at Phase 2 dispatch?** — **RESOLVED 2026-05-23 (planner, t-64bbc474)**: split to ADR-231 (originally pinned ADR-226, drafted at 029ae9c; renumbered 2026-05-23 14:00 MYT per OQ-PLANNER1 after parent trunk's concurrent ADR-226 'orchd auto-merge subscriber' shipped via sibling EPIC e-a946af69 commit 8d75360). ADR-225 was claimed by sibling EPIC e-cf8a6195 (`epic-dependencies-and-is-ready-toggle.md`) in the interim; the split lands at ADR-231 instead of the originally-pinned 225 / interim 226. See §D4 above for the consumer-vs-substrate rationale.
