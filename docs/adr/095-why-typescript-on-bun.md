...
3. **`Bun.spawn`** — typed stdio/stderr capture, structured `stderr` stream, kill/timeout primitives. We replace 45 `tmux` shell-outs and 157 `jq` shell-outs with a single typed wrapper (per ADR-100). Most `jq` calls disappear entirely once we hold parsed objects in TS.
...
- `2>/dev/null || true` has no equivalent in TS. Throw vs. return-null becomes a typed decision per ADR-099.
...
- Runtime dependency on Bun ≥1.3.13 (mise-managed). Operators on hosts without mise need a one-line install. Documented in cutover ADR-104.
...
- Bun's lock-primitive ecosystem is less mature than Node's `proper-lockfile` or libc `flock(2)`. ADR-098 chooses between hand-roll vs npm package and runs a 1000-iteration concurrent-write smoke before accepting.

**Follow-up tickets:**

- ADR-130 (project layout) — lock down `src/`, `tests/`, `schema/` shape.
- ADR-098 (JSON + locking) — pick lock primitive after bake-off.
- ADR-100 (subprocess spawn) — wrap `Bun.spawn` with timeout + stderr capture so we never silently drop subprocess errors (closes Pressure 2 at the spawn boundary).
- ADR-104 (cutover protocol) — pin Bun version in operator install path; document mise install.
...
- ADR-130 (project layout, follows from this), ADR-096 (module taxonomy), ADR-098 (JSON + locking), ADR-100 (Bun.spawn pattern)
