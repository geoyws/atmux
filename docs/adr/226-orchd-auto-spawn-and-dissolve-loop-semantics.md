# ADR-226: orchd auto-spawn + auto-dissolve loop semantics — Honker consumer of ADR-224 §D6 registry + ADR-225 eligibility substrate

**Status**: proposed (deferred: implementation lands across e-60e16169 Phase 2 tasks t-dd42dbc4 / t-b673421f / t-13acce38 / t-f6107a4b / t-30752bb7; status flips to `accepted` at P2.T9 reviewer trunk-signoff per the standard ADR lifecycle for this team)
**Date**: 2026-05-23
**Driver-ref**: EPIC e-60e16169 Phase 2 (master design-task t-10d9f702 in parent atmux kanban, OQ-A resolution in `.atmux/decisions.md` D7). Split decision per ADR-224 §D4 heuristic: §Phase 2 introduced a new top-level concept (consumption pattern for sibling ADR-225's eligibility substrate) AND the per-decision narrative is cleaner in two ADRs than one. ADR-225 (sibling EPIC e-cf8a6195, accepted) became available on trunk via commit 4870833 — making it possible to write this consumer ADR concretely instead of speculatively.
