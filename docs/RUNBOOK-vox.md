...
| Tool answers about the wrong team | Team resolution walked to a different cockpit entry | Resolution is a ladder (exact → case-fold → suffix-strip → unique prefix → Levenshtein ≤2); an ambiguous utterance returns `ambiguous_team` with candidates rather than guessing. `atmux vox --status` shows the provider the running server actually dialled; `list_teams` shows what the index actually holds. |
