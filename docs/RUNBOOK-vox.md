...
| `ATMUX_VOX_READONLY` | unset | no | `1` ⇒ only the 14 read tools exist (12, plus `host_pressure` + `token_budget` from [ADR-273](adr/273-voice-fleet-triage-and-pane-input.md) §Supplement-6); the 4 messaging tools **and `pane_nudge`** are **absent from the catalog**, not merely refused at call time. **This was the setting the feature first shipped in**, which is why `pane_nudge` was unreachable despite being built. Carries a `SUNSET` marker per [ADR-266](adr/266-shim-sunset-policy-and-first-sweep.md) §D1. **CLEARED 2026-08-21 (now `0`) once O2 landed — see §5.** The live posture is whatever `/healthz` reports, never whatever your shell exports; re-arming means setting it back to `1` in `_dotfiles/atmux/vox-launch.sh` and re-running that launcher, because the supervisor holds this variable in its own env and re-exports it to every respawn — killing the Bun child alone brings the old posture straight back. |
...
**Phase O2 — an auth layer in front of Bun.** An unauthenticated request never reaches Bun. Before it, the token is the whole perimeter — and note the token is validated *by* Bun, so under O1 the request has already arrived. `ATMUX_VOX_READONLY` is cleared **after** O2, never before.

> **✅ O2 LANDED 2026-08-21 — via nginx basic auth, not oauth2-proxy, and `ATMUX_VOX_READONLY` is now `0`.**
>
> The OIDC path is still documented in [`deploy/O2-oauth2-runbook.md`](deploy/O2-oauth2-runbook.md) and remains the stronger option; what shipped is that runbook's own §"Alternative: nginx basic auth". Two reasons it was chosen: OIDC has a long-lead prerequisite (Keycloak client + group + two secret files + a compose stack), and oauth2-proxy in front of `/ws` **breaks `scripts/vox-probe.ts`**, which is cookie-less and bearer-authenticated and is how V-3/V-4 are verified on every deploy.
>
> As deployed: `auth_basic` on the page `location /` **and** on `location = /ws`; **`/healthz` deliberately left open** (uptime monitoring depends on it and it discloses only `{ok, provider, readonly}`). `/ws` additionally carries `satisfy any` + `allow 127.0.0.1` — load-bearing, because a request has only **one** `Authorization` header, so a naked `auth_basic` there would make the bearer probe and the browser mutually exclusive. (The OIDC runbook's line about the probe sending "Basic alongside its existing bearer token" is not achievable as written; the loopback exemption is what actually works.)
>
> Verified by attributing each `401` to the layer that issued it, via the presence/absence of `WWW-Authenticate`: `/ws` unauthenticated → **nginx** (html, 188 B, header present); `/ws` with basic credentials → **Bun** (`text/plain`, 13 B `unauthorized`, no header), i.e. nginx passed it through and Bun then demanded the bearer token. Two independent layers. Loopback probe re-verified `ok=true` after the change.
>
> **Honest bound:** basic auth is weaker than OIDC — no SSO, no group-based revocation, no MFA, no session expiry. Revoking means rotating `/etc/nginx/htpasswd.atmux-vox`; if the phone is lost, rotate. If OIDC lands later this comes out in one edit. Credential: `ATMUX_VOX_BASIC_PW` (user `geoyws`) in the git-crypt'd store, pointer row in `keys/KEYS.md`.
>
> **D7 caveat unchanged:** the server enforces the confirm token's binding, TTL and single use, but nothing server-side observes the operator saying yes.
...
| V-17 | Read-only kill switch | With `ATMUX_VOX_READONLY=1`, the 4 messaging tools are **absent from the catalog the model receives** — not merely refused on call. Ask the assistant to send a message: it reports it has no such capability. | placeholder — ⚠️ **the live server has run `readonly=0` since 2026-08-21 (§5), so this row cannot be exercised as-is.** Re-arming to test it means setting `ATMUX_VOX_READONLY=1` in `_dotfiles/atmux/vox-launch.sh` and re-running that launcher (the supervisor re-exports the variable to every respawn, so killing the Bun child alone will not do it), then restoring `0` afterwards. Do NOT infer the posture from your own shell — read `/healthz`. |
