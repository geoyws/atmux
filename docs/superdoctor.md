...
ADR-077 §D1 + §D2 (cockpit topology + schema), §F1 (skill brief in `~/.claude/skills/superdoctor/`), §F2 (complaint box SQLite + `atmux complaints` verb), §F3 (`atmux send __superdoctor__` validator), §F4 (P0 send-keys runbook), §F5 (status verb superdoctor surface), and §F6 (self-escalation primitives — `superdoctor_attempts` table + `renderSelfHealFailed` Discord template) all ship. Setting `superdoctor.enabled: true` in `~/.atmux/cockpit.json` and running `atmux cockpit rebuild` spawns window 2 with a Claude Opus session that — when invoked as `/loop /superdoctor` — runs the hourly diagnosis loop end-to-end.
