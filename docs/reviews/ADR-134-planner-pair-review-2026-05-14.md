...
2. **PR-mode interaction** — ADR-091 pre-flag #8 specifies `mergeMode: "pr"` as schema-valid-but-runtime-noop. ADR-134's `autoMerge.testCommand` runs LOCAL bun-test; what happens under PR-mode where the test should run via CI on the PR rather than locally? Out of v1 scope per ADR-134 §Out of scope; explicit defer to ADR-179b/c fold-in is well-framed.
