...
## Running the tests — `bun scripts/test.ts`, not a bare `bun test` ([ADR-283](adr/283-scrub-the-test-runner-environment.md))

**`bun run test` and `bun scripts/test.ts <args…>` are the way to run the suite.** A bare `bun test` on a machine whose shell carries credentials is **refused** by a bunfig preload, with a message naming the safe form.

The reason is a disclosure, not a preference. A failing assertion prints its received value in full, and on 2026-08-28 one red test put ~180 environment variables **with their values** — live API tokens, database and documentation passwords, Discord webhook URLs — into a test log and an agent transcript, because the operator's `.zshrc` sources a git-crypt'd `.env` into every shell.

Two layers, and they cover different things:

- **`scripts/test.ts` is the wall.** It builds the child environment from an allowlist (`tests/helpers/test-env.ts`) and execs `bun test` with it, so the credentials are **not in the runner process at all**. Measured on `geoywsMBP`: 21 variables through, 103 withheld. Nothing a test does — however it looks for them, in whatever shape — can reach a variable that is not there. Arguments pass through unchanged, and CI uses the same door.
- **`tests/helpers/test-env-guard.ts` is the tripwire.** A bunfig preload, so it runs however `bun test` was invoked, refusing when the runner's own environment still carries credential-shaped names. It **refuses** rather than scrubs, because it cannot scrub: `Bun.spawn` without an explicit `env` uses the environment as it stood at process start, so deleting from `process.env` would leave `Bun.spawnSync({ cmd: ["env"] })` still dumping everything.

The two check each other: `scripts/test.ts` sets the `ATMUX_TEST_ENV_OK` marker the preload looks for **only** when the environment it built has zero credential-shaped names left, so a scrub that regressed would be caught by the preload rather than waved past by its own marker.

Substitutions for the runbooks: `bun test <file>` → `bun scripts/test.ts <file>`; `bun test --coverage` → `bun run test:coverage`. Overrides, both deliberate and both logged: `ATMUX_TEST_ENV_PASSTHROUGH=NAME1,NAME2` admits named variables through the wall; `ATMUX_TEST_ENV_OK=1` accepts an unvetted environment outright.

`tests/regression/no-unfiltered-env-dump.test.ts` remains as a source-level tripwire over `src/`, `tests/`, `scripts/`, `bin/`, `templates/` and `plugins/` — it catches the enumerated capture shapes at the place they are written, and it is **not** a proof of absence; ADR-283 §Residual gaps says what it does not see.
