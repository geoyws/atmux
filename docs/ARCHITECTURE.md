...
**Deploy guard — installing/upgrading never cycles a live cage:** `install.sh` is `git pull --ff-only` plus relinking the two symlinks (`bin/atmux`, `bin/atmux-tmux`) into the target bin dir (`install.sh:26-32,43-50`, writable-or-sudo branches); the new code goes live on the next invocation because `bin/atmux` is a bun shim that forwards to `src/cli.ts` — install touches no tmux server. And `atmux cockpit reconcile` (the canonical workhorse per [ADR-235](adr/235-cockpit-verb-surface-rationalization.md) §D1) preserves live cages by default: its Phase 2 cage loop probes each cage with `cageAlive()` (server up + a pane whose current command is `claude` or `node`) and logs `cage alive — skipping cycle (use --force-cycle to override)` instead of restarting (`src/verbs/cockpit.ts:1001-1005`). Cycling a live cage needs the explicit double gate `--force-cycle --acknowledge-dangerous-bau-interruption --yes` (refusals at `src/verbs/cockpit.ts:812` and `:830`), and `reload` is `reconcile --no-cycle --no-launch` (`src/verbs/cockpit.ts:675-682`) — so upgrade-then-reload is safe against running claude TUI contexts.
