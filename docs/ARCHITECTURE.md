...
**Driver panes never receive automated send-keys.** `pane.sendKeys` / `buffer.pasteBuffer` throw `DriverSendKeysViolation` for `driver` / `driver-N` targets before tmux is touched; the operator's 2026-09-08 revocation of the pane-to-pane ban (capture-first, `dialog`/`draft` refused) governs human-driven pane input only and does not relax this product refusal ([ADR-239](adr/239-three-driver-minimum-per-team-and-no-sendkeys-invariant.md) §D2, [ADR-293](adr/293-driver-send-keys-guard-kept-after-2026-09-08-revocation.md)).
