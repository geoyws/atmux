...
This section documents the proposed contract, not an activation procedure. Source implementation, process-level shadow proof, isolated-tmux offer simulation, static fleet-config validation, and a separate operator activation decision are required before any live cockpit reconcile or team rebuild. Do not use `atmux cockpit rebuild` to experiment with this proposal on the live cockpit.
