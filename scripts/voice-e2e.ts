...
// reasoning that keeps `voice-live-smoke.ts` out. The orchestration it
// drives (`src/core/voice/e2e/**`) IS unit-tested against fakes, so the
...
// of that — see `src/core/voice/e2e/isolation.ts`. It never touches the
...
//   bun scripts/voice-e2e.ts --keep-temp        # leave the cage for inspection
//
...
import { formatOutcome } from "../src/core/voice/e2e/judge.ts";
import { formatDriveResult } from "../src/core/voice/e2e/drive.ts";
import { SCENARIOS, scenarioById, scenarioIds } from "../src/core/voice/e2e/scenarios.ts";
import { VOICE_DEFAULTS } from "../src/core/voice/config.ts";
import { applyDriverScope, buildVoiceDeps, startVoiceServer } from "../src/verbs/voice.ts";

const USAGE = "bun scripts/voice-e2e.ts [--scenario <id>] [--keep-temp] [--list]";
...
function parseArgs(argv: ReadonlyArray<string>): Args {
  ...
  while (i < argv.length) {
    ...
    throw new UsageError({ what: `voice-e2e: unknown arg: ${a ?? ""}`, hint: USAGE });
  }
  return out;
}
...
function requireEnv(name: string): string {
  ...
  if (v === undefined || v.length === 0) {
    throw new UsageError({
      what: `voice-e2e: ${name} is not set`,
...
const io: HarnessIo = {
  ...
  log: (line) => {
    process.stderr.write(`voice-e2e: ${line}\n`);
  },
};
...
async function main(argv: ReadonlyArray<string>): Promise<number> {
  ...
  const scenarios = (() => {
    ...
    if (one === null) {
      throw new UsageError({
        what: `voice-e2e: unknown scenario '${args.scenario}'`,
  ...
  const startServer = async (opts: {
    ...
    if (process.env.ATMUX_COCKPIT_CONFIG !== opts.cockpitPath) {
      throw new Error("voice-e2e: cockpit env drifted before server start — refusing");
    }
    process.env.ATMUX_VOICE_TOKEN = opts.token;
    // Pick a concrete free port rather than passing `port: 0`.
    //
    // `resolveVoiceConfig`'s `resolveNumber` accepts a flag only when it is
    ...
    if (port === VOICE_DEFAULTS.port) {
      throw new Error(`voice-e2e: refusing to bind the default voice port ${port}`);
    }
  ...
  const result = await runHarness({
    ...
    ttsCacheDir: join(tmpdir(), "atmux-voice-e2e-tts"),
